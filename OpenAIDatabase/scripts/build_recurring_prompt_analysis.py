...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Explicit generated-analysis output. Use a disposable build path; "
            "the canonical human directory is updated only through "
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Explicit generated-status output. Use a disposable build path; "
            "the canonical human directory is updated only through "
            "update_human_readable_recurring.py."
        ),
    )
    ...
    parser.add_argument(
        "--as-of",
        help=(
            "ISO-8601 analysis timestamp. Required for deterministic CI; "
            "defaults to current UTC only for local preview."
        ),
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Write directly to the requested output paths; intended only for "
            "disposable CI build directories."
        ),
    )
    return parser
