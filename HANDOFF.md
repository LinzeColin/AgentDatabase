...
- **在办：#162 William Andrew Paton**（财务合规师，1889–1991）。
  ...
  - **卡在第 1 轮盲判的装置上**，两次被自己预登记的规则拦下：
    ...
    ② **规则 4 均长比**：候选/基线 = **1.28（v1）／1.77（v2）**，界是 0.8–1.25；
       ...
       **候选侧读了产物就是会写得更长。**
  - **现在在做**：两侧**同时**重跑，加**同一条**长度约束（每题 350–650 字）。
    ★ **不是让基线写长、也不是让候选写短去凑**——是对两侧施加同一约束控住混杂。
  - ★★ **已跑的次数如实记在 `evals/第1轮-协议记录.md` 的两条偏离记录里**：
    基线 2 遍、候选 3 遍，**每一次都不是因为答得不好，也没有据此挑**。
