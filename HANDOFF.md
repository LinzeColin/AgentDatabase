...
1. `~/Documents/Codex/GithubProject/README.md` 的 **「七条铁律」**
   （2026-08-11 复核：仍在**第 23 行**）—— 本机所有仓共用的硬约束。
   **代价最高的三条**摘在下面第 5 节。
   ★★ **这份不在本仓里，是本机 `GithubProject/` 根目录下的文件**——
   `git pull` 拿不到它。本机接手没问题（文件就在那儿）；
   **换一台机器接手，要先把它带过去，否则七条铁律无从读起**。
2. `CodexSkills/skill_log_evals/persona-distiller/_ledgers/_每次开工必读.md` ——
   这个项目自己的开工必读（**在本仓里**，`git pull` 就有）。
   ★ 2026-08-11 新增「第二步半：抓源一结束先做这两件」。
...
> ## ★★★★ 同日**最晚**的一段（**这一节最新；下面所有内容都在它之前**）
>
> ### ★★★★★ 推送会被自己的闸拒绝——**2167 MB vs 上限 200 MB**
>
> **8-13 按下 `git push` 会失败。** 用 `pre-push` 钩子自己的公式实测：
>
>     远端没有 claude/character-distillation-skill-reorganize-d57595 这个分支
>     → 闸走兜底口径 `HEAD --not --remotes`
>     → 新增 blob 10,183 个、**2167 MB**（LIMIT_MB=200）→ **拒绝推送**
>
> ★ 此前记的「新 blob 33 MB、pre-push 放行」是在**远端已有该分支**时量的；
>   现在远端没有它，兜底口径覆盖的是**所有远端够不到的对象**，两者不是一个数。
>
> ★★ 我在这一步连错两次，都记下来：
>   1. 第一次用 `origin/main..HEAD` 且 `--batch-check` 漏了 `%(rest)`，**算出 0.0 MB**；
>   2. 第二次以为闸也漏了 `%(rest)`——**去读代码发现它早用 `awk '{print $1}'` 切掉路径了**，
>      注释里写着 2026-08-10 已修（第一版拿 1.9 GB 真分支撞过、放行了）。**我的警报是错的。**
>   3. 第三次 `git rev-parse` 对不存在的 ref **把 ref 名打到 stdout**，
...
>
> ### ★★ 指针**已经在仓里了**——在 `locator`，不在 `url`
>
> 「仓里只放指针」要求语料移走后还能重取。实测 2065 条台账：
>
>     locator 有值        1993（96.5%）
>       ├ 完整 http URL      509
>       ├ `item <IA 条目号>` 1483   ← 可机械拼成下载地址
>       └ 纯 IA 条目号          1
>     locator 空            72（3.5%）
>     checksum 有值       2065（100%）  ← 只能校验，不能重取
>
> ⇒ **移走 `_corpora` 之后，96.5% 的源凭 `locator` + `checksum` 可以重取并逐份校验。**
>   要补的只有那 72 条。
>
> ★ 我第一遍量的是 `url` 字段，得到「只有 25/2065（1.2%）有指针」，
>   据此差点写出「甲 不成立、要先补 2040 个 URL」。**字段选错了。**
>   这是本轮第三次同类错（前两次：`origin/main..HEAD` 当成推送范围、
>   `git rev-parse` 对不存在的 ref 把 ref 名打到 stdout）。
>   ⇒ **报一个比例之前，先确认分子分母取的是不是同一件事。**
>
> ### 那 72 条缺口**集中在已延后／已拒发的人物身上**
>
>     wip-paton-162      24   记延后（装置不成立）
>     wip-cicero-166     19   两轮 delta 都不作数
>     wip-nasmyth-153    13   记拒发
>     wip-pacioli-161    10   记延后
>     wip-semmelweis-105  6   ← **唯一一个已入库人物**
>
...
>
> ### ★★★★★ 甲 的实际代价是 **9.8 MB**，不是 153 MB
>
> 「不含 `_corpora` 的分支」到底要推多少字节，实测（**只看当前树，不看历史中间版本**）：
>
>     远端可达对象      24,180
>     HEAD 上非语料文件  8,978
>     其中远端没有的     **516 个 blob = 9.8 MB**（闸 200 MB）→ **过闸，差 20 倍**
>
> ⇒ 一个不含 `_corpora` 的孤儿分支**几乎什么都不用推**——
>   非语料部分的 8,978 个文件里，**8,462 个远端已经有了**，
>   真正新增的只有今天与近日改动的那 516 个。
>
> ★ 我先前按「路径拆解」估的 153 MB **是高估**：那是 `HEAD --not --remotes` 里
>   非 `_corpora` 部分的**全部历史 blob**（含每一个中间版本），
>   而真正要推的只有**当前树**里远端没有的那些。
>
> ★★ 这一步我又差点栽：第一版写 `git rev-list --objects --all --remotes`，
>   `--all` 把**本地分支也算成远端已有**，得到「远端没有的 blob：0 个」。
>   去掉 `--all` 才是 516 个。**同一类错本轮第五次。**
>
> ### 结论
>
> **甲 在字节上几乎零成本**（9.8 MB），在可恢复性上 96.5% 有指针、
> 72 条缺口无一属于已入库人物。**它不是权衡，是执行问题。**
>
> ### 这一条只能由人定，选项与代价
>
> | | 做法 | 代价 |
> |---|---|---|
> | 甲 | 语料另存 Release／私有仓，仓里只放指针 | **你 8-11 已经这样答过**，但当时的分支还没长到 2.1 GB；要真做得先把 `_corpora/*/raw/` 从历史里摘出去 |
> | 乙 | 抬高 `LIMIT_MB` | **等于废掉这道闸**——它就是为拦 2.6 GB 那次事故而立的 |
> | 丙 | 只推判据与台账，语料留在本机 | ChatGPT 接手后**跑不了任何回扫**（判据要读 `raw/`） |
>
> ★ 我不自行选：三条都不可逆或都会让接手方拿到残缺的东西。

>
> ### 人物
>
> - **#171 Kelsen 全流程做完，只卡在待裁定 ㊳**：三道研究稿（writings／expression／decisions）、
>   11 条断言、十份产物、16 道用例全部落盘；研究门通过，发布门只剩两条判分步骤。
>   ★ `02/04/06` 三道**在台账里分到 0 份源**，三道就是这个人物的上限，不是没写。
>   ★★ `05-decisions` 产出 **0 条断言**——两份源全禁引且其一为三人合编，
>     连 `§` 这种结构计数都不能用（**54%／82% 是 OCR 把词尾 s 读成 `§`**：`be§`=des、`ba§`=das）。
> - **#172 前置**：`next_person.py` 推出的 H. L. A. Hart（1907–1992）**记延后**——
>   结构性不可能（1930 年时 23 岁，代表作 1961，按现行分界 2057 年解锁），同 Nowlan #155。
>   现 NEXT 是 **Louis Brandeis**（1856–1941），预筛已写在 `_corpora/wip-brandeis-172/00-预筛.md`。
>   ★ 他与刚记拒发的 Holmes #170 **同一职业剖面**，Holmes 那轮量到的
>     「多数意见 vs 异议第一人称差 35–65 倍」**必须在抓源之前先算**，否则又是一整轮白费。
>
> ### ★★★★★ 一件事故：**我看见了 Kelsen 的 holdout（5%，含完整目录）**
>
> 一句调试 `print` 把 `load_corpus` 的返回值整个打了出来，而它**按台账加载全部源、含 holdout**。
> **判据体系里没有一道门看得见这件事**——两道 holdout 门都在查文件，这次泄漏一个文件都没动。
>
> 已做：划暴露区（取宽 6.86%，可用面 93.14%，落在
> `workspaces/hans-kelsen/reports/holdout-exposed-to-researcher.json`）、
> 出题三规则、防呆写进 `_每次开工必读.md` 第三条、**并落成一道真的门**（见下 6.）。
> **要用户裁的只有一条（㊳）**：这条保留意见要不要让 Kelsen 直接记「装置不成立」。
>
> ### 判据层（**接手后会直接影响你，全部已配正反自测＋变异验证**）
>
> 6. **`load_corpus` 现在返回三元组 `(train, unread, holdout)`**，`verify()` 只拿 train 核；
>    研究道引到 holdout 报成**独立的一类重错「隔离破了」**，不再和「对不上」混为一谈。
>    ★ 这是**故意的破坏性改动**：旧写法 `corp, unread = load_corpus(ws)` 会当场 ValueError。
>    ★★ `split` 缺失一律按 **train** 处理——不许把「不知道」当 holdout。
>    全库回扫：29 个有可加载 holdout 的工作区，**引 holdout 0 条**（另 7 个没有 holdout，那里的 0 是空的）。
> 7. **`check_lane_quotes_verbatim` 补上跨行反引号**（同一个洞的第三次）。
>    此前只认单行反引号，**跨行的引文一条都没验过**而它照报「通过」。全库 73 条在这个盲区里。
>    修后当场抓到 2 条真错，其中一条是我自己吞掉了两个 OCR 版口杂字。
>    ★ 判「是不是引文」靠**结构**（真引文里没有空行／标题／列表／表格竖线／`**`／`★`），
>      删掉那道结构判据会多出 **135 条**假引文。
> 8. **`check_first_person_density` 的语种关不再抽样**。
>    原写法只看排序最前的 6 份，**能被文件排序绕过**：Koch（74% 非英文）照样印出 `0.07/万字`——
>    **那正是它自己文档里举的、说「给了数就会被人当成薄」的那个数**；Galen 0.47、Slavyanov 0.02 同样漏放。
>    现改为**按字符加权判全量**、密度只除可读语种的字符、可读占比一律打印。修后这 6 个全部改判不适用。
>    ★ **没有找到被这个数带偏的已发布判断**（靠它做的两条延后语料都是 100% 英文）。
> 9. **`quality_check.non_placeholder` 不再把机械导出的表格算进「人写了多少」**。
>    此前**跑一个 `emit_lane_scope.py`、一个字不写，研究完成门就过**。
>    全库影响面**只有 Kelsen 一个**，35 个已入库人物都不靠这个漏洞。
> 10. **20 处不存在的版本号 `v0.0.0.155` 已按各自提交日期改成日期**——
>     清单在 `v0.0.0.154`，而用户 2026-08-04 已裁定「判据落地算工具改动、不动版本号」。
>
> ### 台账新增（接手要读）
>
> - `_ledgers/_Koch证据塌缩-重新定性-2026-08-11.md`：**判据报的「15 条」是它的射程，实际是一对源压着 41 条**；
>   候选第二簇 55 组，但**期刊整卷不是他的作品**、**组根题名≠成员题名**两个坑已写明。
>   Nightingale 那 2 条已修（补 `derived_from`，其中一对是**大书把小册装订在内**，只看元数据判不出来）。
> - `_corpora/wip-kelsen-171/06-holdout污染事故…` / `07-合成门差额…` / `08-known题的阻塞…`


> ## ★★★ 同日更晚的一段（**这一节是最新状态，上面那些在它之前**）
>
> ### 人物
>
> - **#169 Blackstone 记延后**（新类别：**可引面与可证面不重合**）——15 份语料里 8 份长 s 讹字率 0.97–0.99，
>   能取逐字引文的只有 1898 年那个混着编者注的印本，方法类断言只剩一种证据簇。
> - **#170 Oliver Wendell Holmes Jr. 记拒发**（名册 141 → 142）。两轮四个「席×轮」读数**全部为负**：
>   第 1 轮 −0.0269（K −0.0356 / L −0.0181）、第 2 轮 −0.0509（K −0.0556 / L −0.0462），quick 门 +0.07 从未接近。
>   ★ 第 2 轮**唯一改动是产物正文**（照第 1 轮评委原话补），结果**更负 0.0240**——
>   「改产物能推动 delta」这条**方向可以是反的**。
> - **#171 Hans Kelsen 进行中**：预筛 + 可得性探测已完成（**正常开工 quick/standard**），
>   抓源前九条已写死，抓源已下完 12 件、报告在写。
>   ★ 探测最重要的正面结果：**Antiqua 9 部 / 1,256 页，Fraktur 只有 3 件且恰好全是本来就该剔的**
>   ⇒ 能引面与能证明面**重合**，Blackstone 的死法在他身上不会重演。
>   ★ deep 档**现在就判够不着**（13 个 IA 条目、差 17–18 件），不挂等裁定。
>
> ### 判据层（**接手后会直接影响你**）
>
> 1. **`check_holdout_overlap` 换了统计量**：除覆盖率外新增**非样板的连续逐字段**（绝对量，与 holdout 大小无关），
>    **且在 research 阶段是硬拦**。Holmes 就是撞出它的人：覆盖率 1.57% 全绿而整段逐字 136 词。
>    **全库回扫出 7 个已入库人物有整段重复 → 待裁定 ㊲。**
> 2. **`check_contract_drift` 现在一起查「发布清单新不新鲜」**。
>    起因：我改了判据没重算 `checksums.sha256`，**逐件自测全绿、漂移门全绿，而 71 个测试里 4 个红**。
>    ⇒ **改完判据跑一次那道漂移门就够**，不必每次跑两分钟的全量自检。
> 3. **`build_blind_payload` 补了两个开关**：`--key-dir`（把 key 放到评委够不着的目录）与
>    `--balanced-positions`（A/B 强制 8/8，**待裁定 ⑱，别单方面打开**）。
>    ★ 加开关时我把 `args.` 写成 main 里没有的名字，**三道静态检查全绿而工具每次调用必崩**，
>    已补一条**走命令行的冒烟**；全库同形暴露面 **94/100**。
> 4. **`render_claims.py` 从一次性脚本提升为正式工具**（原件工作区路径写死在源码里），
>    并修掉「12 个断言类别只映射了 9 个、另 3 类静默丢弃」。
> 5. `check_namesake_criteria` 三处：读 `attribution`、补 `bare_name_after_year`、新增 `adjudicated`
>    （人工定夺**单独计数，不并进「目标本人」**）。
>
> ### 编号提醒
>
> **㊱ 当天登记当天撤回，作废项不腾出编号**——新条从 **㊲** 起。
> 我在同一份文件里撞过一次号，已更正并记档。
...
- **#168 Hugo Grotius —— 语料已灌入，研究门 17 错，全部不需要裁定**（政治法律师，1583–1645）。
  同名护栏已过（`ready/single/grotius-hugo-1583`），**12 个候选里 3 位只靠一个特征才分得开**。
  **19 份公有领域语料已灌入**（一手 0.7778 / 4 道 / 19 份全 ≤1930，
  `check_corpus_feasibility` ✓ 可行）；★ **台账 19 行里 0 行的 title 是文件名**——
  用的是今天新加的 `ingest.py --title`，全库第一个。
  - 研究门 24 错：15 条是「断言层还没写」（正常），**6 条是 `authorship-unproven`**。
  - ★★ 那 6 条是**拉丁属格**：题名页写 `HVGONIS GROTII DE IVRE BELLI AC PACIS`，**没有「By」**。
    我试了三个自动化做法，**全部被自己的数据否掉**（逐条落在
    `wip-grotius-168/00-归属证据实测.md`）——最要紧的一条：拉丁属格规则接线后
    **压红了两条「版口护栏」**（拉丁书每页版口都印属格作者名，父亲的正文同一个版口），已撤回。
  - ★★★★ **那 6 条已经解决，且不需要裁定**：`quality_check.py:2091` 写着
    「**historical + 已声明 `attribution_basis` 时降为报告，不硬拦**」——
    Grotius 正是 historical，**流水线本来就有这条路，我只是没写 `meta.json` 的 `attribution_basis`**。
    写完（四字段齐，依据取自抓源方逐份回读的题名页）→ `authorship-unproven` **6 → 0**，
    研究门 24 → **17**，**没有放宽任何判据**。
...
- **★ 今天新建的四份实测台账**（都在 `_ledgers/`，**每份都写明了口径与不下结论的地方**）：

  | 文件 | 一句话 |
  |---|---|
  | `_跨语言holdout盲区-2026-08-11.md` | **判据按内容重叠判，跨语言看不见**；全库 **56 份 holdout 里 52 份（93%）的隔离无法用元数据核验**；其中 **87% 有 locator 可修、真正无从下手的 0 份**；`propose_title_from_titlepage` 已在 holdout 上验通（dry-run） |
  | `_门可达性全库实测-2026-08-11.md` | `check_gate_reachability` **第一次跑全库**（2,124 样本）：各席上限均值 0.918，两道 quick 门够得着；★ 各席天花板差很远（`seat-D` 41% 给到 9.0+，`seat-E-strict` 只 1.6%，H/J 两席一次没给过）——这是 ⑮ 缺的那组数 |
  | `_装饰性引用射程-2026-08-11.md` | 29 个工作区全部可量：干净 25 / 有装饰性引用 4（**102 个已交付产物一个都不涉及**）；★ 这个数**四步才稳下来**，中途报出的两类问题全是假象 |
  | `_抓源留痕缺口-2026-08-11.md` | 14 个工作区 / 382 行台账无抓源留痕；★ **丢的是「从哪儿再抓」不是「凭什么是他写的」**——`attribution` 95.5% 都在，`url` 只剩 6.5% |

  ★★ **这四份的共同点：都写了「这个数不能读成什么」。** 用它们时先读那几行。
...
- ★ **这是快照，不是活文档。** 里面提到的文件、字段、命令可能已经变了，
  引用到具体路径时**先去仓里确认它还在**。
- ★★ **2026-08-11 已重导（118 条，索引 118 条链接逐条核过失效 0）。**
  ★ 之后每次收工都该再跑一遍这两条——**教训是每天在攒的，隔一天就旧**：
...
5. ~~教训库同步~~ **已做（2026-08-11）**：`_ledgers/_教训库/` **118 条**，
   索引 118 条链接**失效 0**，README 的条数/体积/日期已随实际更新。
   ★ 它原本不在 GitHub 上，接手方一条都读不到——**现在读得到了**。
