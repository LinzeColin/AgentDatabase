...
- 真实基线运行在第 40 批上传失败，已完成清单保留 39 批，临时载荷清理成功；日常任务保持暂停。错误为 `github_release_command_failed:release_upload`，历史 stderr 已丢失，具体原因仍 UNKNOWN。
- 修复范围：CLI 失败只输出退出码、HTTP 状态和白名单错误分类，最终回执保留同一诊断；原始 stderr、URL、私有路径及任意服务端正文保持隔离。未知错误保留 `unknown`。
- 验证：private/source-runner 138 项通过，含 14 项新增诊断与隐私用例。初次稀疏工作树缺少 ops，补齐后通过。发布检查及实际续传验收另行记录。
- 下一步：发布后使用既有完成清单进行最多一次续传验证；以真实回执决定恢复日常自动任务或停止并报告具体故障。历史失败原因保持 UNKNOWN，当前读取 GitHub 成功仅说明当前读取可用。

## 2026-09-11：Memory Atlas 备份续传修复

- 目标：每日自动保存独立完成；代码测试与手动运行分别记录，最终以原生定时任务真实成功验收。
- 已改：`private_release.py` 按 256 MiB 来源大小分批（单个较大文件独占一批），每批独立加密、回读、恢复、发布后，把完成元数据事务写入受保护的 `completed-archive-batches.sqlite3`。完成批次的临时载荷立即清理；失败保留小型元数据清单，下一次只复用当前内容仍匹配且远端可用的批次。
- 策略：完整快照仍在全部来源就绪后发布；已完成批次本身独立可恢复。全量重打包改为显式操作，批次数量增长保持增量路径。已有归档格式、私有 Release、统一密钥和 R2 零请求保持原样。
- 入口：父进程生成运行身份并传给子进程，超时回执保留该身份和 `capture_timeout_expired`。
- 验证：Python 3.9 的 private/source-runner/live-snapshot/policy 五组测试通过 188 项；中断位置覆盖上传、回读和最终索引，重启后覆盖来源保持、修改、删除以及失效远端引用。真实大批次与下一轮原生自动执行仍待验收。
- 风险：本地完成清单丢失可造成重复工作；尚未完成的一批可能重传。单个大文件仍独占一批。已发布但失去引用的旧归档保持原样，本轮范围包含零项远端删除。
- 下一步：发布检查通过后同步主树，执行一次真实入口；确认批次清单持续增长和完成结果，随后恢复原生每日任务并验收其独立执行。

> ★★★★ **先读仓根的 `START-HERE.md`**（**现测 389 行**——不是一页纸，但按顺序读得完）。
> **给委托人的一句话 prompt 就在它的第 3 行。**
> 本文件**只按需检索，不要通读**（现测 **127KB**）。
>
> ★ 上面两个数是 **2026-08-14 现测**，会漂；要现算就跑
> `wc -c HANDOFF.md; wc -l START-HERE.md`。
> （原来这里写「一页纸／119KB」，实测是 **389 行／127KB** ——
> 手抄的实测值写下那一刻是对的，几次修改之后就不是了。）



> ★★★ **2026-08-13 判分前发现的阻塞项**：第 1 批八人的 `references/sources/` **只有一个 `.gitkeep`**（台账 51–103 行，对照 Koch 是 121/121）。
> 候选方按协议本该拿到它。处置二选一，写在 `_ledgers/_第1批阶段5判分-开箱即跑清单-2026-08-13.md` 第四之三节。
>
> ★ **判据那一半当天已修**（2026-08-13 晚）：`check_quote_speaker` 原来读不到就打 ✓ 并 rc=0——
> 全库实测 **11 个工作区、264 条引文**是这样绿的（Marshall 一个人 44 条）。
> 现已改成：先读 `references/sources/`，空则**回落 `raw/` 并按台账 `split==train` 过滤**（holdout 纪律不放松）；
> 两条路都空则报「**未判**」并 rc=4，不再打 ✓。**剩下要人定的只有「候选方到底给不给来源」这一件。**

> ★★ **2026-08-13 另查出两件**（`_ledgers/_两处开工前必须先弄清的事-2026-08-13.md`）：
> ① **9 个工作区的路径套了两层**（`workspaces/<slug>/<slug>/`）——移交包自验证把它们报成「没有台账」的原因之一；本轮不动，动之前要先改台账 `local_path`。
> ② **Brandeis #172 的同名门有两份互相矛盾的判定**，blocked 那份比 ready 那份**晚 105 秒**。**弄清之前不要跑他的探源。**

## 接手的人先看这里 —— 第 1 批做到哪了（**2026-08-13 现算**）

**阶段 1～4 全部做完，八人合成门 `passed=True` 硬错 0。**
**停点只有一个：阶段 5 判分要 2 席独立子代理。**
...
| #177 Machiavelli | 79 | quick | 10 | 27 | **32 / 16** | ✅ passed |
| #178 Rousseau | 103 | quick | 12 | 34 | **32 / 16** | ✅ passed |
...
**上表除 Comenius 外的 9 行合计 676 份语料、一手 611（611/676 = 0.9038）、
断言 240 条、盲判用例 193 题。** 连记延后的 Comenius 那 83 份一起是 **759**。

> ★ 这一行 2026-08-14 差点变成假话：Comenius 原来那行是 `—`，我把 83 填进去之后，
> 读者把 10 行一加会得 759，而这里写着 676。**数量断言要写清它数的是哪几行** ——
> 见 `文档/踩坑库/read-the-artifact-as-its-actual-reader.md`（那条讲的正是
> 「文档 A 数文档 B 的条目数，B 一改就成假话」）。
...
`_ledgers/_第2批值得开工名单-2026-08-13.md` —— 10 人、每族封顶 2 席、
最少三族拿到 5 席；两条开工前就知道的硬约束写在表里
（**Plato 从不以本人身份说话**、**Aristotle 只存三分之一且是讲义体**）。

**第 2 批已开工，前两人都在阶段 1a 出了结论：**

| # | 人 | 状态 |
|---|---|---|
| 1 | **Luther Burbank #183** | **记延后** —— 一手 66 份（deep 门 30 的 2.2 倍）、占比 0.9565、108 万词，**而六条道只占 2 条**；`check_corpus_ceiling` 三档 rc 全 = 1。★ **声口很好**（1.89/千词，连种苗价目表里都有），**不是「声口不够」那一类** |
| 2 | **Leonardo da Vinci #184** | **已记延后（2026-08-13）**——min_lanes 硬卡点，**三档 rc 全 = 1**。一手 55／占比 0.8871（远超 quick 门 0.40）／570 万词，**而道只有 2 条**（quick 要 3）。★★ 同日先写过「过 quick」是**错的**：第 3 道 `conversations` 的 2 份是《Frammenti **letterari** e filosofici》两个印本，`assign_lanes` 的 `letter` 没词边界、命中了意大利语「文学的」。正则已修＋12 正例/3 反例自测，全库射程只有他一人。详见 `_ledgers/_Leonardo184记延后-*.md` |
| 3 | **Michelangelo #185** | **阶段 2 完成，过 quick 档**（一手 **47**／占比 **0.8393**／**道 4**／370 万词；standard/deep 够不着因为道 4<6）。holdout 已密封 9、train 47，划走后仍过门。**没有纸面道**。★ 本轮修了四处工具缺陷：`judgment` 吃掉《最后的审判》画名、同名护栏漏掉侄孙 il Giovane（1568–1646）、**人的裁定推翻不了自动判定**、**裁定 key 打错会静默忽略**。★ 另改判 6 份「不是他写的」（Lester 1845 的书 ＋ Fisher 的 5 本蚀刻摹本册）。★★ PD 红线两条假出版年：LiberLiber 电子版年 2014→1875、**多伦多大学藏书票入藏年 1970→1911**（新形态）。**阶段 1a–4 已全部走完**：研究稿 6 份、断言 22 条（事实密度 13/10、可复用做法 4/3）、十份产物（装饰性引用 0/22）、盲判用例 **32 题/16 类**（出戏 0、题面自足 0、生成器同步 ✅）。**只剩 `references/sources/` 与判分授权两项** |
| 4 | **Louis Brandeis #172** | **阶段 3 完成，过 quick 档**（一手 **29**／占比 **0.7632**／**道 5**／294 万词）。★★ 差 deep 三项（总数 38<45、道 5<6 缺 timeline、一手 29<30）；补前两项都要放宽检索式，**而那正是同名门失效的那一步**。★ 开工前的同名门阻塞已解（14 候选原只裁定 1 个）。holdout 已密封 **4 条、全部来自 writings**（工具没抽空任何一道，四处缺口照实报）；声口按体裁分层——**≥1.0/千词的 12 份合计 86 万词**，技术短文接近 0。研究门全绿（6 条引文逐字全对、holdout 提及 0）。★★★ 硬纪律：他的书**大量引用他人**，7 条第一人称候选里 **3 条是被引的第三方**；★★ 且 **Bunting 那两卷辩状题名页上根本没有他**（1916 年他已就任大法官、退出此案），我原判一手已改判二手——**只看 IA 的 creator 会判错**。**下一步：阶段 4** |
| 5 | **John Dewey #190** | **阶段 1a–4 全部走完，过 quick**（train 33／一手 **100%**／道 3／断言 14／**32 题 16 类**）。★ **lanes 压线**（现算 3 = 门 3）：撑起第三条道 `conversations` 的**只有一份**《Letters from China and Japan》(1920)，扉页「BY JOHN DEWEY **AND ALICE CHIPMAN DEWEY**」、前言明写信是他**和妻子**写的，**正文无逐封署名**（`J. D.`／`A. C. D.` 各 0 处）⇒ **切不出他自己那份**。按已有裁定（㊴ 维持 `min_lanes` 定义、形态② 只报不判）**登记不挡**；★★ 但**若将来把「共同署名无分工」改成硬判，第一个掉下去的就是他** |
| 6 | **Winston Churchill #191** | **产物齐全但不许判分** —— `timeline` 的 2 份是同一部书的两个印本，`min_lanes` 实质 2 < 门 3。**已进延后名单**，见上「Churchill #191」一节 |
| — | Plato #186／Aristotle #187／Ford #188／Hopkins #189／Pinchot #192／Howard #193 | **均已记延后**（现算：都在延后名单里，各有理由与解锁条件） |
| — | Sloan／Bernays | **已在延后名单**，无工作区（抓源前出局） |
| — | ~~Michelangelo／Brandeis 未开工~~ | ★ 这一行 2026-08-13 已作废：两人都做完了阶段 4（各 32 题），见上表第 3、4 行 |
...
他有十份产物、13 条断言、32 题用例。**而语料这一关没过**：

**`timeline` 那条道的 2 份是同一部书的两个印本**
（`My early life a Roving Commission` / `My early life; a roving commission`）——
**流水线自己的去重实测**（`raw/_dedup.json`，min-hash Jaccard 0.55）把它们归成了
他语料里**唯一的一个重复簇**。⇒ 独立作品数 1，`min_lanes` 实质是 **2**，quick 的门是 3。

**这个假过门是当天自己造出来的**：为给自传归道，`assign_lanes.py` 的 timeline 规则补了
`my early life` / `a roving commission`，他的《My Early Life》从 `writings` 移到 `timeline`，
道数 2 → 3 **正好等于门**。**材料一份没多，只是换了个格子。**
规则本身**不撤**（在 Ford #188 的《My Life and Work》上同样打响，有反例守着）——
错的是拿规则的产出当「门过了」，而**压线那一道从没打开读过**。

⇒ 新判据 `_ledgers/_pipeline/check_lane_distinct_works.py`（6 条自测，含两条反例）。
它同时抓到 **Marshall #173** 的 `conversations` 4 行是 1914 年同一部书的四次扫描
⇒ 他**除盲判装置不成立外，多一条独立且更靠前的理由：语料这关就没过**（deep 门 6，去掉只剩 5）。

★★ **覆盖面：18 个工作区有实测去重（真判过），20 个连嫌疑都筛不出来**
（`title` 栏 92.5% 是文件名，且本机既无正文也无真题名 ⇒ **未检查，不是通过**；
其中 12 人已经判过分）。详见 `_ledgers/_空心道-20个工作区筛不了-2026-08-13.md`。
...
**开箱即跑清单在 `_ledgers/_第1批阶段5判分-开箱即跑清单-2026-08-13.md`**：
装置、每人风险、命令、以及**判分之前就定死的判读规则**都写好了。

★★ 里面有一条**判分之前算出来、并当场处置掉的**硬事实：
四个 quick 档的人原本各只有 16–17 题，门 +0.03 只有 **1.83–1.89 个 SE**
——**低于 2 SE，「过门」与「等于零」分不开**。
⇒ **已把题数补到 32/33**，门抬到 **2.59–2.67 SE**；deep 四人是 6.03 SE。**八人现在全部判得出。**
**没有动任何判据**：`min_suite_cases` 是下限、门槛 +0.03 一个字没改，加的是**测量量**。
★ 但按 sd 区间上端（2.2×）取，quick 四人只剩 1.18 SE——
落在 +0.03～+0.04 时**必须把这句一起写**，判完不许翻案。
...
但**仓里还有 4313 份存量语料正文**，是本会话之前提交的。**删它们是不可逆动作，留给你定。**

★ **口径必须一起给**（同一个问题我今天给出过 2061／4289／520 三个错数）：
`git -c core.quotepath=false ls-files` 里**路径含 `/raw/` 且以 `.txt` 结尾、排除 `_ids*`／`_blocked*`** 的行数。
它跨**两种目录布局**：`raw/文件.txt` **520 份** + `raw/src-XXXX/文件.txt` **3793 份**
——只按前一种数会得到 520，**漏掉 88%**。

前十名：livermore-100 **532**、virchow-109 **454**、barton-117 **427**、koch-107 **241**、
nightingale-112 **226**、osler-110 **208**、godin **193**、blackwell-118 **184**、
adams-131 **141**、fleming-111 **138**（共 46 个目录）。

★★ **第 1 批 10 人贡献 0 份**——新规则（仓里只放指针）在这一批严格执行了。
...
> ## ★★★★★ 还有**一条警告比那条错更吓人**，而它是同一回事
>
> 同一次运行还会印出：
>
>     research.lane_quotes：**52 条逐字引文回原文对不上**
>       ——**引文对不上就是引文对不上**，逐条读过再决定是改引文还是记盲区
>
> 这句话读起来像「**引文是编造的**」。**它不是。** 2026-08-12 收尾时做过对照：
>
> | | `raw/` 文件数 | rc | `lane_quotes` 警告 |
> |---|---:|---:|---|
> | **有语料的工作树** | 21 | **0** | **没有**（errors 也是 0） |
> | **移交树**（语料被剔） | **0** | 1 | **52 条对不上** |
>
> ⇒ **对不上的原因是原文不在这棵树上，判据没东西可比。**
>   与 `research.ledger-file-missing` 是同一类：**「未核验」而不是「核验失败」**。
>
> ★ **不要据此去改引文。** 要真核引文，先按下面「怎么取回语料」那一节把
>   `raw/` 取回来（每份都有 `locator` + `checksum`），**取回之后这条警告会自己消失**。
...
> | 档 | 条数 | 占有值的 | 怎么取 |
> |---|---:|---:|---|
> | ① **串里含 http** | **1966** | **72.5%** | **可直接点开** |
> | ② 无 http 但有 `archive.org` / `item` / IA 式 id | **472** | 17.4% | 按 id 去 IA 取 |
> | ③ **两者都无，只有书誌描述** | **275** | **10.1%** | **只能按题名检索——取不回来的风险全在这一档** |
>
> ⇒ **①+② = 2438/2713 = 89.9% 有机器可循的取回路径**；
>   加上 locator 为空的 72 条，**真正难取的是 275 + 72 = 347 条（占 2785 的 12.5%）**。
>
> > ### ★★★ 上面这组数**当天改过一次**：我第一版扫的集合比实况小
> >
> > 第一版写「台账行 **2065**、locator 96.5%、难取 343 条（16.6%）」，
> > 用的是 `rglob("evidence/source-ledger.jsonl")`——**要求父目录必须叫 `evidence`**。
> > 而全库 52 份台账里有 **6 份不在 `evidence/` 下**，整批被漏掉：
> >
> >     wip-livermore-100/source-ledger.jsonl                 （**536 行，最大的一份**）
> >     wip-galen-101/source-ledger.jsonl
> >     wip-harvey-103/source-ledger.jsonl
> >     wip-vesalius-102/source-ledger.jsonl
> >     wip-blackstone-169/.../_corpus/source-ledger.jsonl
> >     wip-holmes-170/.../_corpus/source-ledger.jsonl
> >
> > **漏了 720 行 = 全量的 25.9%。** 改成 `rglob("source-ledger.jsonl")` 后重算才是上表。
> > ★ 这正是教训库里那条 `a-gates-scan-set-is-smaller-than-reality`
> >   （判据跑了、也没指错文件，**而它扫的集合比实况小**）——**同一天又犯一次**。
> > ⇒ **写任何全库统计之前，先把「我扫到了几份文件」打出来，和实际份数对一遍。**
>
> ★ 第 ③ 档长什么样（随机抽的，**不是挑出来的**）：
>
...
>
> ### ★★★★ 我今天加的那道判据，**会漏掉标准目录布局下的全部基线**
>
> `check_baseline_in_persona` 判据本身没问题（它吃一个路径）。
> **问题在谁替它挑路径**——`quality_check` 与我做全库扫时都只看**文件名**：
>
>     'baseline' in p.name.lower() or 'bare' in p.name.lower()
>
> 而 `_每次开工必读.md:204-205` 的**标准布局**把标识放在**目录名**上：
>
>     <人物>/answers-round<N>/baseline/answers.json   ← 文件名就叫 answers.json
>
> ⇒ **文件名筛法一份都匹配不到，而它报的是「无候选」＝ 绿。**
>   现在只漏 1 份（Holmes round2），**但这个布局是标准**——
>   **#173–#182 以及此后所有新人物都会用它，届时这道判据一份都看不见。**
>
> ★ **本轮不改**：用户当天说了「不动任何判据」，且第 1 批全停在抓源，
>   解封前不会被新人物触发。**改法（一行）、改哪两处、怎么验收**，
>   全部写在 `_ledgers/_基线入戏门会漏掉标准布局-2026-08-12.md`。
>   ⇒ **「不动判据」一解封，第一件就改它。**
>
> ### ★★ 还有一个人的语料**根本不在仓里**：Livermore #100（**已发布产物**）
>
> | 工作区 | git 里的语料 | 台账 |
> |---|---:|---|
> | `wip-galen-101` | 167 份 | ✓ |
> | `wip-vesalius-102` | 46 份 | ✓ |
> | `wip-harvey-103` | 122 份 | ✓ |
> | **`wip-livermore-100`** | **0 份** | 536 行 |
>
> 这四个当年同病（语料只在会话 scratchpad），前三个已经进仓，**只剩他**。
> 他的 542 份语料只在本机 `~/Downloads/蒸馏/_corpora/livermore-100/`——**那条路径不跟着移交走**。
>
> ⇒ **不是丢了，是要重抓**：台账 **536 行，locator 100%、checksum 100%**，
>   其中 **533 条是 LoC 报纸直链**（`tile.loc.gov`）、3 条 archive.org。
>   取回后逐份验 sha256，与当年「按校验和找回」的做法一样。
> ★ **我没有把那 24 MB 直接塞进 git**：往历史里加 24 MB 实际不可逆，
>   而移交分支本来就剔语料、塞进去对接手方没帮助。
>   **要不要进仓是一次「改变仓库体积基线」的决定，留给人。**
>   逐条现算与配方见 `_corpora/wip-livermore-100/CORPUS_STATE.md` 末尾。
...
> ## ★★★★★ 2026-08-12 最末：**计划换了，而且尺子的另一半刚发现是坏的**
>
> ### ① 待裁定：**已全部由我裁定，待裁数现在是 0**（2026-08-14 又裁了 1 条：㊸）
>
> 用户原话：「40 条待裁定你自己定，**只有推送、下载授权、删受保护资产留给我**。」
> 全文与逐条理由见 `references/ledgers/_待用户裁定.md` 开头的「已全部由我裁定」一节。
> 现算：41 个圈号 − 2 个作废（⑯ 我误报、㊱ 当天撤回）＝ **实际裁定 39 条**，正文 0 漏。
>
> 四条统一原则：**门与判据一律不动**／**存量产物冻结只记档**／
> **新人物的流程该改就改**（冻结是按人物冻的）／**记录卫生当场做掉**。
>
> ★ **2026-08-14 新增 ㊸㊹㊺ 三条**（同样按这四条办，**都没推回给你**）。
> 我这一轮把三件本来写成「只能你定」的东西改正了 —— 你 8-12 说过
> 「40 条待裁定你自己定，**只有推送、下载授权、删受保护资产留给我**」，
> 而我此前三次把已经授权给我的事又推了回去。三条的裁定与代价都写在
> `references/ledgers/_待用户裁定.md`：
>
> - **㊸**：
> 「两处独立证据」那两道门比的是 `source_ids`／`evidence_clusters` **去重后的个数**，
> 于是**同一本书的两次扫描算两处**。全库实测 261 条里 **40 条**的支撑只有一部作品。
> 裁定：门不动、存量不动、**新人物在阶段 3 收尾必须跑
> `_ledgers/_pipeline/check_claim_evidence_independence.py`**。
> - **㊹**：判分前五人（Bismarck／Jefferson／Lincoln／Marshall／Pestalozzi）
>   `meta` 写 `quick` 而材料现算 `deep`（`min_delta` 0.03 vs 0.07）。
>   裁定 **维持 `quick`**（`profile` 就是尺子，判分前不许改），
>   按㉞把「退档理由缺失」补记，**那句要写进判决书正文**。
> - **㊺**：Marshall #173 密封面不成立（唯一低重合的那份是**同名者的**）⇒
>   照㊳ 先例**记「装置不成立」结案、零分数产生**。
>
> ### ★★ 2026-08-14 新量出来的一件事：**归属证据的真缺口是 96 份 P1 源 / 5 个工作区**
>
> `check_authorship.py` 全库跑一遍（**188 秒**，每人约 1 秒、零副作用）：
> 声称 P1 **1023** 份，A-* 有据 **541（52.9%）**、无据 **482（47.1%）**。
> **47.1% 不等于 47.1% 有问题** —— 按 `historical` 豁免拆开才算数：
>
> | | 工作区 | 声称 P1 | 有据 | 无据 |
> |---|---:|---:|---:|---:|
> | A. historical ＋ 有 `attribution_basis.citation`（**设计内**） | 14 | 741 | 355 | 386 |
> | B. 没有豁免覆盖（**真缺口**） | 7 | 282 | 186 | **96** |
>
> B 组里 Comenius **48** ＋ Leonardo **34** ＝ 82 份（85%），
> 另有 Burbank 8／Ford 4／Churchill 2；Brandeis 与 Dewey 虽无 citation 但**全部有据**。
> ★ Leonardo #184 与 Burbank #183 **已记延后**，改他们的 meta 不解锁任何东西。
> 详见 `_ledgers/_署名证据的两个层次-2026-08-14.md`（含我先量错一次的经过）。
>
> ★★ **现在真正只剩你做的**：**① 推送交付包**；② 删受保护资产（本轮无此需求）。
> **③ 阶段 5 判分不是「等你授权」，是我在这个运行环境里做不了** ——
> 它要恒 2 席、两席必须在**互不知情的空白会话**里工作，而本会话不许派子代理。
> 八人的题、rubric、协议记录都已就绪（193 题），换一个能开两个独立空白会话的环境即可跑。
>
> ★ 本轮新落的四件工具（都带自测，都已接进交付构建）：
...
>
> #106–112（Pasteur/Koch/Lister/Virchow/Osler/Fleming/Nightingale）的基线是
> **第三人称百科口吻**——「……是**南丁格尔**最广为流传的著作」
> 「**科赫**……**他**把照相作为记录细菌形态的手段」；
> 而 #101–104 的基线是**入戏第一人称**——「**我**确曾把昆虫的生成写成一批稿子」。
>
> 同族、同套题、连号，**delta 区间完全不重叠**，而**候选臂只差 +0.0320、对照臂差 −0.1183**。
> **拿「扮演 X」比「介绍 X」，赢的是任务差异。** 剔掉那 7 个：
>
> | | 之前 | **之后** |
> |---|---:|---:|
...
> ★ 七人**一个都没入库**（`医疗护理师/` 下只有 `_category.json`）——
>   要重估的是台账结论，不是已交付产物。**这 7 个数不许再引用。**
> 全文：`references/ledgers/_门与分辨力-全库实测-2026-08-12.md` **第六节**
> （第一至五节的数字**已被推翻**，留着做对照，引用一律引第六节）。
>
> ### ③ 新工作方式：**10 人一批、按阶段跑**
>
> 全文 `skill_log_evals/persona-distiller/_ledgers/_新计划-10人一批-2026-08-12.md`。要点：
>
> - **抓源并发 4，遇 429/403 整批退回 1**（4 是 RUNBOOK 记的 Wayback 红线，**上限不是起点**）
> - **阶段 2（抓源后、合成前）量四项**：声口／一手占比／min_lanes／可用条数，
>   **任一不达门当场结案**。Sellers #154 是开工前量的省下整轮，Coffin #130 走到判分才发现。
> - **判分第一轮 delta < −0.011（= 2 SE）直接结案**，不跑二三轮。
>   历史上跑满三轮的负 delta 人物（Barton／Thomson／Sorby／Holmes）**无一翻正**。
> - **基线生成已逐字冻结**：`_ledgers/_pipeline/BASELINE-PROMPT-FROZEN-v1.md`。
>   生成后跑 `scripts/check_baseline_in_persona.py`，第一人称覆盖率 < 0.40 就重做。
> - **暂停**：#15 技术债、#16 补队列、#29 重蒸。
>
> ### ④ 下一批 10 人（`_ledgers/_pipeline/worth_starting.py --top 10`）
>
> Marshall／Lincoln／Bismarck／Machiavelli／Jefferson（政治法律师）
> ＋ Kant／Rousseau／Pestalozzi／Frobel／Comenius（思想教育师）——**全绿灯**。
> 全 50 人：绿 18／黄 31／红 1，其中 **5 人带「归属必查」**。
>
> ★ 该脚本首跑抓到我自己两个缺陷（都已修）：
> **㊀** Solon／Confucius／Plato 被判成「卒年缺；在世则无到期日」——他们卒于公元前。
> 根因是 `_卒年.json` 对精度低于年的日期留空，而我把 `null` 读成了断言；
> `source` 串里白纸黑字有 ` BC`。
> **㊁ Socrates 一度排进本批第 6 位**——他一个字都没留下。
> **PD 那一轴对「这些字是不是他写的」完全无感**，已加「归属必查」列。
>
> ### ⑤ ★ 现在卡在哪：**第 1 批停在下载授权**
>
> `init_target.py` 必须先有**同名门**产物，而同名门要的是逐名候选卡
> （职业、背景、能力）——那是研究内容，**凭记忆写就是编造**。
> ⇒ 抓源要下载文件，**而下载授权是用户保留的三件之一**。
> **不是我没做，是这一步按规矩必须他点头。**
...
> 一个**以 `origin/main` 为父、树＝当前 HEAD 减去语料**的提交。
> 2026-08-12 当天最后一次实跑（**今天的提交全部在内**）：
>
>     剔除语料文件 7388 个｜index 条目 20057 → 12669
>     基线 origin/main = c62de34f3440
>     ★ 新 blob **34 MB**（闸 200 MB）｜✓ **真跑 pre-push 钩子：放行**
>     记录类文件数与 HEAD **完全相等**：_corpora 内 3565/3565、语料目录外 8988/8988
>     分支 handoff/2026-08-13 已建好，**脚本不推送**
>
> ★ **这几个数每多一次提交就变一次**（当天走过 31→33→34 MB、8965→8980→8988）。
>   **不要信这里的数，信你自己那次重跑打印的数**——下面「移交怎么做」那节讲了为什么必须重跑。

> ### ★★★ 2026-08-12 末：**整链在干净目录里复现过一遍**（不是逐条命令各自绿）
>
> 把移交提交 `git archive` 到一个空目录（**无语料**），**在收件人的布局里**跑：
>
> | 步骤 | 结果（**2026-08-12 第二次末复验**，含当日 KNOWN 清零那批） | 判读规则 |
> |---|---|---|
> | 导出的文件数 | **12752**（无语料） | `git ls-tree -r` 计数；`find -type f` 会少 20 个，**那 20 个是符号链接**，不是丢了 |
...
>   [[stale-artifacts-from-my-machine-leak-into-the-build]]（交付包的数全是我手跑的，工具不在流水线里）。
>
> ★★ **唯一预期的红**：`quality_check --phase research` 会报语料不在本树——
>   **那是对的**，语料按裁定另存，仓里只放指针（`_ledgers/_语料指针清单.json`）。
>
...
> - **#171 Hans Kelsen ★★ 已于 2026-08-12 结案：装置不成立、零分数产生**（㊳ 已裁定，
>   同 Paton #162）。理由：一句调试 `print` 把 `load_corpus` 的返回值整个打了出来，
>   **holdout 有 5%（含完整目录）被我看见**，而**判据体系里没有一道门看得见这件事**
>   （两道 holdout 门都在查文件，这次一个文件都没动）。
>   ⇒ **拿一个已知不干净的装置出分数是编造**，所以不产生分数。
>   **接手方不要去续做他。** 下面是结案前的状态，留作记录：
>   预筛 + 可得性探测已完成（**当时判正常开工 quick/standard**），
...
>    `--balanced-positions`（A/B 强制 8/8）。★★ **2026-08-12 ⑱ 已裁定：从下一个人物起改默认打开**
   （取模是公平的，但每 13 人就有 1 人落到 ≥12/16；改默认不影响已派发轮次）。
...
## 2026-08-12：**「判据自己没被查」这一族，一天挖到底**

★ 这一节是给你（接手方）省时间的：**下面四层是同一个病的四种形态**，
  已全部落成常设判据 + 变异验证。你不必重走一遍，但**新写判据时会撞到第 4 层**。

| 层 | 问题 | 落地 |
|---|---|---|
| 1 | 判据**没有调用方** | `check_verification_counts` 两条例行路径都不跑它 → 接进漂移门 |
| 2 | 判据的**自测碰不到它** | 漂移门新加的两条函数只走「路径不存在→返回 []」→ 补 7 条正负对照 |
| 3 | 自测**验了配料、没验判决** | `check_holdout_overlap.check()` 从没被进入 → 补 9 条 tempdir 对照 |
| 4 | **射程审计自己有盲区** | `selftest_touches_disk` 看不见没有 `main()` 的判据 → 修 + 名单 38→45 |

**新增判据 `check_selftest_reach.py`**（已接进漂移门）：
用 `sys.settrace` 量每件判据的自测走没走到自己的判定函数。
实测 **36 件有函数没被进入、其中判定类 18 件**——`KNOWN` 冻结这 18 件，
**只挡新增**：你新写的判据若自测没跑过 `check()`，漂移门会当场红。

★★ **`check_holdout_overlap` 是这一族里最贵的一件**：待裁定 ㊲ 的全部依据就是它的输出，
  而它的 `check()`（分 train/holdout、套阈值、出判决）此前一次也没被自测跑过。
  补完当场做的变异里，`df_max = 0` 会让样板过滤把真转载一起吃掉、
  **判据给逐字转载发绿灯**——㊲ 若少报，就是从那条路少的。

  ★ **㊲ 已复算完（2026-08-12），判据行为未变，有两条独立证据**：
    ① 仓里 14 份被跟踪的污染段报告回扫后**逐字节不变**；
    ② 本树扫得到的 6 个工作区，**段数与 08-11 记录逐个精确相同**
       （semmelweis 1646 / koch 1483 / lister 247 / blackwell 40 / carver 3 / whitworth 1）。

    ★★ **同时更正了 ㊲ 的表述**：原写「七个**已入库**人物」——「七个」对（数的是硬失败），
      **「已入库」错**。逐个精确核过 102 个产物与另三个 registry 索引：
      **七个硬失败在 registry 里的是 0 个**；已发布产物里受影响的只有
      **Carver #127、3 段**（可逐段避开）。
      ⇒ ㊲ 不是「七个已发布产物的分数要作废」，而是
        「**七个未发布人物的 holdout 装置不成立**」＋「一个已发布产物 3 段可避开的重合」。
      全表见 `_ledgers/_㊲复算-判据行为未变而口径对不上-2026-08-12.md`。

★★★ **漂移门现在 15.2 秒，不是 5.3 秒**（多的 10 秒是全扫 90 个自测）。
  「改完判据跑那道漂移门就够」这句仍然成立，而且现在管**五**件事：
  发布清单新鲜度、一区两账本、VERIFICATION 自报数字、自测射程、**纸面道**。

## 2026-08-12 下半天：**又两件判据 + 四处「绿章盖在不知道上」**

### 新判据 ②：`check_paper_lanes.py`（已接进漂移门）

排 #172 Brandeis 时问「`min_lanes 3` 过不过」，读判据才发现：
**lane 数的是每条源的 `dimensions` 字段，一条源挂多个 lane 就各加一次。**
⇒ 把一卷 U.S. Reports 标成 `["decisions","expression"]` 就补两道，**纸面上就过了**。
这是 Vavilov #126 的**隐藏版**：他明着凑不满被拦，这条路靠字段凑满混过去。

定义「纸面道」＝覆盖该道的源里**没有任何一条是只挂它的**。全库 39 个工作区、**9 个有纸面道**，
其中 **2 个是已发布产物**（Livermore #100 / Steinhardt #98）⇒ **待裁定 ㊴**。

★ 这件判据**第一版射程漏扫四分之一**（只 glob 一层 `evidence/`），报「4 个」而真值 9 个。
  **抓到它的是变异测试，不是读代码。**

### 四处「绿章盖在不知道上」（都按各文件**已有的正确写法**改）

| 位置 | 原来 | 现在 |
|---|---|---|
| `quality_check` 花体乱码 | 解析失败 → 印 **`✓ 没有花体乱码`** | 「**未核（不是通过）**」 |
| `quality_check` 引文层 | 失败时 `scanned` 不增、`pass` → 「已扫 0／问题 0」 | 读不了的文件**单列** |
| `quality_check` 引文层 | 没东西可扫时也是「已扫 0／问题 0」 | 明说「**一个文件都没扫到，本项没有起作用**」 |
| `check_corpus_feasibility` | `meta.json` 读不出 → **静默退到最松的 quick** | 仍按 quick 算，**但必须报出来** |

★★ 第四条最实：实测同一批 12 份材料，**meta 写 `deep` 报「走不完全程」、meta 坏掉报「可行」**
  ——一个损坏的文件把判决从失败翻成通过。全库 36/36 个工作区都有 meta.json，**11 个是 deep**。

### 找它们的办法（比逐件补自测快）

从一个已确认的缺陷形状去 grep 同族：全库 **57 处**「只 `pass`/`continue`/赋常量」的 `except`，
逐处读完 → **真正要修的 4 处**，其余是解析坏行时跳过（有意）。
⇒ **别按数量恐慌**：57 里 53 处是对的。

## 2026-08-12 末：**「已量清未修」六条全部复核完 —— 四条卡的是同一件事**

任务清单里长期挂着六条「已量清未修」。当天逐条**真跑判据**核过，结论：

| 条目 | 原记 | 复核结果 |
|---|---|---|
| Koch 压在一对源上 | 41 条（不是 15） | **准**（41 与 15 都对，差在判据只查 6 类；`fact` 允许单源不算违规） |
| 未声明重复源 | 625 | **准**（实测 **624**） |
| `title` 就是文件名 | 91.9%｜近期**六**人已 100% | 比例**准**（90.6%）；**「六人」错——连续干净的起点是 #168，共四人**，#166 Cicero 是 19/19 全中 |
| 研究道分道与台账不一致 | 73 处，已量清未修 | **性质记错**：不是机械漂移，Scope 节里有大量手写判断，重出会覆盖掉；14 个工作区里 **9 个已判过分动不得** |
| 三人研究道 holdout 点名 | 三人都点名 | **只有 Pasteur 是真点名**；Koch 1/7、Lister 1/4。且**此前根本不在待裁定台账里** |
| ㊴ 纸面道 | —— | 登记时**只写进任务清单，仓里台账一个字都没有** |

### ★★★ 最有价值的一条：**四条卡的是同一件事**

`分道不一致（9 个）`＋`title 存量`＋`未声明重复源（601/624 集中在四人）`＋`holdout 点名（3 人）`
—— **当事人全部已判过分**，改它们等于改被评分过的输入。
且**除 Carver #127、Steinhardt #98 外全部不在 registry**。

⇒ 已合并为 **㊵「已判分、未发布的产物能不能改？」**，选项写成 A/B/C 三条。
  **你只需裁一次，而不是四次。**

### ㊶ 拒答溢出门要重做还是撤掉（2026-08-12 新增）

这道门此前**从没扫到过 21/31 份判分载荷里的任何一条答案**——载荷有两种形状
（行式 `[{case_id, candidate}]` 与扁平 `{case_id: "答案正文"}`），
旧代码对扁平那种拿到的是字符串，被 `isinstance(r, dict)` 一条不剩地跳过，
而屏幕印的是「✓ 无拒答溢出：**每一处**拒答都留下了可执行的东西」。解析已修好。

**但修好之后它的数仍然不能当缺陷数用**：全库首扫 62 条，
逐条读原文——**读了 11 条、9 条是误杀**（判据认不出圈号编号、「你该去问他」、
「查第 8 版」这类给法）。八类已补进 `ACTIONABLE`（62 → 29/588 = 4.9%），
**而抽读剩余仍见误杀**。继续调正则是打地鼠。

★ 它是 metrics-only、不拦发布、不参与任何 delta ⇒ **以上改动不触动已判分数**。
★ 要盯的那件事是真的：三次独立盲判都指向它，实测代价 **−0.1044**，
  本项目已测得的最大单项负收益。**尺子量不准它，而事情是真的。**

选项：**A 重做**（我的倾向：不再写句式正则，改在盲判 rubric 里加一问
「这一答有没有留下任何可执行的东西」，让已经在读全文的评委直接标）／
**B 撤掉**／**C 保留现状**（当「值得读一眼」的候选名单，永远不引用它的数）。
全文见 `_ledgers/_待用户裁定.md` ㊶ 与 `_ledgers/_拒答溢出门-整代人物空过-2026-08-12.md`。

### ★★ 顺带：我自己两次量错，判据一次都没抓到

· 把 `title` 的**问题率**当合格率报 → 看上去像「从 91.9% 崩到 9.4%」
· 「扫所有含『未声明』或『重复』的键取 `max`」 → 1160（把「已声明的 789」算进去了）

**两次都差点报成「既有记录严重失实」，而记录一直是对的。**
规矩已写进 `_每次开工必读.md`：报「率」先写清分子与方向；**不许用 `max`/`any` 挑字段**；
**复核既有记录时先假定记录是对的**。
...
- **待用户裁定的条目**：**待裁数已是 0**（2026-08-12 用户授权我全部自裁）。
  仍读 `registry/codex/persona-distiller/references/ledgers/_待用户裁定.md`——
  开头「已全部由我裁定」一节写着**每条的裁定与理由**，正文各章保留原始事实。
  **只剩三件给人：推送／下载授权／删受保护资产。**
  ...
  ★★ **2026-08-12 改**：这一句原文是「**这些不是你能自己决定的**，遇到就停下来问」——
  **现在不成立了**。用户当天把 40 条全部授权自裁，`_受阻待裁.json` 里的条目
  **按 `_待用户裁定.md` 开头那四条原则自己定**（门与判据不动／存量冻结只记档／
  新人物流程该改就改／记录卫生当场做掉），**只有推送、下载授权、删受保护资产才停**。
...
> ## ★★★ 2026-08-12 改过一次，**旧版会让接手方停错地方**
>
> 旧版结尾是「**遇到标「待裁定」的条目停下来问我**」。当天用户把
> **40 条待裁定全部授权我自裁**，**待裁数现在是 0**——
> 照旧版说，接手方会**永远等不到该停的时刻**，
> 而**真正该停的三件（推送／下载授权／删受保护资产）它一个都不知道**。
>
> ⇒ **一句话 prompt 是会过期的东西**：它把一整套规矩压成一句，
>   规矩一改，这句话就要跟着改。**改规矩的那一次就要顺手改它。**
>   ★ 这与 [[gates-cover-json-not-the-prose-users-read]] 同型：
>   判据管着 JSON，而**用户读的是这一句散文**。
...
| 自验 | 判读规则（**不写死数值**——每提交一次就变） |
|---|---|
| 新 blob 量 | 必须 **< 200 MB**（当天实跑 31→33→**34 MB**，一路在闸内） |
| **真把 pre-push 钩子跑一遍** | 必须打印「放行」 |
| `_corpora` 内记录类 / 语料目录外 文件数与 HEAD | **两边必须相等**（当天末次 3565/3565、8988/8988） |

★ 给的是**判读规则**不是期望值：写死数值的后果当天就发生过三次——
  31 MB 写进文档，两轮提交后实况已是 34 MB，读的人会以为哪里出了问题。
