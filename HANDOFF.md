...
★ 不需要语料的部分（台账、断言、产物、用例、判据自测、漂移门、pytest 全套）
  在移交树里**全部能跑**。

  ```bash
  python3 -m pytest tests/ -q          # 全量自检（约 3 分钟）
  python3 scripts/check_contract_drift.py   # 快检：改完判据跑这个就够
  ```

  ★ 这里**有意不写测试件数**：写死的数每加一个测试就陈旧一次（本行原写「71 个」，
    2026-08-12 加了「判据真跑得起来」两条就变成 73）。**数由命令现算，不由文档记账。**
    ⇒ 另有**两处**写着「71 个测试里 4 个红」——本文 §2 讲 `check_contract_drift`
      新增「发布清单新不新鲜」的那条、`_每次开工必读.md` 里「**代价**：本日我改了…」那段。
      那两处是**事故当时的实录**（当时确实是 71），**不要跟着改**——改了就是把记录改成假的。
...
>
> ### ★ 关于推送体量：**脚本早就处理好了，我绕了一大圈**
>
> 我今天量出「推这条分支要 2167 MB，闸 200 MB，会被拒绝」，并据此写了一长段
> 「甲还能不能做」的分析。**那一段已删，因为前提就错了**：
>
> **移交从来不推这条分支。** `machine/handoff/build_handoff_commit.sh` 造的是
> 一个**以 `origin/main` 为父、树＝当前 HEAD 减去语料**的提交，2026-08-12 实跑：
>
>     剔除语料文件 7388 个｜index 条目 20049 → 12661
>     基线 origin/main = c62de34f3440｜移交提交 = 30de31bb50b0
>     ★ 新 blob **33 MB**（闸 200 MB）｜✓ **真跑 pre-push 钩子：放行**
>     记录类文件数与 HEAD **完全相等**：_corpora 内 3565/3565、语料目录外 8980/8980
>     分支 handoff/2026-08-13 已建好，**脚本不推送**
>
> ⇒ 2167 MB 只适用于推**这条工作分支**，而移交不走那条路。
>
> ★★ 这是 [[tool-existed-and-i-did-it-by-hand]]：**工具在仓里、用法就写在本文件下面
>   「移交怎么做」那一节，而我花了好几轮重新推导它已经做过并且已经自验过的事。**
>   连带产生四次量错（用错基准算出 0.0 MB、误报闸有 bug、`git rev-parse` 对不存在的
>   ref 得到假的「0 MB 放行」、查 `url` 而指针在 `locator`）——
>   **那四次全是在解一个不存在的问题。**
>
> ★★★ 留下的教训对接手的人有用：**开工前先搜有没有现成脚本**。
>   本仓 `machine/handoff/` 下就有，且它自验的三件事比我手算的可靠。
>
