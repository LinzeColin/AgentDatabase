...
## Codex Non-Blocking Defaults

This installed skill is configured for Codex execution. Do not block on style, density, purpose, length, or template preference questions unless the user explicitly asks to choose first or the missing information makes generation impossible. Infer sensible defaults from the prompt and produce a runnable first draft.

Use `references/style-selection-matrix.md` before asking style questions. For template inspiration, inspect `references/beautiful-html-templates/index.json` and only open the chosen template files. Default choices: internal/business/technical/reporting decks use high-readability light professional or Swiss-style systems; launches/brand/marketing use stronger visual systems; data dashboards use dense structured layouts; investor materials use restrained high-formality layouts. State assumptions in the final response instead of pausing mid-run.
