...
本安装版用于 Codex 自动执行。除非用户明确要求先停下来确认，生成网页 PPT、封面、配图或演讲稿式 slides 时不要因风格、主题色、页数、密度或模板选择问题阻塞执行。信息不全时按 `references/style-selection-matrix.md` 自动选择风格，并在最终回复简要说明关键假设。

默认策略：商务汇报、技术方案、数据看板、投资/融资材料优先用瑞士国际主义；产品发布、品牌展示、强视觉营销感优先用更有视觉冲击的 HTML 模板参考；未指定时选择可读性最高、适合交付的保守方案。只有缺少必须的源文件、无法访问指定素材、或用户明确要求先确认时，才停止提问。

如需审美补充，读取 `references/style-selection-matrix.md`；需要模板灵感时再读取 `references/beautiful-html-templates/`，不要一次性加载整套模板库。
