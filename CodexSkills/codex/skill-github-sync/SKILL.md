...
description: 把本机全部 Skill（Codex 用户、Codex 系统/OpenAI 官方、Claude/Anthropic、Agents 通用目录）全量镜像备份到 GitHub 公开仓 LinzeColin/AgentDatabase 的 CodexSkills/，重建人读与机器读索引，提交并推送。当用户要求备份 skill、同步 skill 到 GitHub、更新 skill 索引，或由每周定时自动化触发时使用。推送前有凭据硬门，扫到密钥即中止。
---

# Skill 同步到 GitHub

把本机所有 skill 全量镜像到 `LinzeColin/AgentDatabase` 的 `CodexSkills/`，让任何 LLM 都能通过公网按需检索和加载。
...
| 目录 | 本机路径 | 内容 |
|---|---|---|
| `codex/` | `~/.codex/skills/` | 自建、下载、从 GitHub 装的 |
| `codex-system/` | `~/.codex/skills/.system/` | OpenAI 官方随 Codex 分发（Apache-2.0，LICENSE 原样保留） |
| `claude/` | `~/.claude/skills/` | Anthropic 侧 |
| `agents/` | `~/.agents/skills/` | 跨工具通用目录 |
