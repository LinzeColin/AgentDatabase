...
## 五·补 ★ 我猜「署名判据会被书名里的人名骗到」——实测**没有**

那 9 份里题名含他名字的（论战文、公开信德英两版、Youmans 文集、期刊）
拿去跑同一条署名正则：**5/5 全部未命中，0 假阳。**

也就是说**署名判据在这批上是个有效的判别器**，
不是「因为门弱所以漏了」——**门根本没被调用到那一步**：
那 9 份是靠**分档字段自称一手**混进去的，而分档是抓源自己填的。
`research.source-unclaimed` 一旦跑起来，9 份一份没漏。

★ 记这一条是因为我差点把「判据不可靠」写进结论。**先量再写。**

## 五·再补 覆盖面：这道门对多少人生效

17 份工作区 `meta.json` 里 **`historical` 14 份、`public` 3 份**，
其中 **13 份已写 `attribution_basis`**（`semmelweis-105` 是 historical 而没写，会报错）。

**所以这不是一道罕见的门，它对绝大多数人物都生效。**
Liebig 之所以撞上，是因为**他是第一个在这道门下逐份核到底的人**。
