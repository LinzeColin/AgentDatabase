...
## ★★★ 射程：**只跑 `--phase research`，不许扩到 `--phase release`**

2026-08-18 我试过扩，**当场被实测否掉，记在这里免得下一个人再「顺手扩一下」**：

拿同一个空语料工作区跑 `--phase release`，`content_review` 有 10 栏，本件的判定标出 2 栏：

    answer_surface_leak  → ✓ 总体均长比 1.21（门 ≤1.3）　候选更短 5/16 = 31%
    gate_reachability    → ✓ 各绝对分门都在两席实测可达范围内

**这两个 ✓ 是真读数，不是假绿** —— 它们读的是 `evals/candidate_answers.json`（16 题）
与 `evals/results.jsonl`（64 行），**那些文件都在**；空的只是语料。

⇒ **本件的前提「语料为空 ⇒ 每栏都该说未核」，只对读语料的那些栏成立。**
研究门那 6 栏全部读语料，所以成立；发布门读答案与分数，**扩过去就是 2 条误报**。

★ 这条也是本件为什么**不点名栏位**的另一面：不点名让新增的栏自动被覆盖，
但同时意味着**换一个 phase 就会把不该管的栏也收进来**。射程靠 phase 划，不靠名单。
