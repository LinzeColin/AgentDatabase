# 抓源 → 研究阶段的工具（2026-08-12 新建，第 1 批 10 人跑通）

> ★ **件数以下方表格为准，本文不写死数字** —— 原来标题写「十件」、正文第 3 行写「九件」，而表格实际是 **13 件**：**同一份文档里两个数、都错**。
> 会再加工具的东西，写判别式不写计数。

**这些不是判据**（判据在 `registry/codex/persona-distiller/scripts/`，本轮一件没动；**件数现数不写死**：`ls scripts/check_*.py | wc -l` —— 此处原写 91，2026-08-17 实测 **93**）。
