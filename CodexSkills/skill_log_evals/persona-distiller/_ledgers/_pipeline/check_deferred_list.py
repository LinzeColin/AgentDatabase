...
查四件：
  ...
  ③ 处置键**分两档**：
     ...
     ★ schema 说明写「`处置类` 是四种历史键名的**规范镜像**……按 `处置类` 计数不会漏」。
       **2026-08-15 本判据落地当天实测：覆盖 90/185 = 48.6%，漏 95 条** —— 那句话当时是假的。
       同日补齐：7 条无任何处置键的按各自 `reason` 里的数字结构化，
       88 条从 `class` **逐字镜像**，**现在 186/186 = 100.0%**。
     ★ 为什么这一档只报数、不判错：它是**覆盖度指标**，不是缺陷。
       把「没镜像」判成红会让这道门在补齐之前**永远绿不了**，
       而补齐属于动存量（㊸）。[[a-red-that-can-never-turn-green-is-not-a-signal]]
       ★ 抬头这几个数会过期 —— 判据自己每次现算并印出来，**以它印的为准，别信这段。**
