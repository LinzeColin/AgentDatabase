# T2 并行线程 · session 启动 prompt（2026-08-20）

> 复制本文件整段作为新 session 的第一条消息。goal prompt 见文末（用户在 goal 模式用）。
> 你的工作目录：`/Users/linzezhang/Documents/Codex/GithubProject/AgentDatabase-T2`（git worktree，branch `thread2-301-450`）

## 你是谁

你是 persona-distiller「蒸馏至 600」三线程中的 **T2**：负责编号 **301-450**
（政治法律师 50 + 客户营销师 50 + 建造采购师前 50）。T1 主线程负责 1-300，T3 负责 451-604。

## 第一步（必做，按序读）

1. `cd /Users/linzezhang/Documents/Codex/GithubProject/AgentDatabase-T2 && git merge main --no-edit`（本 worktree 与主树共享 .git；T1 在 main 上提交，你本地 merge 即可拿到最新指令与 T1 产物，无需 remote pull）
...
6. 读你的切片 `CodexSkills/skill_log_evals/persona-distiller/_ledgers/_并行分片/T2-301-450.json`（status：REG 已入库/DEF 延后/TODO 待蒸）
...
- 你只 commit 自己的切片产物：`_corpora/wip-<slug>-*/`、`persona-distiller-group/政治法律师或客户营销师或建造采购师/<slug>/`、自己切片 JSON 的 status。
...
- **政治法律师族**：Hammurabi / Pericles / Lycurgus / Polybius / Jean Bodin / Thomas Hobbes …（32 TODO）
- **客户营销师族**（每 5 人须含最少三族之一，客户营销是 T2 的缺口族）：P. T. Barnum / Albert Lasker / John E. Kennedy …
- **建造采购师族**：Donald Reinertsen / Martin Fischer / David Hulett / Vitruvius …
...
T2 在过程中暴露的 skill 缺陷 / 判据问题 / 台账不一致，写进
`CodexSkills/skill_log_evals/persona-distiller/_ledgers/_pipeline/THREAD-FEEDBACK-T2.md` 并 commit；
...
```
蒸馏至600：T2负责301-450（政治法律/客户营销/建造采购前50），在途当天清、日蒸12-15人；
