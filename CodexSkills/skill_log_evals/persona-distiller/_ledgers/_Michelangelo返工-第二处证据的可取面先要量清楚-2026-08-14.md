...
# 续三：八条**全部**落在同一部书信集上 —— 逐条搜出来的，不是推断

上一节说「另 7 条大概率同形状，但『大概率』不是实测」。现在实测完了。

## 先把「独立作品」这个数算对

写了 `workclusters.py`（抽样哈希 8-gram ＋ 并查集，包含率 ≥0.30 判同一部），
**4 秒跑完 1081 对**：

    train 有正文 **47 份** → 工具按 id 报 29／47 → **实测独立作品 32 部**（合并 21 对）

合并出来的组里，与本件相关的两组：

    ── 一部（4 个 id）：书信集
         src-6094206729a1 / src-14161091ddb3 / src-8999a5688bea / src-34bb6d56038a
    ── 一部（2 个 id）：Le rime 1821
         src-43c819c03a55 / src-b69075a7bf0b
    ── 一部（2 个 id）：**英文 A record ＋ MICHELANGELO**
         src-8539ad71569a / src-4b92a7f08408

★ 判据自己印了限制：**「没被合并 ≠ 独立」**，跨语言 n-gram 恒 0，译本会落在不同组。
所以 32 是**上界**。
...
| 断言 | 引的 source | 实际属于 |
|---|---|---|
| `clm-a928f1063ac7` | `src-34bb6d56038a` | 书信集 |
| `clm-57132310d198` | `src-34bb6d56038a` | 书信集 |
| `clm-584dece9bffe` | `src-34bb6d56038a` | 书信集 |
| `clm-2a3b37136bc9` | `src-34bb6d56038a` | 书信集 |
| `clm-7cee3cb6b511` | `src-8999a5688bea` | **书信集**（与上面同一部，包含率 0.5930） |
| `clm-f7f67f29f5af` | `src-8539ad71569a` | 书信的**英译本** |
| `clm-504ac43c44d3` | `src-8539ad71569a` | 书信的**英译本** |
| `clm-f7a2ee35980a` | `src-43c819c03a55` | 诗集，**但引的那段是一封信** |

最后一条单独查过：那段施工指令落款是
`Roma 35, Settembre 1550 Michelagnoìo Buonarroti`（给瓦萨里的洛伦佐图书馆楼梯信），
只是印在《Le rime》1821 的附录里。拿三个技术短语回搜书信集：

    discostino=1｜tre palmi=1｜imbasamento del ricetto=1   （4 部书信集里的 3 部）

⇒ **同一封信，诗集只是转印。**

## ⇒ 定案

**八条断言，全部落在同一部书信集上。** 语料 3,516,622 词、32 部独立作品，
而**方法/价值/盲区类的证据面只有一部**。

这不是「没搜够」：
- `clm-a928f1063ac7` 已经搜到了**第二处独立行为**（1507 与 1555 两封信，隔 48 年），
  它装不进去只因为两封信在同一卷里；
- 其余七条即使也搜得到第二封，**结局完全一样**。

**⇒ Michelangelo #185 记延后类⑦「方法证据全部汇到一部作品」。**

★★ 与 Pacioli #161 的区别必须写在判决书里，否则下一个人会去重新抓源：

| | Pacioli #161 | Michelangelo #185 |
|---|---|---|
| 语料量 | 不够 | **够**（351.6 万词 / 56 份全读得到 / 32 部独立作品） |
| 第二处证据 | 真的没有 | **有**（已找到一例并逐字核过） |
| 卡点 | 证据只有一部书 | **证据单位是「卷」，而书信的自然单位是「封」** |

**语料是够的。不够的是格子。**

## 本轮没做的、以及为什么

- **没重切语料。** 按封重新入库会改变全库有书信人物的源计数/一手占比/道数/阈值，
  而 ㊵ 已裁「已判分即冻结」。为一个人改全库证据模型，代价远大于收益。
- **没动门。** 判据是纯结构的（数 set 长度），填两个同一部书的 id 就能绿 ——
  那是 ㊸ 判过的假独立，不做。
- **没给另七条各写一段搜索记录。** 它们的源已逐条归位到同一部书信集，
  再搜也改变不了结局；这一条是**结构判定**，不是「搜累了」。
