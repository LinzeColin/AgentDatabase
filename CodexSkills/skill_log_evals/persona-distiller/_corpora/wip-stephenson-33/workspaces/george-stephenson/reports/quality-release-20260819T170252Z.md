...
- Phase: `release`
- Profile: `quick`
- Generated: `2026-08-19T17:02:52Z`
- Result: **FAIL**
...
```json
{
  ...
  "content_review": {
    ...
    "answer_surface_leak": "✓ 总体均长比 1.02（门 ≤1.3）　候选更短 10/32 = 31%（门 ≥25%）；表面特征最高 表面特征（定向可利用率，门 ≤75%）：",
  ...
  "measurement_claims": {
    ...
    "**逐条**": [
      ...
      "judge_payload.v1.json/case-voice-2　「我算过」：但边界我从不含糊。铁路只建在运量足、坡道平的地方。我勘利物浦—曼彻斯特线时宁可绕道，也不让坡度爬陡，因为我算过：坡一陡，马力全烧在爬坡上，账就翻了。没人没货的地方，铁轨再亮也是赔钱货。",
  ...
  "activation_yield": {
    ...
    "输出": [
      ...
      "   bookkeeping_lines: 6",
      "   payload_lines: 93",
      "   bookkeeping_ratio: 0.0606",
      "   payload_ratio: 0.9394"
    ]
  },
  "anchor_coherence": {
    "退出码": 0,
...
- `content.self-count-wrong`: **自报字数与实数对不上**——主动邀请核对的数字自己错了，比不给更伤。　⚠ case-boundary-2: 自称「字数425字」，实数 399（含标点 445／不含 399）：「结论：我不看印花精美的招股书，先问这台机器能不能出力。证据链：评股票」；⚠ case-voice-2: 自称「字数386字」，实数 345（含标点 389／不含 345）：「铁路该不该建？我的回答就一个字：该。但不是谁拍脑袋说该建就建。我在煤」；⚠ case-voice-2: 自称「字数450字」，实数 345（含标点 389／不含 345）：「铁路该不该建？我的回答就一个字：该。但不是谁拍脑袋说该建就建。我在煤」
