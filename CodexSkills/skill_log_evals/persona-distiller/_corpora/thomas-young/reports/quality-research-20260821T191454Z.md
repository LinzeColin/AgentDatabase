...
- Phase: `research`
- Profile: `quick`
- Generated: `2026-08-21T19:14:54Z`
- Result: **FAIL**
...
```json
{
  ...
  "fact_density": {
    ...
    "**未达**": [
      "可核 `fact` 断言 0 条 < 要求 5 条（12 条可用源 ÷ 5） —— **语料里可核的具体事实没有进入断言层**；这正是 Galen #101 真 delta −0.1259 的根因"
    ]
  },
  "quote_layer": {
    "已扫文件": 0,
    ...
    "逐份": {
      ...
      "src-e81a21303566": {
        ...
        "逐语域": {
          "英文": {
            "语域": "英文",
            "anchors_per_10k": 2387.9,
            "panel_good": 1642,
            "panel_bad": 0,
            "若无语种门会读到": 0.0,
            "verdict": "干净",
            "rate": 0.0,
            "reason": "英文讹字率 0.0000（正形 1642／讹形 0）"
          }
        },
        "verdict": "干净",
        "rate": 0.0,
        "reason": "英文讹字率 0.0000（正形 1642／讹形 0）",
        "file": "miscellaneouswo01youngoog.txt"
      },
      "src-e0b52403d6e4": {
        "words": 217989,
        "diagnostic_est_eft": [
      ...
      "src-307f6de6c9ac": {
        ...
        "逐语域": {
          "英文": {
            "语域": "英文",
            "anchors_per_10k": 2511.6,
            "panel_good": 140,
            "panel_bad": 0,
            "若无语种门会读到": 0.0,
            "verdict": "干净",
            "rate": 0.0,
            "reason": "英文讹字率 0.0000（正形 140／讹形 0）"
          }
        },
        "verdict": "干净",
        "rate": 0.0,
        "reason": "英文讹字率 0.0000（正形 140／讹形 0）",
        "file": "bakerianlectureo00youn.txt"
      },
      "src-aa3bf0356385": {
        "words": 13277,
        "diagnostic_est_eft": [
      ...
      "src-a593eb85e6fa": {
        ...
        "逐语域": {
          "英文": {
            "语域": "英文",
            "anchors_per_10k": 2476.1,
            "panel_good": 270,
            "panel_bad": 0,
            "若无语种门会读到": 0.0,
            "verdict": "干净",
            "rate": 0.0,
            "reason": "英文讹字率 0.0000（正形 270／讹形 0）"
          }
        },
        "verdict": "干净",
        "rate": 0.0,
        "reason": "英文讹字率 0.0000（正形 270／讹形 0）",
        "file": "b2934962x_0001.txt"
      },
      "src-2ed4144cd162": {
        "words": 112226,
        "diagnostic_est_eft": [
      ...
      "src-dfd0f1c3d578": {
        ...
        "逐语域": {
          "英文": {
            "语域": "英文",
            "anchors_per_10k": 2338.0,
            "panel_good": 561,
            "panel_bad": 0,
            "若无语种门会读到": 0.0,
            "verdict": "干净",
            "rate": 0.0,
            "reason": "英文讹字率 0.0000（正形 561／讹形 0）"
          }
        },
        "verdict": "干净",
        "rate": 0.0,
        "reason": "英文讹字率 0.0000（正形 561／讹形 0）",
        "file": "elementaryillust00laplrich.txt"
      }
    }
  },
  "lane_scope": {
  ...
  "corpus_ceiling": {
    ...
    "有材料的道数": 1,
    "quick 要的一手份数": 4,
    "够得着吗": "够不着：六条道只占 1 < 3——**空着的道抓再多别的也补不上**"
  },
  "corpus_feasibility": {
    "profile": "quick",
    "可用材料总数": 13,
  ...
  "holdout_overlap": {
    ...
    "其中·无法判定": 0,
    "**逐条**": [
      "✗ src-dfd0f1c3d578 **最长连续逐字段 131 词**（≥50 词的段共 52 处）",
      "✗ **非样板的连续逐字段 52 处 ≥ 50（最长 131 词）**"
    ],
    "口径": "holdout 的内容已在 train 中出现——**用它出的 known 题不测泛化，且一定得高分**。正解是换源，不是调阈值。"
  }
}
```
...
- `research.authorship-unproven`: src-307f6de6c9ac bakerianlectureo00youn.txt｜文中他人署名：无 —— 账本声称本人所著，但文中查无归属证据（署名／编者注／逐字稿轮次三者皆无）
- `research.authorship-unproven`: src-803adfdb95b7 jstor-107159.txt｜文中他人署名：无 —— 账本声称本人所著，但文中查无归属证据（署名／编者注／逐字稿轮次三者皆无）
- `research.authorship-unproven`: src-dfd0f1c3d578 elementaryillust00laplrich.txt｜文中他人署名：无 —— 账本声称本人所著，但文中查无归属证据（署名／编者注／逐字稿轮次三者皆无）
- `corpus.undeclared-duplicate-sources`: **8 对来源重叠 ≥0.3 而两边都没声明 `derived_from`**——台账上看不出它们是同一部作品。**清掉这条错有两条路**：① 补 `derived_from`（同一部作品的另一份扫本）；② 在 `meta.json` 的 `attribution_basis.counting_convention` 里**逐对点名**说明为什么它们该当两处证据（2026-08-17 起这条路真的通了——在那之前它只是句话，代码从没读过 `counting_convention`）——★★ 2026-08-17 订正：上面这几句原本写的是「本件只读 `derived_from`，在 `counting_convention` 里写散文不会让它变绿」——**那句当时是对的**，因为那条出路从没被实现。今天已实现，但**只认逐对点名**：约定文本里要同时出现这一对的两个文件名才算，泛泛的散文仍然不豁免任何一对。　[('lecturescourseof02younrich', 'bakerianlectureo00youn.txt', 0.4018), ('lecturescourseof02younrich', 'jstor-107049.txt', 0.4048), ('lecturescourseof02younrich', 'jstor-107159.txt', 0.4762)]
- `corpus.source-count-inflated-by-duplicates`: **`source.minimum` 只是被重份撑绿的**：可用来源 12 ≥ 门 8，而按内容去重后只有 **6 部作品**（虚高 2.0×）。**这道门量的是「有几个 source_id」，不是「有几处独立证据」。**
- `research.lane-completion`: completed source-linked lanes 0 < profile minimum 3: []
- `corpus.holdout-overlap`: holdout 与 train 有内容重合（2 条**真重合**，另有 0 条无法判定另计）——**现在换源还来得及**；逐条见 metrics.holdout_overlap，受污染段清单见 reports/holdout-contaminated-passages.json

## Warnings

- `corpus.no-viable-holdout-split`: **扣任何一份当 holdout 都满足不了 profile 门**：道数 1 < min_lanes 3（['writings']） —— 差的是材料，不是文字
