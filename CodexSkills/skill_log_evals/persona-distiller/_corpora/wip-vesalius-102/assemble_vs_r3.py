...
WS = SP / "ws-vesalius/andreas-vesalius"


def main() -> int:
    key = json.loads((SP / "vs_blind_key.json").read_text(encoding="utf-8"))
    cases = {json.loads(l)["case_id"]: json.loads(l)
             for l in (WS / "evals/cases.jsonl").read_text(encoding="utf-8").splitlines() if l.strip()}
    seats = {}
    for seat, f in (("seat-D-score-v1", "vs_judge_D3.json"), ("seat-E-strict-v1", "vs_judge_E3.json")):
        p = SP / f
        if not p.is_file():
    ...
    json.dump({"candidate": round(C, 4), "bare_model": round(B, 4), "delta": round(C - B, 4),
              ...
              open(SP / "galen_blind_result_r3.json", "w", encoding="utf-8"), ensure_ascii=False, indent=1)
    return 0
