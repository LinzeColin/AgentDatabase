...
def main() -> int:
    cand = json.loads((SP / "vs_candidate.json").read_text(encoding="utf-8"))
    bare_p = SP / "vs_baseline_bare.json"
    if not bare_p.is_file():
        print("缺 vs_baseline_bare.json —— 裸模型对照臂未就绪，**不构造载荷**", file=sys.stderr)
        return 3
    bare = json.loads(bare_p.read_text(encoding="utf-8"))
    cases = {json.loads(l)["case_id"]: json.loads(l)
             for l in (SP / "ws-vesalius/andreas-vesalius/evals/cases.jsonl").read_text(encoding="utf-8").splitlines() if l.strip()}
    ...
    (SP / "vs_blind_payload.json").write_text(json.dumps(payload, ensure_ascii=False, indent=1), encoding="utf-8")
    (SP / "vs_blind_key.json").write_text(json.dumps(key, ensure_ascii=False, indent=1), encoding="utf-8")
