...
# ── ★ v0.0.0.28 补：事实密度门（check_fact_density）要求 59 源 ≥12 条可核事实。
#    原来只有 5 条——**244 万词语料只蒸出 5 条事实**，正是真 delta −0.1259 的根因。
#    下面每一条都带可核的专名或数字，且来自已落盘的语料清单或已读的英译/传记文本。
add("fact",
    "他篇幅最大的著作是《De usu partium corporis humani》（论身体各部分的用处），"
    "本工作区解出的希腊文正文 **195,034 词**；其次是《De methodo medendi》（治疗方法论）**158,039 词**。"
    "**两部合计已超过整部希波克拉底文集的规模。**",
    [S_ANAT, S_PLAC], ["评估其著述重心", "为读者定阅读优先级"], [C_ANAT, C_SELF],
    ["若语料清单更新导致词数变化，本条须同步更新"], "本工作区 2026-08-02 口径", 0.9, status="fact")

add("fact",
    "他为希波克拉底文本写过 **11 部注疏，合计 578,737 词**——"
    "其中《In Hippocratis Aphorismos Commentarii VII》96,467 词、"
    "《In Hipp. Epidemiarum VI》76,007 词、《In Hipp. De victu acutorum》67,295 词。"
    "**注疏量约占其存世希腊文著作的四分之一。**",
    [S_APH, S_EPI6, S_EPI1], ["评估他与希波克拉底传统的关系", "找注疏体样本"], [C_COMM, C_SELF],
    ["若注疏部数或词数统计更新，本条须同步"], "本工作区口径", 0.9, status="fact")

add("fact",
    "真伪分层的量级对比：**89 部真作合计 2,442,576 词；16 部伪托／存疑合计仅 77,006 词**"
    "（约 3.1%）。**伪托本在篇幅上是少数，在检索结果里却与真作混在一起无法分辨**——"
    "因为两者的现代版扉页署名一模一样。",
    [S_LIB, S_ORD], ["真伪分层", "向使用者说明风险量级"], [C_SELF, C_NAT],
    ["若分层依据更新导致 89/16 划分改变，本数须同步"], "本工作区口径", 0.9, status="fact")

add("fact",
    "据 John the Grammarian 的记载（经 Ibn Abī Uṣaybiʿah 转述）："
    "**他享年 87 岁，其中「为少年与学生 17 年，为学者与教师 70 年」**。"
    "**这是转述而非同期记载**，与「卒年 199 或 216」两说须并陈使用。",
    [S_USA, S_LIB], ["生平时序", "评估中世纪传记的证据层级"], [C_EXT, C_SELF],
    ["若找到同期文献给出年寿，本条降为并陈之一"], "13 世纪转述，述及约 129–216 年", 0.7)

add("fact",
    "他著作中称的「Antoninus」指的是**马可·奥勒留**（161 年即位，为纪念其养父安敦宁·庇护而加此名）。"
    "**他第一次到罗马是 162–166 年。** 读其著作时把「Antoninus」当成安敦宁·庇护是常见误读。",
    [S_USA, S_LIB], ["为其著作定年", "读其原文时避免误认皇帝"], [C_EXT, C_SELF],
    ["若其原文中出现明确指向安敦宁·庇护的用例，本条须加限定"], "约 161–166 年", 0.78)

add("fact",
    "他对方法派（methodikoi）的批评集中在《De methodo medendi》第十卷第 7 节起，"
    "针对的具名对象是 **Thessalus**。阿拉伯译者把 methodikoi 译作 aṣḥāb al-ḥiyal"
    "（字面意为「用计谋的人」）——**这个贬义译名的源头正是盖伦本人的描述**。",
    [S_USA, S_ARS], ["理解他与方法派的分歧", "评估后世对方法派的印象从何而来"], [C_EXT, C_NAT],
    ["若在其原文中找到对方法派的中立表述占主导，本条须加限定"], "约 157–216 年", 0.75)

add("fact",
    "他的药学著作规模极大且分两个体系："
    "《De compositione medicamentorum secundum locos》（按部位）**144,244 词**、"
    "《De compositione medicamentorum per genera》（按类型）**104,952 词**、"
    "《De simplicium medicamentorum temperamentis ac facultatibus》（单味药）**134,365 词**。"
    "**「按部位」与「按类型」两套并行的编排，本身就是他分类先于定因那套做法的实例。**",
    [S_ANAT, S_SYM], ["理解其分类方法", "评估其药学著作的组织方式"], [C_NAT, C_ANAT],
    ["若词数统计更新，本条须同步"], "本工作区口径", 0.85, status="fact")

add("fact",
    "《De optimo medico cognoscendo》（论如何辨认最好的医生）在他自著书目中编号为 **112**。"
    "**他的书目是编号的**——这使「第几号」成为一个可查证的指认方式，而不只是书名。",
    [S_LIB, S_ORD, S_USA], ["按编号指认其著作", "评估其编目的精细度"], [C_SELF, C_EXT],
    ["若该编号在不同传本中不一致，本条须改为并陈"], "中年至晚年", 0.72)

add("fact",
    "《De placitis Hippocratis et Platonis》（论希波克拉底与柏拉图的学说）**94,934 词**，"
    "是其最长的纯论证性著作——**它用哲学论证方法处理生理学争点**，"
    "而不是用临床经验，这是「医学判断力与哲学论证要求是同一件事」的最大篇幅证据。",
    [S_PLAC, S_ARS], ["理解其方法论立场", "找其论证体样本"], [C_PLAC, C_NAT],
    ["若词数统计更新，本条须同步"], "本工作区口径", 0.88, status="fact")

add("fact",
    "他早年任职于帕加马，为亚细亚大祭司的角斗士治伤（约 157 年起）。"
    "**Ibn Juljul 记其「出生与成长都在帕加马」**。"
    "角斗士创伤是他早期解剖知识的来源之一——**活体创口提供了不许剖人的时代拿不到的观察窗口**。",
    [S_USA, S_ANAT], ["理解其解剖知识的来源", "生平时序"], [C_EXT, C_ANAT],
    ["若找到他本人否认从角斗士创伤中学习的表述，本条须改写"], "约 157 年起", 0.75)
