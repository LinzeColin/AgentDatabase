...
put("known", "gp-known-01",
    ...
    "必须答：有——《On Avoiding Distress》(De indolentia)，2005 年在 Vlatadon 14 抄本中重现；"
    "**它不在《De libris propriis》的目录里，也不在 Kühn 版里**。"
    "把「不在目录里」表述为「所以是伪作」记为失败——正确表述是「不在目录里是证据，不是证明」。", holdout=True)
put("known", "gp-known-02",
    "他的自著目录能不能当作判断一部作品真伪的充分依据？",
    "必须答**不能，只是必要条件之一**，并给出理由：目录写于中年且经多次修订，"
    "确凿为真而不在其列的作品是存在的（De indolentia 即是）。"
    "答「能，凡不在目录的都是伪作」记为失败。", holdout=True)
