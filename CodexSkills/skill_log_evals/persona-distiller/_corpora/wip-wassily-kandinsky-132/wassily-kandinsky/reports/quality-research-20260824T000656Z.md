...
- Phase: `research`
- Profile: `quick`
- Generated: `2026-08-24T00:06:56Z`
- Result: **FAIL**
...
```json
{
  ...
  "material_split": {
    ...
    "**逐条**": [
      ...
      "✗✗ holdout 正文出现在此：src-8602bfda9146  ← 隔离失效，本轮 known 分数不可信",
      "✗✗ holdout 正文出现在此：src-7854e9844c36  ← 隔离失效，本轮 known 分数不可信",
      "✗✗ holdout 正文出现在此：src-8602bfda9146  ← 隔离失效，本轮 known 分数不可信"
    ],
...
- `research.authorship-unproven`: src-3c2a619a83a1 kandinsky_202603.txt｜文中他人署名：无 —— 账本声称本人所著，但文中查无归属证据（署名／编者注／逐字稿轮次三者皆无）
- `research.authorship-unproven`: src-b7ab80209390 unset0000unse_n2u2.txt｜文中他人署名：无 —— 账本声称本人所著，但文中查无归属证据（署名／编者注／逐字稿轮次三者皆无）
- `research.authorship-unproven`: src-8602bfda9146 kandinsky190119102kand.txt｜文中他人署名：无 —— 账本声称本人所著，但文中查无归属证据（署名／编者注／逐字稿轮次三者皆无）
- `research.authorship-unproven`: src-7854e9844c36 derblauereiter00kand.txt｜文中他人署名：无 —— 账本声称本人所著，但文中查无归属证据（署名／编者注／逐字稿轮次三者皆无）
...
- `research.attribution-basis`: 6 条 P1 源未挂 `attribution` 字段：src-be465c1fefa1, src-a027b18b6270, src-e62aac6f8503, src-b7ab80209390, src-8602bfda9146, src-7854e9844c36 —— 每条声称是他写的源，都要说清是按哪一条依据认定的
- `research.source-unclaimed`: `src-be465c1fefa1` berdasgeistigein46203gut.txt —— 声称 `Wassily Kandinsky` 所著，**既无 A-* 署名证据，也未在 attribution_basis 里被逐份点名**。
- `research.source-unclaimed`: `src-3c2a619a83a1` kandinsky_202603.txt —— 声称 `Wassily Kandinsky` 所著，**既无 A-* 署名证据，也未在 attribution_basis 里被逐份点名**。
- `research.source-unclaimed`: `src-a027b18b6270` concerningthespi05321gut.txt —— 声称 `Wassily Kandinsky` 所著，**既无 A-* 署名证据，也未在 attribution_basis 里被逐份点名**。
- `research.source-unclaimed`: `src-8f01bd832c2a` TheSpiritualInArtByWassilyKandinsky.txt —— 声称 `Wassily Kandinsky` 所著，**既无 A-* 署名证据，也未在 attribution_basis 里被逐份点名**。
- `research.source-unclaimed`: `src-e62aac6f8503` artofspiritualha00kandrich.txt —— 声称 `Wassily Kandinsky` 所著，**既无 A-* 署名证据，也未在 attribution_basis 里被逐份点名**。
- `research.source-unclaimed`: `src-2b12ff13e0d3` kandinsky-wassily-1866-1944.-the-art-of-spiritual-harmony.txt —— 声称 `Wassily Kandinsky` 所著，**既无 A-* 署名证据，也未在 attribution_basis 里被逐份点名**。
- `research.source-unclaimed`: `src-b7ab80209390` unset0000unse_n2u2.txt —— 声称 `Wassily Kandinsky` 所著，**既无 A-* 署名证据，也未在 attribution_basis 里被逐份点名**。
- `research.source-unclaimed`: `src-8602bfda9146` kandinsky190119102kand.txt —— 声称 `Wassily Kandinsky` 所著，**既无 A-* 署名证据，也未在 attribution_basis 里被逐份点名**。
- `research.source-unclaimed`: `src-7854e9844c36` derblauereiter00kand.txt —— 声称 `Wassily Kandinsky` 所著，**既无 A-* 署名证据，也未在 attribution_basis 里被逐份点名**。
- `corpus.undeclared-duplicate-sources`: **4 对来源重叠 ≥0.3 而两边都没声明 `derived_from`**——台账上看不出它们是同一部作品。**清掉这条错有两条路**：① 补 `derived_from`（同一部作品的另一份扫本）；② 在 `meta.json` 的 `attribution_basis.counting_convention` 里**逐对点名**说明为什么它们该当两处证据（2026-08-17 起这条路真的通了——在那之前它只是句话，代码从没读过 `counting_convention`）——★★ 2026-08-17 订正：上面这几句原本写的是「本件只读 `derived_from`，在 `counting_convention` 里写散文不会让它变绿」——**那句当时是对的**，因为那条出路从没被实现。今天已实现，但**只认逐对点名**：约定文本里要同时出现这一对的两个文件名才算，泛泛的散文仍然不豁免任何一对。　[('concerningthespi05321gut.t', 'artofspiritualha00kandrich', 0.691), ('concerningthespi05321gut.t', 'kandinsky-wassily-1866-194', 0.7474), ('TheSpiritualInArtByWassily', 'artofspiritualha00kandrich', 0.6782)]
