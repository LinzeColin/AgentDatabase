...
- Phase: `research`
- Profile: `quick`
- Generated: `2026-08-24T00:28:44Z`
- Result: **FAIL**
...
```json
{
  ...
  "content_review": {
    ...
    "research_quote": "**有引文未在语料中找到**——未命中不等于伪造，须人工核对；但「改了 OCR 错字再当逐字引文」也落在这里。引文 10 条，切分后核验片段 9 个，未命中 2 个，长 s 还原后才命中 0 个｜⚠ 研究/03-expression.md: 「KANDINSKY 1901-1913 Verlag Der Sturm Berlin」｜⚠ 研究/06-timeline.md: 「Kandinsky ist 1866 in Moskau geboren worden」",
...
- `corpus.holdout-work-named-in-artifacts`: **建模者读得到的文件里有 1 处直接说出了 holdout 是哪一份**（书名／卷次页码／文件名／源 id）——这比「提到有个 holdout」严重得多，**它把那道题考什么也告诉了**。　[('03-expression.md', 5)]
- `corpus.holdout-mentioned-in-artifacts`: **建模者读得到的文件里有 2 处提到 holdout**——知道「存在一份取不到的材料、它关于某某」已足够定位那道题。　[('03-expression.md', 'holdout'), ('03-expression.md', 'holdout')]
