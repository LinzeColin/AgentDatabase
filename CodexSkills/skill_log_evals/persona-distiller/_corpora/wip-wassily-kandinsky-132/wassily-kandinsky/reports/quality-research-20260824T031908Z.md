...
- Phase: `research`
- Profile: `quick`
- Generated: `2026-08-24T03:19:08Z`
- Result: **FAIL**
...
```json
{
  ...
  "corpus_integrity": {
    ...
    "可疑": 3,
    "可疑（只报不拦）": [
      "raw/src-7adc3107df41/kandinskijvvstupenimizdotdelaizob56.txt　可读字符占比 21% < 55%——多半是二进制或彻底崩坏的 OCR",
      "raw/src-c4a2e38fa70f/vvkandinskij72.txt　可读字符占比 21% < 55%——多半是二进制或彻底崩坏的 OCR",
      "raw/src-a06b9c25b683/poschechinaobshchestvennomuvkusudburlju72.txt　可读字符占比 26% < 55%——多半是二进制或彻底崩坏的 OCR"
    ],
  ...
  "longs_corruption": {
    ...
    "逐份": {
      ...
      "src-7adc3107df41": {
        ...
        "reason": "空文本",
        "words": 0,
        "file": "kandinskijvvstupenimizdotdelaizob56.txt"
      },
      "src-c4a2e38fa70f": {
        "verdict": "未核",
        "reason": "空文本",
        "words": 0,
        "file": "vvkandinskij72.txt"
      },
      "src-df5d2ac81af0": {
        "words": 62717,
        "diagnostic_est_eft": [
        ...
        "逐语域": {
          "德语": {
            "语域": "德语",
            "anchors_per_10k": 625.7,
            "panel_good": 1653,
            "panel_bad": 0,
            "若无语种门会读到": 0.0,
            "verdict": "干净",
            "rate": 0.0,
            "reason": "德语讹字率 0.0000（正形 1653／讹形 0）"
          }
        },
        "德语附加": {
          "h→b率": 0.0,
        ...
        "rate": 0.0,
        "reason": "德语讹字率 0.0000（正形 1653／讹形 0）",
        "file": "imkampfumdiekuns00unse.txt"
      },
      "src-a06b9c25b683": {
        "verdict": "未核",
        "reason": "空文本",
        "words": 0,
        "file": "poschechinaobshchestvennomuvkusudburlju72.txt"
      }
    }
  },
  "lane_scope": {
  ...
  "content_review": {
    "unexamined_band": {
      ...
      "files": [
        ...
        "kandinskijvvstupenimizdotdelaizob56.txt",
        "poschechinaobshchestvennomuvkusudburlju72.txt",
        "vvkandinskij72.txt"
      ]
    },
    "byline_in_carrier": "核过 16 条，指错 0 条",
    "fraktur_mojibake": "✓ 没有花体乱码（德文 **7** 份逐份看过；共读到 19 份）",
    ...
    "research_quote": "**有引文未在语料中找到**——未命中不等于伪造，须人工核对；但「改了 OCR 错字再当逐字引文」也落在这里。引文 10 条，切分后核验片段 9 个，未命中 1 个，长 s 还原后才命中 0 个｜⚠ 研究/06-timeline.md: 「Kandinsky ist 1866 in Moskau geboren worden」",
  ...
  "stance_density": {
    ...
    "★ 未核验的逐条（不并进分母，也不算 0）": [
      ...
      {
        "source_id": "src-7adc3107df41",
        "原因": "语种判为 **?**（en=0.000 de=0.000 fr=0.000）——**本件只认英语，不是「这个人没有声口」**"
      },
      {
        "source_id": "src-c4a2e38fa70f",
        "原因": "语种判为 **?**（en=0.000 de=0.000 fr=0.000）——**本件只认英语，不是「这个人没有声口」**"
      },
      {
        "source_id": "src-df5d2ac81af0",
        "原因": "语种判为 **de**（en=0.000 de=0.146 fr=0.007）——**本件只认英语，不是「这个人没有声口」**"
      }
    ],
    "第一人称（动词式）/万字": 7.34,
    "**立场句/万字**": 0.33,
...
- `research.source-unclaimed`: `src-7adc3107df41` kandinskijvvstupenimizdotdelaizob56.txt —— 声称 `Wassily Kandinsky` 所著，**既无 A-* 署名证据，也未在 attribution_basis 里被逐份点名**。
- `research.source-unclaimed`: `src-c4a2e38fa70f` vvkandinskij72.txt —— 声称 `Wassily Kandinsky` 所著，**既无 A-* 署名证据，也未在 attribution_basis 里被逐份点名**。
- `research.source-unclaimed`: `src-df5d2ac81af0` imkampfumdiekuns00unse.txt —— 声称 `Wassily Kandinsky` 所著，**既无 A-* 署名证据，也未在 attribution_basis 里被逐份点名**。
- `research.source-unclaimed`: `src-a06b9c25b683` poschechinaobshchestvennomuvkusudburlju72.txt —— 声称 `Wassily Kandinsky` 所著，**既无 A-* 署名证据，也未在 attribution_basis 里被逐份点名**。
...
- `corpus.unexamined-band`: **4/37 份语料落在两道门之间**：字符数够 `non_placeholder`（≥500）而词数不够语种判据（<500），**没有任何判据看过它们的内容**。★ 短不等于坏，本条是覆盖缺口不是缺陷；但若某条道只靠这类文件撑着，请人看一眼。
...
- `corpus.title-is-just-the-filename`: **1/20 行的 `title` 就是文件名**（5%）——这个字段没有承载信息。后果不是难看：判「两份是不是同一部作品」时**除了内容重叠没有第二个证据源**，引文坐标与「挂到哪部作品」也全落在文件名上。★ 与空值不同——**空值至少诚实，填成文件名的字段看起来是填过的**。
