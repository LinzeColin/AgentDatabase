# Decisions

## Scope and assigned sources

- `src-7b92506095cc`（P1）— 1841 论文：**本道唯一有一手支撑的决定**在这里。
- `src-231dfc291417`（P1）— 1836 证词：一处**当场做出的划界决定**。
- `src-940890f8d2dd`（S2）— 讣告：职业生涯的几个转折，**全部只有这一处出处**。

★ **自传未读**。本道的 S2 条目**几乎都能在自传里核到**，那是下一轮的第一件事。

## Source-linked observations

### ★★★★ D1（P1）：他决定在自己的论文里把功劳记在别人名下

1841 ¶491——**这是他自己写的、可核的一次选择**，不是别人对他的评价：

> It would be **blamable** indeed (after having endeavoured to set forth the vast advantages
> which have been conferred on the mechanical world … by the invention and introduction of
> the slide rest) **were I to suppress the name of that admirable individual** to whom we are
> indebted for this powerful agent … I allude to the late **Henry Maudslay** … **to him we are
> certainly indebted for the slide resU**

★ 三处细节使它成为「决定」而非客套：
1. 他用 `blamable`——**不点名是该受责备的**，即他把这当成义务而非礼节；
2. 文末 ¶504 **再点一次**（`as introduced hy the late celehrated Henry Maudslay`）；
3. 这篇论文的主题**正是滑动原理本身**——**他是在自己最有资格居功的地方让功。**

### ★★ D2（P1）：被追问时他当场把自己的主张缩小

1836 Q.307 他提出在工厂里陈列有品味的作品；主席追问这是不是要取代 Mechanics Institutions
（Q.308）——他没有顺势扩大，而是**当场缩小射程**：

> **not as a substitute** for similar wivantages afforded by Mechanics Institutions, **but as a
> collateral assistance**

★ 与 D1 同一族：**在有机会把自己的东西说大的位置上，他选择说小。**

### D3（P1）：他决定把「靠直觉」的手艺写成可推的规则——理由是操作者换人了

1841 ¶505 明说动机：

> now that slide lathes and planing machines are becoming so very common … and that such
> machines, from their automaton power, **no longer require regularly bred mechanics to
> attend them**, it becomes more than ever necessary to reduce the subject to those simple
> principles

★ **他不是为了传播知识而写，是因为操作岗位的人变了。** 这与 `01-writings.md` 的 C3
（他对工匠能力的两种相反评价）是同一处文本的两面——**C3 已记为矛盾项，此处不重复调和。**

### D4（S2）：1829 年他用自己手做的模型换来 Maudslay 门下的位置

> producing some of the model steam-engines and other apparatus he had made with his own hands, ™
> he obtained a situation, at the modest salary of 110s. per week, as assistant to Henry Maudsley

★ **出示实物而不是出示履历。** 与 `02-conversations.md` 里 Q.304
（他把资格说成「两件事的交集」而非权威）方向一致——**但那是 P1，这是 S2，不能并列成两处证据。**

### D5（S2）：Maudslay 一去世他就自立，而不是留在名厂

> death in 1831, he resolved to start business on his own. account. He returned to Edinburgh, put
> up a temporary i workshop, and set to work to construct machine tools

★ 顺序值得注意：**先造机床，两年后才去曼彻斯特开业。**——**先备工具，再开张。**

### ★★★ D6（S2）：他先把方案给了对手，专利是**发现对方造出来之后**才申请的

见 `06-timeline.md` 的完整引文：1839 构思 → 锻件取消 → 把方案好处给了 Le Creusot 的
Schneider → 两年后访 Creusot 发现已建成运转 → `After this he lost no time. in obtaining a patent`（句中那个句点是 OCR 的，原字保留）。

★ **这一条如果在自传里核实，是本人物最强的一条决策证据**：
它同时说明**他起初不打算圈占**，以及**他在什么条件下改变主意**。
**但目前只有 S2 一处，且涉及第三方。产物里不得当成确定事实叙述。**

### D7（S2）：1857 年、49 岁，带着大笔财产退休

> in 1857 he was able to retire | with a large fortune. He bought a delightful retreat in Kent

★ 与 P1 侧对得上：MNRAS 论文与 Crayon 文章集中在 **1851–1855**，**退休前就已在做**；
1874 年《The Moon》在退休后 17 年。**「退休后转向天文」这个通行说法与 P1 的年份不完全吻合**——
**他退休前就在发表天文论文。**

## Candidate Claims

**K1（value，达标）**：**在自己最有资格居功的地方让功，并把「不点名」称为该受责备的。**
- 证据 A：1841 ¶491＋¶504（`src-7b92506095cc`）
- 证据 B：1836 Q.308 当场把主张缩小为 `collateral assistance`（`src-231dfc291417`）
- 语境：「技术论辩」「答问」　→ **2 个 source_id、2 个语境，达标。**
- ★ 与 `01-writings.md` C4、`03-expression.md` E2 是**同一族的三个侧面**；
  **合并还是分列，等自传读完再定**——现在分列会造成断言层重复计数。

**K2（work-method，待核）**：**先备工具，再开张**（D5）。
- 只有 S2 一处。**自传里必然写到，核到即可成条。**

**K3（fact，待核，★ 但已可证伪一个通行说法）**：
**他在退休（1857）之前就已发表天文论文**（MNRAS 1851/1852/1855 均在退休前）。
- 证据 A：五篇 P1 自证年份
- 证据 B：讣告称退休后 `devoting himself to astronomical … researches`（S2）
- ★ **两者不冲突，但常见叙述「退休后转向天文」是不准确的**——**这一条可直接进 divergence-map。**

## Contradictions and alternative explanations

- **D1／D2／D3 都出自同一篇 1841 论文**——K1 之所以达标，靠的是 D2 来自另一份来源（1836）。
  **不要把同一篇里的三处当成三处证据。** [[two-source-ids-is-not-two-evidences]]
- **D4–D7 全部只有讣告一处**，而讣告与自传高度重合（见 `04-external.md` 的独立性警告）。
  **自传核到之前，它们不是两处证据，是一处。**
- **D6 的另一种解释**：讣告是褒扬体裁，「先给对手、后来才申请专利」是**很讨好的叙述**。
  **必须在自传里核，且要留意自传本身也是他自己的叙述**——两者都不是中立记录。
- **D3 的动机陈述可能是修辞**：说「因为操作者不再是科班机匠」也许只是给一篇技术文章找由头。
  ★ 但他在同一段里紧接着写 `from whom we want no more than careful attention`，
  **措辞是配套的**，不像临时找的理由。

## ★★★★ 2026-08-10 更正：**D6 被自传（P1）推翻，讣告把一次意外写成了一桩慷慨**

`05-decisions.md` 初稿据讣告（S2）写「他先把方案给了对手，专利是发现对方造出来之后才申请的」，
并已标明「自传核到之前不得当成确定事实」。**核了，不成立。**

- **讣告（S2）**：原文这一句 OCR 已经坏到不能当引文用——逐字是
  `Mr mara Bipeber ab the | gave the benefit of his Shox to M. Ecinaidae, the proprietor of
  the great ironworks at Creuzot`。**主语与受赠人两处都读不出来。**
  ★★★★ 此处**曾被我写成**‹Mr Nasmyth … gave the benefit of his ［sketch］ to M. ［Schneider］›
  （★ 这一行**有意不用反引号、方括号也换成全角**：本道判据把反引号内的长英文串一律
  当成「声称逐字」去核，而这句**恰恰是反面例子**。**反面例子不该冒充引文**——
  改的是我的写法，不是判据的口径。）
  ——方括号里的两个词**是我猜的，语料里没有**。**这不是转写，是编造。**
  因此「主动相赠」这个读法**在本库里没有证据支撑**，本条只记「讣告这一句读不出来」。
- **自传（P1，`src-75240b0a8dbb`，他自己写的）**：他当时**不在场**；合伙人 Gaskell
  `as an act of courtesy he brought them my Scheme Book and allowed them to examine it`；
  Bourdon `took careful notes and sketches of the constructive details of the hammer`；
  而他本人 `was, therefore, **in entire ignorance** of the fact that these foreign visitors had
  taken with them to France a copy of the plan and details of my steam hammer`。

★ **真实经过**：1842 年 4 月他应法国海军部长 Bouchier 之邀访法，顺道到 Creusot；
Schneider 不在，Bourdon 接待。他注意到一根锻得异常精确的大型船机曲柄，问是怎么锻的——

> His immediate reply was, "**It was forged by your steam hammer!**"

而他自己的记述是：

> **Great was my surprise and pleasure at hearing this statement.**

★★ **不是愤怒，是「惊讶且高兴」。** 这一句是本人物性格上很硬的一处证据，**且是 P1**。

★★★ **本条对产物的意义**：`04-external.md` 那条独立性警告在这里拿到了最强的实例——
**讣告不只是复述自传，它在这一处改写了事实的性质**（意外 → 慷慨）。
**这正是 divergence-map 该收的东西。**

## Unknowns and source gaps

- **自传未读**——D4–D7 全部待核，K2 待成条，K1 的合并判断也等它。
- **1839–1842 那段 Creusot 经过**是本道最值钱也最悬的一条：**只有 S2，且涉第三方**。
- **没有任何他与人往来的书信可读**：Dibner 七封亲笔信 **OCR 完全不可用**
  （**有意不入库**，图像在，只能人工转录）。**「他怎么与人商定一件事」这一层本轮完全缺席。**

## Proposed Holdout cases

IDs only; research Agents must not inspect Holdout bodies.

## Handoff to adjudication

- Validate origin independence and evaluation-set separation before promotion.
- ★ **K1 达标可进断言层**，但要先与 writings C4、expression E2 做合并判断，**否则重复计数**。
- ★★ **K3 可直接进 divergence-map**：通行说法「1857 年退休后转向天文」与 P1 年份不符——
  **他退休前就在发表天文论文。**
- ★★★ **D6 是本人物最强的一条决策证据，但目前只有 S2 一处且涉第三方**。
  **在自传核到之前，产物里不得把它当成确定事实叙述。**
