...
- Phase: `research`
- Profile: `quick`
- Generated: `2026-08-11T05:44:14Z`
- Result: **FAIL**
...
```json
{
  ...
  "corpus_ceiling": {
    ...
    "有材料的道数": 2,
    "quick 要的一手份数": 4,
    "够得着吗": "够不着：六条道只占 2 < 3——**空着的道抓再多别的也补不上**"
  },
  "corpus_feasibility": {
    "profile": "quick",
    "可用材料总数": 14,
    ...
    "还差": null,
    "拦路的": [
      "道数 2 < min_lanes 3（['external', 'writings']）"
    ],
...
- `source.lane-coverage`: source metadata covers 2 lanes < profile minimum 3: ['writings', 'external']
- `research.source-unclaimed`: `src-bcf4065a0233` analysis_laws_england_1766_en.txt —— 声称 `William Blackstone` 所著，**既无 A-* 署名证据，也未在 attribution_basis 里被逐份点名**。
- `research.source-unclaimed`: `src-5854111768a9` commentaries_bk1_1898_en.txt —— 声称 `William Blackstone` 所著，**既无 A-* 署名证据，也未在 attribution_basis 里被逐份点名**。
...
- `research.source-unclaimed`: `src-f7528e759247` commentaries_bk3_1898_en.txt —— 声称 `William Blackstone` 所著，**既无 A-* 署名证据，也未在 attribution_basis 里被逐份点名**。
- `research.source-unclaimed`: `src-9525769f856e` commentaries_bk4_1898_en.txt —— 声称 `William Blackstone` 所著，**既无 A-* 署名证据，也未在 attribution_basis 里被逐份点名**。
...
- `research.source-unclaimed`: `src-1605220f591e` law_tracts_1762_en_vol1.txt —— 声称 `William Blackstone` 所著，**既无 A-* 署名证据，也未在 attribution_basis 里被逐份点名**。
- `research.source-unclaimed`: `src-435de4a6e51d` law_tracts_1762_en_vol2.txt —— 声称 `William Blackstone` 所著，**既无 A-* 署名证据，也未在 attribution_basis 里被逐份点名**。
...
- `research.source-unclaimed`: `src-9a5ff4e9a5e6` reports_westminster_1781_en_vol1.txt —— 声称 `William Blackstone` 所著，**既无 A-* 署名证据，也未在 attribution_basis 里被逐份点名**。
...
- `corpus.undeclared-duplicate-sources`: **1 对来源重叠 ≥0.3 而两边都没声明 `derived_from`**——台账上看不出它们是同一部作品。**清掉这条错的唯一办法是补 `derived_from`**——★ 本件只读 `derived_from`（`check_source_dedup.py` 第 182 行），**在 `counting_convention` 里写散文不会让它变绿**：那件判据当初正是因为「散文里写了、机器读得到的字段里没写」才建的。散文该写，但它是给人看的，不是给这道门看的。　[('EXT_interesting_appendix_p', 'reply_to_priestley_1773_en', 1.0)]
...
- `corpus.no-viable-holdout-split`: **扣任何一份当 holdout 都满足不了 profile 门**：道数 2 < min_lanes 3（['external', 'writings']） —— 差的是材料，不是文字
