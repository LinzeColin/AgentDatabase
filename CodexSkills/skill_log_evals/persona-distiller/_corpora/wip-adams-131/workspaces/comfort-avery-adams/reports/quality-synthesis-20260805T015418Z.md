...
- Phase: `synthesis`
- Profile: `quick`
- Generated: `2026-08-05T01:54:18Z`
- Result: **FAIL**
...
```json
{
  ...
  "fact_density": {
    ...
    "**未达**": [
      "可核 `fact` 断言 1 条 < 要求 15 条（72 条可用源 ÷ 5） —— **语料里可核的具体事实没有进入断言层**；这正是 Galen #101 真 delta −0.1259 的根因",
  ...
  "evidence_per_claim": {
    "断言条数": 5,
    "source_ids": "用得太少不判（非空 5/5，不同取值 5）",
    "evidence_clusters": "用得太少不判（非空 5/5，不同取值 5）",
    "counter_source_ids": "整批都空（非空 0/5，不同取值 0）"
  },
...
- `source.no-holdout`: no Holdout source exists; evaluation would be circular or ungrounded
