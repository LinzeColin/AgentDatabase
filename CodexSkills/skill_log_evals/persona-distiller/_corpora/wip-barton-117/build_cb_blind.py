...
# ── 改这三行即可 ──────────────────────────────────────────────
CAND = "cb_candidate.json"        # {case_id: 候选答案}
BASE = "cb_baseline.json"    # {case_id: 基线答案}
PREFIX = "cb"                     # 落盘文件名前缀
# ────────────────────────────────────────────────────────────
...
cases = {json.loads(l)["case_id"]: json.loads(l)["prompt"]
         for l in pathlib.Path("workspaces/clara-barton/clara-barton/evals/cases.jsonl").read_text(encoding="utf-8").splitlines()
         if l.strip()}
