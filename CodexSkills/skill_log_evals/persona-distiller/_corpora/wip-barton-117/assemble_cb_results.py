...
KEY = json.loads((pathlib.Path("round1") / "cb_blind_key.json").read_text(encoding="utf-8"))
...
for seat, fn in (("seat-D-score-v1", "cb_judge_D.json"), ("seat-E-strict-v1", "cb_judge_E.json")):
    f = SP / fn
    ...
    for cid, v in json.loads(f.read_text(encoding="utf-8")).items():
        ...
        c = a_raw if k["A"] == "candidate" else b_raw
        b = b_raw if k["A"] == "candidate" else a_raw
        rows.append({"case_id": cid, "seat": seat, "candidate": c, "baseline": b,
                     "suite": SUITE.get(cid), "note": note})
