...
★★ **这句话原本写的是「本道分到的 12 份里没有一份是护教／神学体裁」。**
那是一句**按体裁描述缺口**的话，而本人物的 holdout 恰好就是一部护教之作——
**等于把 holdout 的题目说了出来**，尽管它一个字面提及、一处内容重叠都没有。
`check_holdout_mention` 报绿是对的，它的文件头明写抓不到这一类；
**抓到它的是候选侧子代理的主动上报**。已改写成不描述缺什么体裁。
