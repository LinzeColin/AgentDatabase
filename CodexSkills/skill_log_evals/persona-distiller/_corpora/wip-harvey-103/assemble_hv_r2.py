...
WS = SP / "ws-harvey/william-harvey"


def main() -> int:
    key = json.loads((SP / "hv_blind_key.json").read_text(encoding="utf-8"))
    cases = {json.loads(l)["case_id"]: json.loads(l)
             for l in (WS / "evals/cases.jsonl").read_text(encoding="utf-8").splitlines() if l.strip()}
    seats = {}
    for seat, f in (("seat-D-score-v1", "hv_judge_D2.json"), ("seat-E-strict-v1", "hv_judge_E2.json")):
        p = SP / f
        if not p.is_file():
