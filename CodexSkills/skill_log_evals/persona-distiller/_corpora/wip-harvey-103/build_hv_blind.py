...
def main() -> int:
    cand = json.loads((SP / "hv_candidate.json").read_text(encoding="utf-8"))
    bare_p = SP / "hv_baseline_bare.json"
    if not bare_p.is_file():
        print("缺 hv_baseline_bare.json —— 裸模型对照臂未就绪，**不构造载荷**", file=sys.stderr)
        return 3
    bare = json.loads(bare_p.read_text(encoding="utf-8"))
    cases = {json.loads(l)["case_id"]: json.loads(l)
             for l in (SP / "ws-harvey/william-harvey/evals/cases.jsonl").read_text(encoding="utf-8").splitlines() if l.strip()}
    ...
    (SP / "hv_blind_payload.json").write_text(json.dumps(payload, ensure_ascii=False, indent=1), encoding="utf-8")
    (SP / "hv_blind_key.json").write_text(json.dumps(key, ensure_ascii=False, indent=1), encoding="utf-8")
