...
- Phase: `research`
- Profile: `quick`
- Generated: `2026-08-20T06:55:51Z`
- Result: **FAIL**
...
```json
{
  ...
  "fact_density": {
    ...
    "**未达**": [
      "可核 `fact` 断言 0 条 < 要求 5 条（10 条可用源 ÷ 5） —— **语料里可核的具体事实没有进入断言层**；这正是 Galen #101 真 delta −0.1259 的根因"
    ]
  },
  "quote_layer": {
    "已扫文件": 0,
  ...
  "longs_corruption": {
    ...
    "逐份": {
      ...
      "src-659c3ee6ac1a": {
        ...
        "rate": 0.0,
        "reason": "英文讹字率 0.0000（正形 2330／讹形 0）",
        "file": "in.ernet.dli.2015.53284.txt"
      },
      "src-988d9ca66526": {
        "words": 317737,
        "diagnostic_est_eft": [
  ...
  "content_review": {
    ...
    "staged_not_ingested": "⚠ **未核，不是通过** —— `check_staged_but_not_ingested` 的明细里没有 `wip-telford-37`（本人物可能压根没走过抓源台账）",
  ...
  "material_split": {
    ...
    "**holdout 泄漏处**": 6,
    "**逐条**": [
      "✗✗ holdout 正文出现在此：src-0fd7902a2175  ← 隔离失效，本轮 known 分数不可信",
      "✗✗ holdout 正文出现在此：src-1d53e600828d  ← 隔离失效，本轮 known 分数不可信",
      "✗✗ holdout 正文出现在此：src-2bdb30fc96f0  ← 隔离失效，本轮 known 分数不可信",
      "✗✗ holdout 正文出现在此：src-0fd7902a2175  ← 隔离失效，本轮 known 分数不可信",
      "✗✗ holdout 正文出现在此：src-1d53e600828d  ← 隔离失效，本轮 known 分数不可信",
      "✗✗ holdout 正文出现在此：src-2bdb30fc96f0  ← 隔离失效，本轮 known 分数不可信"
    ],
  ...
  "filename_year_vs_ledger": {
    ...
    "**逐条**": [
      {
        "source_id": "src-659c3ee6ac1a",
        "文件名": "in.ernet.dli.2015.53284.txt",
        "文件名里的年份": [
          2015
        ],
...
- `research.authorship-unproven`: src-01d9132416de b30448645.txt｜文中他人署名：无 —— 账本声称本人所著，但文中查无归属证据（署名／编者注／逐字稿轮次三者皆无）
- `research.authorship-unproven`: src-0fd7902a2175 t-088-1830-telford.-bridge-edinburgh-encyclopaedia.txt｜文中他人署名：无 —— 账本声称本人所著，但文中查无归属证据（署名／编者注／逐字稿轮次三者皆无）
- corpus.holdout-leak: 6 处（隔离失效）
