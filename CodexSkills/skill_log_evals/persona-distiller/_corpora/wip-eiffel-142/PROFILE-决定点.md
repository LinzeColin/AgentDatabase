...
### ★★★ 2026-08-18：三条里的第 1、第 3 条**已用真法文正文实测**（拿今天取到的 1910 那部）

**第 3 条（逐字引文判据在法文上没跑过）—— 跑了，正常。**
造夹具：同一段法文原文，三种写法进研究道 ——
① 中文引号「…」逐字照抄　② 法文书名号 «…» 逐字照抄　③ 中文引号但**把一个 `é` 改成 `e`**。
实测：`引文 3 条`（三种形式**都抽得出**，包括法文书名号），
**未命中恰好 1 条 —— 正是改过的那条**，rc=1。
⇒ **法文可用，且对单个重音符的改动都报得出来**（这正是「改了 OCR 讹字再当逐字引文」要防的）。

**第 1 条（`check_first_person_density` 的语种关）—— 跑了，判据正确地拒绝出数。**
实测：374,811 字符，`可读语种（英文）0／不可读 374,811 ——**可读占比 0%**`，
输出 `**本判据不适用**`，`实质第一人称句` 与 `密度` 均为 **`null` 而不是 `0`**。
⇒ **没有假绿**：它没给出一个会被读成「声口薄」的数（这正是德语那次 25–180 倍的教训修好的地方）。

**★★★ 但由此得出一个必须在抓源前知道的后果：**
> **Eiffel 的「声口」这一项，在法文语料上量不出来 —— 是「未量」，不是「够」也不是「不够」。**

而声口正是 `worth_starting` 点名的三个**开工后检查点**之一
（另两个：一手源占比、min_lanes），Coffin #130／Bain #136／Sellers #154 都是栽在这一项上的。
⇒ 抓源之后**不要指望这道判据给 Eiffel 一个声口读数**；
  要么换一条能读法文的量法（本轮**没有**建，不为凑数造尺子），
  要么在判决书里如实写「声口未量：判据只认英文锚点」。

**第 2 条（`check_authorship` 的法文 `par` 前缀）** 本轮未单独验 —— 记录说 v0.0.0.157 已加（Martens 那次），
**留到落盘后连真数据一起验**（没有真署名行，空跑没意义）。
