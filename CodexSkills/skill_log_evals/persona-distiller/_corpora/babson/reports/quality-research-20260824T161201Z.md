...
- Phase: `research`
- Profile: `quick`
- Generated: `2026-08-24T16:12:01Z`
- Result: **FAIL**
...
```json
{
  ...
  "fact_density": {
    ...
    "**未达**": [
      "可核 `fact` 断言 0 条 < 要求 5 条（11 条可用源 ÷ 5） —— **语料里可核的具体事实没有进入断言层**；这正是 Galen #101 真 delta −0.1259 的根因"
    ]
  },
  "quote_layer": {
    "已扫文件": 0,
  ...
  "content_review": {
    ...
    "fraktur_mojibake": "⚠ **德文语料 0 份 —— 未核，不是通过**（「没有花体乱码」在空集上恒真；共读到 11 份）",
  ...
  "corpus_feasibility": {
    ...
    "还差": null,
    "拦路的": [
      "道数 2 < min_lanes 3（['expression', 'writings']）"
    ],
...
- `source.lane-coverage`: source metadata covers 2 lanes < profile minimum 3: ['writings', 'expression']
- `research.lane-completion`: completed source-linked lanes 0 < profile minimum 3: []

## Warnings

- `corpus.no-viable-holdout-split`: **扣任何一份当 holdout 都满足不了 profile 门**：道数 2 < min_lanes 3（['expression', 'writings']） —— 差的是材料，不是文字
