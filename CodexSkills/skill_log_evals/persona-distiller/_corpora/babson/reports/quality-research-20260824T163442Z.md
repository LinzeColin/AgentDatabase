...
- Phase: `research`
- Profile: `quick`
- Generated: `2026-08-24T16:34:42Z`
- Result: **FAIL**
...
- `corpus.title-is-just-the-filename`: **3/17 行的 `title` 就是文件名**（18%）——这个字段没有承载信息。后果不是难看：判「两份是不是同一部作品」时**除了内容重叠没有第二个证据源**，引文坐标与「挂到哪部作品」也全落在文件名上。★ 与空值不同——**空值至少诚实，填成文件名的字段看起来是填过的**。
