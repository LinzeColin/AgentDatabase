...
- Phase: `research`
- Profile: `quick`
- Generated: `2026-08-21T01:04:57Z`
- Result: **FAIL**
...
```json
{
  ...
  "ocr_homoglyphs": {
    ...
    "**这些是 OCR 件，取引文时避开脏位置**": [
      {
        "源": "in.ernet.dli.2015.205779.txt",
        "非拉丁字符": 22,
        "全同形字词": 11,
        "样例": [
          "а 读作 a",
    ...
    "逐份": {
      ...
      "src-8b6e75b33fbc": {
        ...
        "逐语域": {
          "英文": {
            "语域": "英文",
            "anchors_per_10k": 2152.4,
            "panel_good": 1366,
            "panel_bad": 0,
            "若无语种门会读到": 0.0,
            "verdict": "干净",
            "rate": 0.0,
            "reason": "英文讹字率 0.0000（正形 1366／讹形 0）"
          }
        },
        "verdict": "干净",
        "rate": 0.0,
        "reason": "英文讹字率 0.0000（正形 1366／讹形 0）",
        "file": "in.ernet.dli.2015.205779.txt"
      },
      "src-f94b66599392": {
        "words": 140181,
        "diagnostic_est_eft": [
  ...
  "corpus_feasibility": {
    ...
    "还差": null,
    "拦路的": [
      "道数 2 < min_lanes 3（['decisions', 'writings']）"
    ],
  ...
  "holdout_overlap": {
    ...
    "其中·无法判定": 0,
    "**逐条**": [
      "✗ src-fac3e5a319e5  in.gov.ignca.20600.txt",
      "✗ src-fac3e5a319e5 **最长连续逐字段 215 词**（≥50 词的段共 849 处）",
      "✗ **非样板的连续逐字段 849 处 ≥ 50（最长 215 词）**"
    ],
...
- `source.lane-coverage`: source metadata covers 2 lanes < profile minimum 3: ['writings', 'decisions']
- `research.authorship-unproven`: src-24261c5726ef jstor-1762549.txt｜文中他人署名：无 —— 账本声称本人所著，但文中查无归属证据（署名／编者注／逐字稿轮次三者皆无）
- `research.namesake-unseparable`: 同名者「Robert Abbott Hadfield」与目标姓名分不开，且未被 namesake-criteria.json 的 excluded_names 覆盖
- `corpus.undeclared-duplicate-sources`: **1 对来源重叠 ≥0.3 而两边都没声明 `derived_from`**——台账上看不出它们是同一部作品。**清掉这条错有两条路**：① 补 `derived_from`（同一部作品的另一份扫本）；② 在 `meta.json` 的 `attribution_basis.counting_convention` 里**逐对点名**说明为什么它们该当两处证据（2026-08-17 起这条路真的通了——在那之前它只是句话，代码从没读过 `counting_convention`）——★★ 2026-08-17 订正：上面这几句原本写的是「本件只读 `derived_from`，在 `counting_convention` 里写散文不会让它变绿」——**那句当时是对的**，因为那条出路从没被实现。今天已实现，但**只认逐对点名**：约定文本里要同时出现这一对的两个文件名才算，泛泛的散文仍然不豁免任何一对。　[('in.ernet.dli.2015.205779.t', 'in.ernet.dli.2015.70291.tx', 0.7848)]
- `corpus.source-count-inflated-by-duplicates`: **`source.minimum` 只是被重份撑绿的**：可用来源 8 ≥ 门 8，而按内容去重后只有 **7 部作品**（虚高 1.143×）。**这道门量的是「有几个 source_id」，不是「有几处独立证据」。**
- `research.lane-completion`: completed source-linked lanes 0 < profile minimum 3: []
- `corpus.holdout-overlap`: holdout 与 train 有内容重合（3 条**真重合**，另有 0 条无法判定另计）——**现在换源还来得及**；逐条见 metrics.holdout_overlap，受污染段清单见 reports/holdout-contaminated-passages.json

## Warnings

- `corpus.no-viable-holdout-split`: **扣任何一份当 holdout 都满足不了 profile 门**：道数 2 < min_lanes 3（['decisions', 'writings']） —— 差的是材料，不是文字
