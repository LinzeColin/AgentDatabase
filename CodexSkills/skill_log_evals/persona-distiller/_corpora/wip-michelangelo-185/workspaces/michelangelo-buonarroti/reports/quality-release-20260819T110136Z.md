...
- Phase: `release`
- Profile: `quick`
- Generated: `2026-08-19T11:01:36Z`
- Result: **FAIL**
...
```json
{
  ...
  "translation_witness": {
    ...
    "明细": [
      ...
      "✗ clm-2a3b37136bc9（mental-model）把同一部作品的 2 份见证当成了 2 处来源：['src-34bb6d56038a', 'src-6094206729a1']\n    → **它们是同一部作品的不同译本／版本，只算一处。**「≥2 处独立证据」实际未达成。"
    ],
  ...
  "evidence_per_claim": {
    "断言条数": 27,
    "source_ids": "逐条各异（非空 27/27，不同取值 17）",
    "evidence_clusters": "逐条各异（非空 27/27，不同取值 16）",
  ...
  "claim_coverage_checked": "实际检查 26/27 条（其中按引文判据 0 条；语料元断言 0、无实体无引文 1 未纳入）",
  "baseline_provenance": {
    "baseline_rows": 32,
    "by_source": {
      "unknown": 32
...
- `ledger.invalid`: clm-fix-03 has invalid category
- `ledger.invalid`: clm-fix-04 has invalid category
- `ledger.invalid`: clm-fix-05 has invalid category
...
- `claim.parallel-witness-collapse`: ✗ clm-2a3b37136bc9（mental-model）把同一部作品的 2 份见证当成了 2 处来源：['src-34bb6d56038a', 'src-6094206729a1']     → **它们是同一部作品的不同译本／版本，只算一处。**「≥2 处独立证据」实际未达成。
...
- `claim.insufficient-support`: clm-fix-01 needs at least two supporting sources
- `claim.non-independent`: clm-fix-01 needs at least two independent evidence clusters
- `claim.insufficient-support`: clm-fix-02 needs at least two supporting sources
- `claim.non-independent`: clm-fix-02 needs at least two independent evidence clusters
- `claim.orphan`: active Claim clm-fix-01 is not rendered in any core artifact
- `claim.orphan`: active Claim clm-fix-02 is not rendered in any core artifact
