...
- Phase: `release`
- Profile: `quick`
- Generated: `2026-08-19T15:07:44Z`
- Result: **FAIL**
...
```json
{
  ...
  "measurement_claims": {
    ...
    "诚实弃权（不计问题）": 0,
    "**逐条**": [
      "judge_payload.v1.json/case-style-decoy-2　「实测」：至于你问的现代机器，其钢材、阀门、造价，我皆未亲手实测，超出我经验范围，故只能承诺方法，不能承认厂牌。先把每马力小时汽耗的数据送来，我按老规矩替你逐项算账。",
      "judge_payload.v1.json/case-planning-fidelity-1　「实测」：先下结论：从模型到工业引擎，不可按比例直线放大——先量化偏差，再改原理，最后以实测煤耗验收。"
    ],
  ...
  "eval_results": 136,
  "candidate_overall": 0.8959,
  "baseline_overall": 0.7446,
  "candidate_baseline_delta": 0.1513,
  "suite_candidate_means": {
    ...
    "trajectory": 0.9217,
    "contrast": 0.89,
    "fact-preservation": 0.945,
    "style-decoy": 0.915,
    "task-completion": 0.85,
    "planning-fidelity": 0.905,
    "tool-use": 0.8933,
    "capability-calibration": 0.895,
    "refusal-stop": 0.94,
    "long-horizon": 0.92,
    "identity-routing": 0.81,
...
- `eval.surface-leak`: **表面特征会指出哪一侧是候选**，这一轮的盲判不成立：**候选更短的题只有 7/32 = 22%，要 ≥25%**——长度会变成指认候选的信号
- `content.self-count-wrong`: **自报字数与实数对不上**——主动邀请核对的数字自己错了，比不给更伤。　⚠ case-known-2: 自称「字数373字」，实数 355（含标点 394／不含 355）：「答案如下：；⚠ case-known-2: 自称「字数312字」，实数 355（含标点 394／不含 355）：「答案如下：；⚠ case-voice-1: 自称「字数367字」，实数 334（含标点 369／不含 334）：「结论先说：分离冷凝器之所以省煤，是因为它消除了纽科门机每冲程"先冷却」
...
- `eval.baseline-not-capability-evidence`: 68/68 条基线不可作能力证据（{'unknown': 68}）——**此产物的 delta 不得用于支持「比裸模型强」这类结论**；它只说明产物比该对照写得好
