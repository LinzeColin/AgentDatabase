...
cand = json.loads(pathlib.Path("rk_candidate.json").read_text(encoding="utf-8"))
base = json.loads(pathlib.Path("rk_baseline_bare.json").read_text(encoding="utf-8"))
...
(OUT / "rk_blind_payload.json").write_text(json.dumps(payload, ensure_ascii=False, indent=1), encoding="utf-8")
(OUT / "rk_blind_key.json").write_text(json.dumps(key, ensure_ascii=False, indent=1), encoding="utf-8")
r1 = pathlib.Path("round1/rk_blind_key.json")
