...
from wbi_market.common import (  # noqa: E402
    MarketLabError,
    ValidationError,
    iter_jsonl,
    object_sha256,
...
from wbi_market.integrity import load_and_verify_tree, seal_tree  # noqa: E402
from wbi_market.metrics import (  # noqa: E402
    aggregate_evidence,
    build_next_iteration_plan,
    decide_gate,
)
from wbi_market.privacy import anonymize_feedback_file  # noqa: E402
from wbi_market.specs import (  # noqa: E402
    assert_valid,
...
from wbi_market.stress import STRESS_CATEGORIES, expand_to_jsonl  # noqa: E402
...
from wbi_market.quality import evidence_chain_digest, quality_audit  # noqa: E402
from wbi_market.quality import quality_audit, evidence_chain_digest  # noqa: E402


SKILL_ROOT = SCRIPT_DIR.parent
DEFAULT_TEMPLATE = SKILL_ROOT / "assets" / "market" / "templates" / "experiment_spec.json"
...
def doctor_skill(skill_root: Path) -> Dict[str, Any]:
    """Validate the embedded kernel without redefining Teleiosis packaging rules."""
    root = skill_root.resolve()
    errors: List[str] = []
    warnings: List[str] = []
    skill_md = root / "SKILL.md"
    if not skill_md.exists():
        errors.append("缺少 Teleiosis 根级 SKILL.md")
    else:
        try:
            frontmatter = _parse_frontmatter(skill_md)
            if frontmatter.get("name") != "teleiosis":
                errors.append("市场内核只能嵌入 name=teleiosis 的 Skill")
            if frontmatter.get("metadata") is not None:
                warnings.append("轻量解析器不展开嵌套 metadata；版本由 VERSION 与完整测试校验")
    ...
    required = [
        root / "VERSION",
        root / "scripts" / "wbi_market.py",
        root / "scripts" / "teleiosis_cycle.py",
        root / "assets" / "market" / "templates" / "experiment_spec.json",
        root / "references" / "market" / "ARCHITECTURE.md",
        root / "scripts" / "wbi_market" / "quality.py",
    ]
    for path in required:
        if not path.exists():
            errors.append(f"缺少必要文件: {path.relative_to(root)}")
    if (root / "VERSION").exists() and (root / "VERSION").read_text(encoding="utf-8").strip() != "v0.0.0.2":
        errors.append("VERSION 必须为统一版本 v0.0.0.2")
    forbidden_standalone = [root / "scripts" / "market_lab.py", root / "scripts" / "marketlab"]
    for path in forbidden_standalone:
        if path.exists():
            errors.append(f"发现已废止的独立 Market Lab 控制面: {path.relative_to(root)}")
    ...
    if file_count > 700:
        errors.append(f"文件数超过嵌入式上限 700: {file_count}")
    return {
        "valid": not errors,
        "skill_root": str(root),
        "embedded_kernel": "wbi_market",
...
def cmd_gate(args: argparse.Namespace) -> None:
    ...
    quality = read_json(Path(args.quality_audit))
    gate = decide_gate(spec, summary, quality)
...
def _validated_rows(path: Path, validator, context: str, arm_ids: Sequence[str] | None = None) -> List[Dict[str, Any]]:
...
def cmd_quality_audit(args: argparse.Namespace) -> None:
    spec = _load_spec(Path(args.spec))
    arm_ids = [arm["id"] for arm in spec["arms"]]
    tasks = _validated_rows(Path(args.tasks), validate_task, "任务集")
    assignments = [strip_internal_fields(row) for row in iter_jsonl(Path(args.assignments))]
    results = _validated_rows(Path(args.results), validate_result, "结果集", arm_ids)
    feedback = (
        _validated_rows(Path(args.feedback), validate_feedback, "市场反馈", arm_ids)
        if args.feedback
        else []
    )
    calibration = [strip_internal_fields(row) for row in iter_jsonl(Path(args.calibration))] if args.calibration else []
    blind_map = read_json(Path(args.blind_map)) if args.blind_map else None
    ...
    _print_json(
        {
            ...
            "output": args.output,
            "blocking_reports": result["blocking_reports"],
            "reheat_reports": result["reheat_reports"],
            "quality_audit_digest": result["quality_audit_digest"],
        }
    )
    if result["status"] != "PASS":
        raise ValidationError("冻结质量审计未通过")


def cmd_evidence_chain(args: argparse.Namespace) -> None:
    payload = read_json(Path(args.input))
    result = evidence_chain_digest(payload)
    write_json(Path(args.output), result)
    _print_json(result)
    if not result["valid"]:
        raise ValidationError("证据链缺少必需哈希")
...
def cmd_assurance_check(args: argparse.Namespace) -> None:
    record = read_json(Path(args.input))
    result = evaluate_assurance(record, as_of=args.as_of)
    write_json(Path(args.output), result)
    _print_json({"status": result["status"], "output": args.output, "assurance_digest": result["assurance_digest"]})
    if result["status"] != "PASS":
        raise ValidationError("Assurance 硬门未通过")


def cmd_canary_check(args: argparse.Namespace) -> None:
    contract = read_json(Path(args.contract))
    observations = [strip_internal_fields(row) for row in iter_jsonl(Path(args.observations))]
    result = evaluate_sequential_canary(contract, observations)
    write_json(Path(args.output), result)
    _print_json({"decision": result["decision"], "output": args.output})


def cmd_cluster_aggregate(args: argparse.Namespace) -> None:
    rows = [strip_internal_fields(row) for row in iter_jsonl(Path(args.results))]
    result = aggregate_cluster_effects(
        rows,
        candidate_arm=args.candidate_arm,
        comparator_arm=args.comparator_arm,
        seed=args.seed,
        bootstrap_samples=args.bootstrap_samples,
    )
    write_json(Path(args.output), result)
    _print_json({"output": args.output, "paired_clusters": result["paired_clusters"], "result_digest": result["result_digest"]})
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="wbi_market.py",
        description="Teleiosis 市场实证内核：模拟、因果实验、压力、大数据和真实市场证据；不拥有最终晋级权。",
    )
    ...
    item = sub.add_parser("quality-audit", help="冻结污染、分配、SRM、环境、功效、评委、时效与引用完整性审计")
    item.add_argument("--spec", required=True)
    item.add_argument("--tasks", required=True)
    item.add_argument("--assignments", required=True)
    item.add_argument("--results", required=True)
    item.add_argument("--feedback")
    item.add_argument("--calibration")
    item.add_argument("--blind-map")
    item.add_argument("--assignment-mode", choices=["paired", "exclusive"], default="paired")
    item.add_argument("--output", required=True)
    item.set_defaults(func=cmd_quality_audit)

    item = sub.add_parser("gate", help="依据冻结 Gate 输出证据就绪/KEEP/REVERT/REHEAT/BLOCKED；最终晋级仅由 Teleiosis 决定")
    item.add_argument("--spec", required=True)
    item.add_argument("--summary", required=True)
    item.add_argument("--quality-audit", required=True)
    item.add_argument("--output", required=True)
    item.set_defaults(func=cmd_gate)

    item = sub.add_parser("evidence-chain", help="把 Subject、Spec、Dataset、Assignment、Result、Quality、Summary 与 Gate 哈希绑定为单一证据链")
    item.add_argument("--input", required=True)
    item.add_argument("--output", required=True)
    item.set_defaults(func=cmd_evidence_chain)
    ...
    item = sub.add_parser("assurance-check", help="验证 trace、评委校准、Provider 时效、污染、Canary 与回炉合同")
    item.add_argument("--input", required=True)
    item.add_argument("--output", required=True)
    item.add_argument("--as-of")
    item.set_defaults(func=cmd_assurance_check)

    item = sub.add_parser("canary-check", help="依据运行前冻结规则做顺序 Canary 停止判断")
    item.add_argument("--contract", required=True)
    item.add_argument("--observations", required=True)
    item.add_argument("--output", required=True)
    item.set_defaults(func=cmd_canary_check)

    item = sub.add_parser("cluster-aggregate", help="先聚合重复试验，再以任务簇为推断单位计算配对差异")
    item.add_argument("--results", required=True)
    item.add_argument("--candidate-arm", required=True)
    item.add_argument("--comparator-arm", required=True)
    item.add_argument("--output", required=True)
    item.add_argument("--seed", type=int, default=20260729)
    item.add_argument("--bootstrap-samples", type=int, default=2000)
    item.set_defaults(func=cmd_cluster_aggregate)
