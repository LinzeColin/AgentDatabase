...
Teleiosis 市场实证内核 是一个**证据控制面**，不是模型供应商、在线遥测 SaaS、自动发布机器人或自我批准系统。它把外部执行器产生的观察统一成可复核证据，再依据预先冻结的合同产生证据就绪、保留、回退或回炉建议；正式晋级只由 Teleiosis 决定。
...
└─────────┬─────────┘  └──────────┬──────────┘  └──────────┬──────────┘
          └───────────────────────┼────────────────────────┘
                                  │
                     ┌────────────▼────────────┐
                     │ Evidence Aggregate/Gates │
                     │ paired delta / CI / cost│
                     └────────────┬────────────┘
                                  │
              ┌───────────────────┼────────────────────┐
              │                   │                    │
         EVIDENCE_READY_FOR_TELEIOSIS          KEEP / REVERT       REHEAT / BLOCKED
              │                   │                    │
              └───────────────────▼────────────────────┘
