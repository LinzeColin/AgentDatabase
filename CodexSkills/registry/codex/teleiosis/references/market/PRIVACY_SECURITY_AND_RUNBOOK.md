...
```bash
python3 scripts/wbi_market.py doctor --skill-root .
python3 scripts/wbi_market.py validate-spec --spec /path/experiment.json
python3 scripts/wbi_market.py validate-competitors --registry /path/competitors.json
python3 scripts/wbi_market.py validate-tasks --tasks /path/tasks.jsonl
```

### 6.2 生成与执行准备

```bash
python3 scripts/wbi_market.py expand-stress --input base.jsonl --output stress.jsonl --categories all
python3 scripts/wbi_market.py make-assignments --spec experiment.json --tasks tasks.jsonl \
...
```bash
python3 scripts/wbi_market.py aggregate --spec experiment.json --results results.jsonl \
...
python3 scripts/wbi_market.py quality-audit --spec experiment.json --tasks tasks.jsonl \
  ...
  --output reports/QUALITY_AUDIT.json
python3 scripts/wbi_market.py gate --spec experiment.json --summary reports/SUMMARY.json \
  --quality-audit reports/QUALITY_AUDIT.json --output reports/GATE.json
python3 scripts/wbi_market.py plan-next --spec experiment.json --summary reports/SUMMARY.json \
...
```bash
python3 scripts/wbi_market.py seal --path /path/run --manifest /path/run/TREE_MANIFEST.json
python3 scripts/wbi_market.py verify-seal --path /path/run --manifest /path/run/TREE_MANIFEST.json
```
