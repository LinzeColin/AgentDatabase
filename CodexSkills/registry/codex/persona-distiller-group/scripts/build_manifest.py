...
def included(path: Path) -> bool:
    ...
    if any(part in EXCLUDED_DIRS for part in relative.parts):
        return False
    # ★★ **与上游 persona-distiller 的唯一实质差异，必须写清楚。**
    #   上游排除 `.zip`（那里 zip 是构建产物，算校验和没意义）。
    #   **本 skill 里 zip 就是产物本体** —— 每个人物目录是
    #   `registration.json` + `team-card.json` + `versions/<v>/*-delivery-v<v>.zip`，
    #   运行时加载的正是那个 zip（实测 108 个、20MB）。
    #   照抄上游会让校验面 286/396，**恰好把最该校验的 108 件排在外面** ——
    #   一台按「已校验」报绿而根本没看过产物的机器。
