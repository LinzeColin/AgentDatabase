...
DOMAIN_SIGNALS: dict[str, tuple[str, ...]] = {
    "software-ai": (
        ...
        # ★★★ 2026-08-17：`仓库` 原样在这里，指的是 **git 仓库** ——
        #   而中文 `仓库` 也是 **warehouse**。实测：基准题
        #   `warehouse-automation-roi`（「仓库要不要上自动分拣…」，相关族是
        #   建造采购师/投资资本师/创业经营师，**没有软件开发师**）因此被判成软件题，
        #   域从 2 个变 3 个 ⇒ domain_match 分母变大，人人被稀释。
        #   该题正是基准里最差的几道之一（−11.1%，低于随机）。
        #   `仓库` 在 24 道题里**只命中这一道**，且去掉后该题 software-ai 命中归 0
        #   ⇒ **没有任何真软件题靠它**。改成不含歧义的写法。
        #   [[regex-must-clear-the-corpus-language]]（同音异义，不是语种问题）
        "日志", "监控", "告警", "bug", "缺陷", "编译", "构建", "代码仓库", "repo",
        "repository", "分支", "评审",
