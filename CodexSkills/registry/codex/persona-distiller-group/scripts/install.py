...
SKILL_NAME = Path(__file__).resolve().parents[1].name   # ★ 由自身位置推出，不写死
...
def run_post_install_check(dest: Path) -> dict[str, object]:
    completed = subprocess.run(
        # ★ 上游跑 `self_check.py --skip-tests`（那边整套测试串行 333 秒，会被
        #   下面的 timeout=120 杀掉）。**本 skill 不同**：它没有 self_check.py，
        #   但有自己的门 `run_tests.py` —— 6 件、实测 **11.1 秒**，稳在 120 秒内，
        #   而且那边默认就是门（有红即 rc≠0）。⇒ 装完直接跑真门，不跳过。
        #   ★ 若该文件缺失，下面按**失败**报（不是「跳过」也不是「通过」）——
        #   空默认值不许被读成没问题。[[empty-default-swallows-unknown]]
        [sys.executable, str(dest / 'scripts' / 'run_tests.py')],
