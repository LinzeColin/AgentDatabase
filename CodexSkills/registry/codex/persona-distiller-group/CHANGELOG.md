...
**`backfill_distilled_with --apply` 会把 97 份记录一律盖成 v0.0.0.154 —— 而那几乎必然是错的。**

继续「真的去用一次」：跑 `backfill_distilled_with.py`（它默认 dry-run，设计是对的）。
它说「需更新 **97** 份，已是最新 5 份」，全部归到 `v0.0.0.154 / git-first-commit`。

**停下来问了一句**：97 个人是几周里陆续蒸的，**第一次提交时的版本不可能全是同一个**。
于是把 102 份逐一查了 git：

    「首次落盘提交」的分布：**bfe16379a 一个提交，102 / 102 = 100%**，日期全是 2026-08-14

那不是「它们同一天被蒸出来」，是**整棵树那天才进的 git**（本仓有两个根提交）。
⇒ **归因方法本身是对的，但在这棵树上它什么也看不见。**

**⇒ 一个统一而错误的出处，比没有出处更坏** —— 后者是「不知道」，前者是「知道错了」，
而且还配着一个听起来很权威的 `source: git-first-commit`。

**改法（不硬拒，「不许因为过不了门而卡住流程」）：**

- `run()` 改成**两趟**：先只算并统计归因分布，确认不退化之后再写
  （一趟边算边写的话，退化检查只能在写了一半之后才做得出来）。
- 分布**每次都印**（dry-run 也印）—— 读 dry-run 的人正是要据此决定要不要写。
- 同一提交覆盖 ≥90% ⇒ `--apply` **拦下**（rc=2，一个字节不写），要 `--anyway '<理由>'`；
  理由会**落进输出**，不只留在终端里。

**★★ 顺手修掉一个产品的真 bug**（是测试炸出来的，不是我读出来的）：
`attribute()` 里 `record_path.relative_to(repo)` —— 而
`git rev-parse --show-toplevel` 返回的是**已解析软链**的路径（macOS 的 `/var → /private/var`），
`record_path` 常是未解析的 ⇒ **直接抛 ValueError**。
任何路径上有软链的用户都会踩到。两边都先 `realpath`。

**守卫**：`tests/test_backfill_refuses_degenerate_attribution.py`（4 项，造真 git 树）——
含 **★★★「被拦下时盘上一个字节不许动」**（逐文件比对字节）与
**★★ 正对照「每次提交只加 1 个人 ⇒ 归因不退化 ⇒ --apply 必须照常写盘」**
（否则这道守卫会把所有人都拦住，那就成了另一个毛病）。

**没有跑过 `--apply`。** 97 份记录一个字节没动（已用 `git status` 核实）。
