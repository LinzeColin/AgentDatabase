...
v0.0.0.41 的台账里我写了「`swarm` 档**没跑**……外推不是测量，本件不写那个数」。
**能补就补**，于是补了：

    swarm  30 人  route-plan 318 KB｜dossier 1786 KB｜contract **789 KB**
                  每位专家 **26.3 KB**｜token 区间 **202／323／539 K**

⇒ 与 `deep_team`（28 人 / 776 KB / 27.7 KB 每人）几乎重合 —— **线性模型成立**，
`swarm` 并不额外恶化。

★★ **那道题是我造的，必须一起说**：把 `PARALLEL` 词表（14 个词）塞满、再填到 905 字，
`parallel` 才到 0.860 越过 0.72 的门。**自然任务里 `swarm` 一次都没被选中**
（72 道 oracle 全量 0 次；连 `expected_mode` 就是 swarm 的那道 oracle 也只命中 14 个词里的 3 个）。
⇒ 这一行**只证明链条跑得通与体量**，**不证明用户会遇到 swarm**。
SKILL.md 那一行按这个口径写。
...
**那 776 KB 里没有免费的午餐 —— 拆开看完才有资格说「要不要瘦身」。**

上一版报了体量，**没有回答「能不能砍」**。砍之前先问：**里面有重复吗？**

    execution_units 的 97.2% 是 `capsules`
    33 段里有 **29 种不同内容**；唯一的重复是 **5 段空的控制面角色（0 KB）**
    ⇒ **28 个人 × 每人 22.6 KB**，逐段互不相同

单人 22.6 KB 的构成：

    method_capsule       8.6 KB  37.9%
    evidence_capsule     5.1 KB  22.5%
    failure_capsule      4.3 KB  18.8%
    work_capsule         2.1 KB   9.5%
    boundary_capsule     1.9 KB   8.6%
    voice_capsule        0.4 KB   1.7%
    currentness_capsule  0.2 KB   0.9%

**⇒ 776 KB 不是重复堆出来的。要瘦身就得少给每个人材料 —— 那是质量取舍，
不是免费的清理。** 已开 Task #136，三个选项各带价码。

★ 这一步是**在自己想动手之前**做的：上一版写完体量，我的第一反应是「砍 capsules」。
  **先量有没有重复**，量完发现重复是 0 KB ⇒ **原来的第一反应会直接降低产物质量**。
  （与今天早些时候「静态形状预测 11 个、实测 1 个」同一族：**动手前先量一次。**）

SKILL.md 那一行补上这段。**产品一个字没改。**

## v0.0.0.41

**整条链走了一遍（此前一直只验单步）：`deep_team` 的执行合同是 776 KB。**

    档位            人数   route-plan   dossier    contract   execution_units 占比
...
★ **token 数不是量出来的**：字节是实测，除数是我选的。
  776 KB ÷ (4.0／2.5／1.5 字节每 token) ⇒ **199／318／530 K token**，区间 **199–530 K**。
  本件**印区间不印单值**，并明说除数是选的。

★★ 与档位阶梯并排读（v0.0.0.35／v0.0.0.39）：**32 个无意义词就能拿到 deep_team**
⇒ **一个稍微长一点的请求，就能产出宿主可能装不下的合同。**

★★★ 判据**不判「太大了」** —— 多大算大取决于宿主，不是这个包能定的
（那种门要么恒绿要么恒红，两种都不产生信息）。只判
「**每位专家占的字节不许比基线更胖**」，地板 29.0 KB 贴着实测 27.7。

**发表前修掉自己两个错：**
- `98%` 那行**没走 `%` 格式化**，写成 `%%` 会原样印给用户（已改，并落成一条测试断言
  `assertNotIn("%%", stdout)`）。
- 「98% 是 execution_units」是我在 **small_team** 上量的；`deep_team` 实测 **99.4%**。
  已改成**现算**，自测加一条「占比必须现算且各档不同」（现测 88.1/98.0/99.4）。

新增 `scripts/check_execution_contract_fits_a_context.py`（8 项自测，整条链现跑约 9 秒）
＋ 调用方 `tests/test_execution_contract_fits_a_context.py`（6 项，含
「字节与 token 必须分开印且 token 给区间」「三步都要出现在源码里，否则就不是整条链」）。
SKILL.md「已测量的边界」表新增「**宿主要装进上下文的那份合同有多大**」一行。

**产品一个字没改。**

## v0.0.0.40

**把 v0.0.0.36 那件事换成用户的单位再量一遍：最终坐进队伍的人里，20.2% 靠任务无关通道进来。**

v0.0.0.36 量的是**候选池**（过准入的人里 62.8%／最坏 97.6%）。
用户看不到候选池，用户看到的是**最后那几个人**。所以重量一次：

    12 个独立题面、auto 档、共 173 个席位
    唯一靠 `packet_similarity` 过准入的：**35 / 173 = 20.2%**

**而且分布不是平均的：**

    9 支 small_team（10–12 人）  ⇒ 每支 4–5 人，占 **36–45%**
    3 支 deep_team（24–28 人）    ⇒ **0%**
    1 支长题 small_team（11 人）   ⇒ **0%**

⇒ **题写得越短，队里靠任务无关通道进来的人越多。**
而题短正是**用户给的信息最少、最需要路由替他判断**的时候。

★ 顺带核过一处**产品是诚实的**（我先猜它错了，读源码发现猜错）：
`routing_observability.selected_with_zero_task_relevance` 用的
`TASK_RELEVANCE_COMPONENTS = (task_similarity, scenario_match, capability_match, user_value_match)`
—— **正确地把 `packet_similarity` 排除在外**。它报 `[]` 与上面的 35 人**不矛盾**：
它数的是「被选中的人里**四项全 0**」，口径不同。**先猜后验，猜错就写清楚猜错了。**

SKILL.md 准入那一行补上这段。**产品一个字没改**，Task #135 同步。

## v0.0.0.39

**用户拿到的要么是 1 个人，要么是 9 个人以上 —— 2–8 这一整段取不到。**

只靠推断（不给 `--size` / `--mode`），`persona_expert_target` 的可达值：
...
⇒ **2–4 结构上不可能**；**5–8 合同允许但推断不到** —— 门一过（c=0.3838）、
各分量落地板（r=0.08、domains=1）就是 `5 + 6(0.3838) + 3(0.08) + 1 = 8.54 → **9**`。
要 <9 就得 c<0.38，而那时已经掉回 `single_expert`。

**分水岭有多陡**：31 个无意义词 ⇒ **1 人**；**32 个 ⇒ 9 人**。
真内容那边同样陡：「修复登录接口的空指针崩溃并补回归测试。」（19 字）⇒ **1 人**；
再接一句真话（52 字）⇒ **12 人**。

★★ **出口存在，但它要求的正是它要绕开的那件事**：`--size 5..8` 被接受，
前提是**推断出的模式已经是 `small_team`**；任务短到 `single_expert` 时 `--size 6` 被拒
（`valid: 1–1`）⇒ **想要 6 个人，得先把任务写到足以拿 9 个人**。
何况 `--size` 是运行时旗标，**不是用户在自然语言里能表达的东西**。

**守卫**：新增 `scripts/check_team_size_ladder_has_no_hole.py`（0.4s，7 项自测）——
含 **★★ 公式复算必须等于产品读数**（不靠样本）、**★★★ 出口正例**（small_team 下
`--size 5..8` 全收）与 **★★★ 出口反例**（single_expert 下 `--size 6` 必须被拒）。
守的是「**推断可达的人数档位不许再少**」，**不是**「必须补上那个洞」——
补洞要改公式或改门，会移动每一道任务的人数（Task #134）。
＋ 调用方 `tests/test_team_size_ladder_has_no_hole.py`（6 项）。
SKILL.md「已测量的边界」表新增「**我能拿到几个人**」一行。

**产品一个字没改。**

## v0.0.0.38

**谁来干哪个包，89% 由一个不含任何任务信息的量决定。**

这是 v0.0.0.36 那件事的**第二个下游**：同一批模板 objective，
除了在准入门里放人（v0.0.0.36），还在**包分派**里决定谁拿哪个包。

`assign_packets`（`route_team_moe.py:344–356`）：

    compatibility = overlap_score(packet["objective"], candidate_text(item["card"]))
    score = compatibility + 0.28*base_score − 0.08*载荷 − (0.25 若超载)

**机制**（12 个独立题面、auto 档现算）：出现过的**不同 objective 只有 28 条** ——
5 条固定阶段包 ＋ 23 条「独立处理第 N 个证据／方案／对象分片」（N 只是编号）。
**没有一条带任务内容。** ⇒ `compatibility(人, 包)` 是一张**与任务无关的常量表**。

  ★ 唯一一次「objective 里出现题面片段」是**反向**的巧合：某道题面自己写了
    「证据缺口」，而模板里本来就有「建立事实、来源、未知、当前性和**证据缺口**地图。」

**消融**：12 支多于 1 人的队伍、包合计 173 个，把 `compatibility` 置 0 ⇒
**154 / 173 = 89.0% 的包换了主人**。

★★ **射程写在同一句里**：对照臂（只剩 `0.28*base − 载荷`）**本身是退化的**
（全队按 base 排、载荷惩罚轮转），所以 89% **不能读成「89% 的分派是错的」**。
它只支持一条：**决定谁拿哪个包的，主要是一个与任务无关的量。**

★ 过程中先跑了 4 道手写题，其中 **3 道路由只给 1 人** ⇒ 分派没有选择余地，
  那三行不构成证据，已弃用；换成 12 个独立题面里真正多人的队伍重测。

**复算脚本进版本控制**：`_ledgers/_pipeline/measure_packet_assignment_ablation.py`
（5 项自测：产品源码里那四个常量还在／compatibility 仍取自 `objective`／
objective 里不含任务内容／★★★ 反例「置 0 后第一个包必须归 base 高的那位」）。
上一次同类复算跑在会话临时目录里，路径随会话消失 —— 这次不重犯。
已登记进 `check_cited_scripts_ship_with_the_package.py` 的包外名单。

SKILL.md「已测量的边界」表新增「**谁来干哪个包，由什么决定**」一行。
**产品一个字没改。**

## v0.0.0.37

**分歧检出 0/72，而随机队伍是 22/72 —— 路由在系统性地把唯一检得出分歧的那些人分开。**

用**权威抽取器**（`build_team_dossier.extract_divergences`，逐个开 102 份交付包
读 `divergence-map.md`，只认全名或 slug 在 **≥40 字段落**里的精确出现）现算：

    102 人 ⇒ 5151 个配对；可互相点名的 **24 个 = 0.47%**（与旧记录一致）
    24 个**全部同族**：investor-capital-allocator 17、software-developer 7，**跨族 0**

    72 道 oracle 全量：路由队伍（中位 11 人）含可检出对的 —— **0 / 72**
    同样大小的**随机**队伍（200 次重抽）—— 中位 **22 / 72 = 30.6%**，200 次里 **0 次** ≤0

⇒ **不是碰巧没凑到。** 多样性配重要求同族第 2 人 `base_score` 领先 **0.1382**，
而唯一检得出分歧的配对**恰好全是同族** —— **多样性与分歧检出结构性冲突**。

★ 这是第一次真的跑通权威抽取器。此前我说「不给 ⑥ 报任何新数」是对的：
  我自己写的抽取器按工作区目录名对 `subject_slug`，少了「开包读 divergence-map.md」
  这一层加载、也少了段落切分 ⇒ 0 对是**我的扫描面为空**，不是事实。

★★ 本轮又栽了一次同样的跟头，被自己的打印抓住：第一版取队伍写 `rt.get("team")`，
  而 `build_route` 返回的键是 **`members`** ⇒ 72 支队伍全空、`0/72` 是假的。
  **是「队伍规模：中位 0 人（0–0）」这行打印救的**。已在复算脚本里加 `assert min(sizes)>0`。

**守卫**：新增 `scripts/check_divergence_pairs_survive_extraction.py`（0.3s，7+1 项自测）——
守的是「**可检出配对数不许掉到 24 以下**」，因为检出链条有三处会静默断掉：
段落被切碎到 <40 字／`canonical_name` 写法改了／重打的包里没有 `divergence-map.md`，
**三种都会让 `divergences` 恒为 `[]`，而下游读起来像「专家一致」**。
含**退化对照**（清空全部 `divergence_text` ⇒ 必须 0 对）与**切碎反例**（段落 <40 字 ⇒ 必须掉）。
自测 ⑥ 第一版写成 `a == b + (a-b)` —— **恒等式不是断言**，已换成「族标签取得到且不止一种」。
＋ 调用方 `tests/test_divergence_pairs_survive_extraction.py`（6 项，含
「地板抬高必须判红」与「`extract_divergences` 仍然会 pop 掉输入」）。

SKILL.md「分歧检测能不能命中」一行按新读数重写（旧行写的「24 个任务里 0 个」
用的是另一份 24 条样本，且当时没跑通权威抽取器）。

**产品一个字没改。** 改多样性配重会移动每一道任务选出的人 ⇒ 属「门、席位一概不动」。
