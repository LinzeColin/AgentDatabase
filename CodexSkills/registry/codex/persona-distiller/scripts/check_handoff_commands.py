...
## ★★★★ 射程：**只管命令，不管路径引用**——路径那一版已量过，**不做**

2026-08-11 有人（我）提议「HANDOFF 里的路径引用也该落成判据」。**先量了再说**：
...
1. **裸文件名**（`check_handoff_commands.py`、`_索引.md`）——正文就是这么引用的，
   文件真实存在，只是不在我拼的那几个前缀下（要全仓搜，不是拼路径）；
2. **目录概念**（`GithubProject/`、`_protected/`、`evals/`）——不是具体路径；
3. **省略号示意**（`.../references/pipeline/checkers/`、`.../workspaces`）；
4. **分支名**（`claude/character-distillation-…`）——根本不是路径。

★★ 抽查五个可疑的（`_生成索引.py`、`check_filename_year_vs_ledger.py` …）
**仓内全部命中**（`check_*` 各 2 处 = `scripts/` + 镜像树，正是合同要求的）。

→ **不落这道判据。** 要落得先解决那四类，而每一类都要单独量假阳；
**成本已超过它能挡的东西**——2026-08-11 核出 HANDOFF 六处过期，
**没有一处是路径失效**，全是数字（pack 大小、文件数）与状态（教训库、Grotius）。

★★★ 记下来是为了省掉下一次的弯路：**这个方案被自己的数据否掉了，别再提一遍。**
