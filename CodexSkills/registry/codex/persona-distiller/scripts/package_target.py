...
def run_quality_gate(target: Path, *, skip: bool,
    ...
    if completed.returncode != 0 or not payload.get("passed"):
        # ★ v0.0.0.131：此前这里只印 `errors`。而 **strict 下 warning 也会让 passed=False**，
        #   于是「因某条 warning 被拦」会印成 `strict release quality gate failed: []`
        #   ——**一个空列表当失败理由**，看起来像判据自己坏了，而不是「你有一条没过」。
        #   Carver #127 实测：卡的是 `eval.baseline-not-capability-evidence`（基线不可作能力证据），
        #   而报出来的是 `[]`。**空默认值把原因吞掉了**，这在本项目是反复出现的一类。
        errs = payload.get("errors") or []
        warns = payload.get("warnings") or []
        why = []
        for e in errs:
            why.append("✗ " + (e.get("message") if isinstance(e, dict) else str(e)))
        for w in warns:
            why.append("⚠（strict 下 warning 亦拦） "
