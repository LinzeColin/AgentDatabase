...
#: 抓源方在语料文件顶上写的**出处表头**：`SOURCE:` 开头，以一整行 `=` 收尾。
#: ★★★★ 它是好东西（出处、权利依据、同名者排除依据都在里面），**不要删它**。
#: 坏的是**判据把它当成语料正文读回去**。实测（2026-08-10，全库 .txt）：
#:   comfort-avery-adams  144 份带头，**表头占全文中位 39.1%**
#:   charles-l-coffin      36 份带头，表头占全文中位 16.1%
#: 后果已量到两处：
#:   ① `check_source_dedup`：Adams 两两 2556 对里 **1764 对（69%）** 重叠 ≥0.30；
#:      共有的词片就是表头的句子（`a adams mixed case directly followed by prose`）。
#:   ② `check_claim_source_independence`：Adams **6 条断言**被判「证据塌缩」。
#: ★ 与 [[i-almost-deleted-guards-that-blocked-my-number]] 同型——那次是 Barton 的
...
def read_corpus_text(path: Path) -> str:
    """读一份语料并剥掉出处表头。**凡是要把语料当「他的话」来量的判据都该用这个。**"""
    return corpus_body(path.read_text(encoding='utf-8', errors='ignore'))
