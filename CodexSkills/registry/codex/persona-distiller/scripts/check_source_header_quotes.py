...
## ★★★ 第一个真实用户就把它绕过去了（2026-08-05，Thomson #129）

抓源方交付 56 份语料，本件报 **0 缺陷 / 0 条头部引文**——
**因为它的头部里一个引号都没写。** 而抓源方自己另做了一次
「33 条头部主张逐条对正文」的检查，**抓出 3 处它自己写下的真缺陷**：

1. 把 OCR 连写词 `Itis` 悄悄分开成 `It is`；
2. 把 `54 'Deniscn 'Stra` 修成 `54 Denison Street`；
3. 头部引的目录行**落在所取切片之外**。

**这三处本件一处也看不见。**

> **它的射程不是「头部里的主张」，只是「头部里被引号包起来的主张」。
> 不写引号，它就全盲——而缺陷照旧发生。**

★ 这与「泄题门查不到直角引号」是同一类（待裁定 ⑬）：
**判据只覆盖它数得出来的那个通道，换个写法就绕过去了。**
★★ 所以本件的 `0` **永远要连同「头部引文 N 条」一起读**；
N 为 0 时它说的是「**这里没有我能看的东西**」，不是「这里没问题」。

★★★ 不修的理由：要抓「不带引号的头部主张」，就得判断哪一句是在声称照录——
**那是语义活，不是正则活**。抓源方用的是逐条人工比对，本件做不到，
**硬做只会造出一堆误报，而误报会让人学会忽略这道门。**
