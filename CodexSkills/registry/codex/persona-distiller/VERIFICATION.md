# Release verification — Persona Distiller v0.0.0.94

Date: 2026-08-04（v0.0.0.83，下表已重跑）
...
| **全部检查器自测（v0.0.0.47 起逐件跑）** | **50 / 50 passed，0 未过、0 缺 `--self-test`**（本次实跑：`通过 50　未过 0`；`check_checkers.py` 审 47 件：`可用 47 / 未过 0 / 不可独立验证 0 / 无负对照 0`）（v0.0.0.48 时 4 件根本没有 `--self-test`，见下节） | 逐件 `check_*.py --self-test` |
