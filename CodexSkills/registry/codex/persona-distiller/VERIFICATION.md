# Release verification — Persona Distiller v0.0.0.81

Date: 2026-08-04（v0.0.0.81，下表已重跑）
...
| **全部检查器自测（v0.0.0.47 起逐件跑）** | **48 / 48 passed，0 未过、0 缺 `--self-test`**（本次实跑：`通过 48　未过 0　无 --self-test 0`；`check_checkers.py` 审 45 件：`可用 45 / 未过 0 / 不可独立验证 0 / 无负对照 0`）（v0.0.0.48 时 4 件根本没有 `--self-test`，见下节） | 逐件 `check_*.py --self-test` |
