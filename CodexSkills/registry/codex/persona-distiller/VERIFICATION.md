...
> 判据 **93** 件、Python 脚本 **136** 件、checksum 全量校验 **499 files**、身份族 **12** 族、**名册 102 人**。
>
> ★★★ **2026-08-17 第二十三次更新：工具改动后的复验 —— 三项重跑，且 ★不升版**
>
> 自 v0.0.0.154 落版（`bfe16379a`，08-14）以来 registry 侧改了 **90 个文件**：
> 工具（scripts/tests）41、工具（references，`example-knuth/` 就在这下面）43、
> 发布元数据 5、SKILL.md 1 —— **人物产物改动 0 个**。
> 依 2026-08-04 裁定「判据落地算工具改动，不动版本号」，**本轮不升版**。
> （我又跑了一次 `bump_version.py v0.0.0.155`，**与下面第 335 行记的是同一个错**，已全部还原。）
> 但改的是**判据本身**，不是文档，所以下面三项是**真的重跑了**：
>
> | 项 | 本次实测 | 上一次记录 | 命令 |
> |---|---|---|---|
> | 全部检查器自测 | **93 / 93 通过，0 失败，9.6 秒** | v0.0.0.148 记的是 66 件 | 逐件 `scripts/check_*.py --self-test` |
> | 离线测试套件 | **绿 18｜红 1**（红的那件是待 Owner 裁定的路由期望值，非回归）；墙钟 324.7 / 342.9 / **344.2** 秒（三次，并行度 4） | v0.0.0.96 记的是 70/70 | `scripts/run_tests.py` |
> | 合同漂移门 | **0 条（rc=0）** | v0.0.0.96 实跑 0 条 | `scripts/check_contract_drift.py` |
>
> ★ **没重跑的**：下表 150 行里其余那些**对真工作区跑的门**（`check_rights_basis`
> 对 11 份账本、`check_answer_surface_leak` 对 Barton 真工作区、Osler 两轮长度回验…）。
> 它们各自的行里都写着当时的版本号，**照旧有效地代表那一次**，本次**未重新测量**。
> ⇒ **本文件不是「v0.0.0.154 全量复验通过」**，是「三项重跑 + 其余沿用各自版本的记录」。
>
> ★★ 并行度实测顺带定了一个参数：`run_tests.py` 从 8 路改 **4** 路 ——
> 8 路省 23% 时间但制造 1–4 盏**假红**（测试自己的 `run_script` 被压超时）。
>
> ★★★ **订正（同段，08-18）**：我按 4 路的**第一次**跑就写下了「4 路 0 假红」，
> **下一次跑就翻了号**（1 盏假超时）。一次观测撑不起一个全称判断。
> 查清后真因不在并行度：全套 19 件里**只有一件**带墙钟断言
> （`test_checkers_actually_run.py` 的「全量扫一遍 <90 秒」+ 180 秒单件超时），
> 并行时它量到的是**机器负载**而不是判据。⇒ 加 `SERIAL_ONLY`，把它放在并行批**之后**
> 单独跑；名单由自测**现扫**「谁有墙钟断言」来核，对不上就红（反例实验：清空名单 ⇒ rc=1）。
> 改后实测：该件 70.3 秒绿。
> ★★★★ **再订正（同日，第二次被自己的下一次实测打脸）**：我写完就说「假超时只出在这一件上」，
> 下一次跑红的是 `test_quality_and_eval.py`（用 `helpers.run_script(timeout=60)`）。
> 两件事**危害不同**：墙钟**断言**在负载下给出**错误判定**（必须独占跑）；
> 子进程**超时**只是噪声，且 runner **已正确归成「超时，不是回归」**。
> `SERIAL_ONLY` 只收第一类，**它没有也不该消灭所有假超时**
> （串行实测 ~818 秒 vs 并行 ~350 秒，不值得为噪声把整套串行化）。
>
> ★ **2026-08-19 第二十七次更新：checksum 500 → 499，包内文件数 501 → 500** ——
>    **不是我减了东西，是把越界的那一个并回去了。**
>    ① 硬查：`029a19699`（08-18 加 `test_show_gate_survives_mixed_shapes.py`）的**父提交是 500 文件**，
>       该提交本身 **501** —— 上限 ≤500 就是那一刻被越过的，**当时没重跑文件数门，没人看见**。
>    ② 我随后加 `fetch_gallica_alto.py` 两份，顶到 **503**；全量测试 9 项失败全是同一句
>       `package has 501 files; must remain <=500`。
>    ③ 处置一律「不抬门」（沿用第二十五次的裁定）：
>       · ALTO 通道并进 `emit_corpus_pointer.py`（它本就在做「按记录过的通道取回语料」）
>       · 那个测试**一条不删**，6 个用例并进 `test_markdown_report_handles_string_items.py`
>         —— 两者守的是同一族缺陷：生产者同时发 dict 与裸字符串，两个消费者各崩一次
>       · 顺带清掉 `dist/` 里两个交付 zip 残留（原件在 `~/Downloads/蒸馏/`，46 份）
>    ④ 现状实测：盘上 **500** 文件、checksum **499** 行、`test_markdown_report_handles_string_items` **12 个用例全过**。
>    **只订正这两个数，没有重跑整份复验**；纯工具侧改动，依 2026-08-04 裁定不升版。

> ★ **2026-08-18 第二十六次更新：checksum 499 → 500** —— 新增
>    `tests/test_show_gate_survives_mixed_shapes.py`（`show_gate` 在真工作区抛栈的守卫）。
>    ★ 顺带记一笔判据的**过度匹配**：`check_contract_drift` 把「checksum行数」的候选值
>    数成 `[499, 404]`，而 **404 是 Release bundle 的 checksum**（另一件产物），
>    与源码树那个数无关。**同名不同物，判据把它们并到一处比了。**
>
> ★ **2026-08-18 第二十五次更新：checksum 499 → 500 → **回到 499****
> —— 一次被两道门连着拦下来的改动，如实记全过程：
> ① 我新增 `references/pipeline/scaffold/baseline_provenance_template.md`（对照侧母版），
>    checksum 变 500；**合同漂移门当场抓到**（`check_contract_drift` rc=1：实况 500 / 文中 499），
>    我据此把文中改成 500。
> ② 紧接着 `test_release_bundle` 红：**`package has 501 files; must remain <=500`**
>    （硬上限在 `scripts/self_check.py:136`）。
> ③ **没有抬那道门** —— 为塞进自己的文件去放宽判据，正是本仓一直在挑的毛病。
>    改法：那份母版是**纯散文**（不像 `answers_template.py` 要被 import），
>    正文并进本来就随包的 `references/pipeline/RUNBOOK.md`，独立文件删掉。
>    包回到 500 文件 / checksum 499，纪律照样随包，两棵树仍逐字节相同。
> **只订正这一个数，没有重跑整份复验**；纯工具侧改动，依 2026-08-04 裁定不升版。
>
> ★ **2026-08-18 第二十四次更新：checksum 498→499** —— 新增 `references/ledgers/_周报_2026-08-18.md`（账期 08-13→08-18）。
> **只订正这一个数，没有重跑整份复验**；本文件首行版本号因此不动（本轮改动为纯工具侧，依 2026-08-04 裁定不升版，见 CHANGELOG）。
>
