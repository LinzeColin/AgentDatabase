# Release verification — Persona Distiller v0.0.0.16
...
| Gate | 本次结果 | 证据来源 |
|---|---:|---|
| Offline unit / integration / concurrency tests | **64 / 64 passed** | `python3 -m pytest tests/ -q` |
...
| 蒸馏版本新鲜度 | 100 / 100 ≥ 下限 `v0.0.0.6`；**其中 70 条是上界值** | `check_distillation_freshness.py` |
| Release checksum 全量校验 | passed，274 files | `self_check.py` |
| Canonical group validation | **12 categories, 98 products, 100 artifacts**; passed | `validate_persona_registry.py` |
...
| Python script 覆盖 | 48 scripts | `self_check.py` |
...
| **Complete-release deterministic rebuild** | **passed**（自 v0.0.0.9 起首次可构建，v0.0.0.15 解绑锁步） | `test_complete_release_is_one_deterministic_zip_and_installs_both_skills` |
...
## 必须与「100/100 达标」一起说的一件事

新鲜度那一行的 **70 条是上界值**——来自 `a31cb12d` 的十二族重组，
那次**只重打包没重蒸**，正文实际来自 ≤`v0.0.0.5`。
单说达标率是误导；收窄的唯一途径是重蒸，已排进 600 人完成后的收尾序列。
