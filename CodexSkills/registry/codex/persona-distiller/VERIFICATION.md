# Release verification — Persona Distiller v0.0.0.30
...
>
> 本次（v0.0.0.30）**是真的重跑了一遍**，下表每一行都是本次实跑输出。
...
| **★★ 归属依据门（v0.0.0.30 新增，硬拦，见下节）** | **passed**（负对照 10 项，含「争议为空」与「没查过」必须分开）；**真工作区实测**：Galen 工作区未声明依据时 exit 1 | `check_attribution_basis.py --self-test`、对 `ws-galen` 实跑 |
| **★★ 事实密度门（v0.0.0.28 新增／v0.0.0.30 分账本与人物，只报不拦）** | **passed**（负对照含 **4 条真实样本**）；Galen 实测 15 条 `fact` → **人物事实 10 条、账本事实 5 条不计入**，仍 < 要求 12 | `check_fact_density.py --self-test` |
| 检查器元普查（负对照有没有） | **15 件中 10 OK / 4 无负对照 / 1 不可独立验证**（v0.0.0.22 时为 11 件 6 OK） | `check_checkers.py scripts/ --json` |
| **★★ 真实夹具普查（v0.0.0.30 新增，只报不拦）** | **0 / 15 件的负对照里含真实样本夹具**——见下节 | `check_checkers.py scripts/` |
| 蒸馏版本新鲜度 | 下限 `v0.0.0.20`；**102 条中 3 达标 / 99 低于下限 / 0 未知**；见下方说明 | `check_distillation_freshness.py` |
...
## ★★ v0.0.0.30 新增：归属依据门——**印刷时代之前的人物，靠什么证明是他写的**
...
## ★★ v0.0.0.30：**「名 + 姓」是一个西方近代假设，名册里大量人物不满足它**
...
### 三之前：**15 件检查器里，0 件的负对照含真实样本**（v0.0.0.30 新增普查）
