# Release verification — Persona Distiller v0.0.0.51

Date: 2026-08-04（v0.0.0.51，下表已重跑）
...
>
> 本次（v0.0.0.51）**是真的重跑了一遍**，下表每一行都是本次实跑输出。
...
| **★★★★★★★★★ 长度泄题门（v0.0.0.51 新增，**硬门**，见下节）** | **passed**（**10 项自测，其中 5 组反向对照，正/反夹具都取自实测形状**）；**Osler #110 两轮回验与当时手工结论逐位吻合**（R2 1.30／9 更短、R3 1.30／14 更短）；**接线三支退出码实测** 0／1／3 | `check_answer_length_leak.py --self-test` + 对 Osler 两轮实跑 |
