...
| **全部检查器自测（v0.0.0.47 起逐件跑）** | **`v0.0.0.137 实跑`：判据 **63** 件（新增 `check_persona_frame_break`、`check_first_person_density`；另新增工具件 `stamp_authorship_evidence`）、接线审计**在生产代码里找不到调用方的 0 件**；语料射程审计 该扫 **18** 个工作区、实际扫了 **18** 个；★ 新增「台账有、工作区没有」审计：7 人 16 份，其中一手 10 份 | 逐件 `check_*.py --self-test`、`check_checkers.py scripts` |
...
| Release checksum 全量校验 | passed，**399 files**（`v0.0.0.137 实跑`；v0.0.0.96 写的 341、v0.0.0.75 写的 305 都是旧数——**这一行漂过三次，v0.0.0.111 起由 `check_verification_counts.py` 盯着**） | `self_check.py` |
