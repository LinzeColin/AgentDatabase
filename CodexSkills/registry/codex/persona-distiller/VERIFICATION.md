# Release verification — Persona Distiller v0.0.0.154

Date: 2026-08-05（**v0.0.0.154**）
...
> ### v0.0.0.154 这一版重跑了什么
>
> **抓源实测打回来两个真 bug，都在我昨天刚建的 `check_namesake_criteria` 里。**
>
> ① **排除名单没带词首边界**：MMJ v.XIII p.205 的页眉被 OCR 打成 `IT. C. Sorby`，
> 那是**目标本人**的页眉，字面里却含建筑师的 `T. C. Sorby`——**他自己的文章会被判成他人**。
> ★ 这是「丢材料」的方向，**少了的东西不会报错**，比误收更难发现。
> ★★ 第一版的修法把空白抹掉再比，`By T. C. Sorby` 变成 `byt.c.sorby`，
> **词首边界被自己抹没了**，真建筑师反而漏判——**抹空白与查边界不能同时做**。
> 改成把名字编成「记号间允许任意空白」的正则，边界在**原文**上查。
>
> ② **区分符全篇找**：A8 是他自己的文，正文写着 `Professor Clifton's laboratory at Oxford`
> ——那是 R. B. Clifton，牛津物理学家。**那一篇碰巧确实是他的，结论对而理由错**。
> 改成只认贴着姓氏 40 字以内的。
>
> 两条已固化为反向对照⑥⑦，自测 16 项全过。
>
> **`finalize_release` 把两道秒级检查提到 pytest 之前**：
> 同一个循环今天走了 5 次（升版 → 等两分钟 → 一堆红 → 全是 VERIFICATION 标题没改）。
> 植入陈旧标题实测：**1 秒报出，此前要等约 2 分钟**。
> ★ 省的不是机器时间，是「等两分钟才知道自己漏改一行」——**慢速失败只会让人跳过检查**。
...
> ### v0.0.0.151 这一版重跑了什么
>
> **同名护栏比的是「名 + 姓」，而 Sorby #133 的父亲也叫 Henry Sorby。**
> 实跑 `ocr_byline_evidence("By Henry Sorby.", "Henry", "Sorby")` → **判为目标本人**。
> 新增 `check_namesake_criteria.py`：按人物定制判据，先排除后确认，
> **「说不准」单列成 `unknown`，不许并进任何一边**（10 项自测，其中 5 组反向对照）。
> ★ 这次是**抓源之前**测出来的——上一次同形事故（GE 总裁被当成焊接发明人署名）是入库之后。

> ### v0.0.0.150 这一版重跑了什么
>
> **两道门各自报绿，而人工读同一批输入读出大量违规。两条都在判据侧修好了。**
>
> **门 1「rubric 抄答案」报 `0/16`。** 冻结指令写着「中译与压缩也算抄」，
> 而这道门的 `MIN_RUN=24` 是**权重**、中日韩按 2 计——**中文要 12 个字才够**，
> 实测违规全在 3–5 字（「说清已有」4、「算不过」3、「不是格言体」5）。
> 新增中译/压缩层（连续串 ≥4 字／引号内短语被**部分回声**／3-gram 包含度），
> **Bessemer 实测 0/16 → 11/16**，与席 E 手数（10+2）与 `diag_rubric_gloss_overlap`（11/2）**三方吻合**。
> ★ 两侧都比：**基线侧是趋同下限**（写判据时它还不存在，抄它不可能）；只比一侧会把趋同算成污染。
> ★★ 标定踩了两次——越线门槛先写成 8 字报 0/16、引号层先要求整句出现漏掉 4 字回声，
> **两次都是拿真数据跑了才发现的**。
>
> **门 2「出戏门」报「判据把资料层答案指定为正确 0 题」。**
> 根因：`SOURCING_SUITES` 的豁免对**答案**是对的，套到**判据**上就错了——判据是要求不是回答。
> `hb-fact-preservation-01 ②` 要求「照录扫描件里的排印异常」，
> **contrast / trajectory / voice 三题因此长出「扫描件按版面分词」的脚注**，
> 又被同一道门的产物侧记成「产物出戏」。★ 那三处脚注是我自己加的。
> 已改：判据侧不再按套组豁免（答案侧原样保留）＋ 共现归因。
> **实测：判据 0 → 1 题；产物 3 题 → 0 题。**
>
> ★★★ 本版有**两条既有反向对照锁的是缺陷**，已改判并写明理由
> （出戏门「对照④」、同名门 `test_no_candidate_is_ready_and_does_not_block`）。
> **判据一改，锁着旧行为的夹具与断言会立刻现形——那是信号，不是麻烦。**
>
> **未动 `judge_prompts/v1.md`**（按人物冻结）。修法全在判据侧。

> ### v0.0.0.149 这一版重跑了什么
>
> **表面泄题门里有一条通道，它的「合并」本身在制造盲区。**
> `CHANNELS` 原有一条 `粗体/反引号`（正则 `\*\*|\``），把两个特征合成一条。
> Bessemer #132 第 2 轮实测：**粗体 候选 0/16、基线 11/16；反引号 候选 9/16、基线 0/16**——
> **两条各自都是「看见就能定侧」的完美线索**。
> 而定向可利用率数的是「只有一侧具备」的题，合并之后两侧都算具备，
> **互相抵消 → 31%，轻松过门**。已拆成两条。
> **五人复测粗体 0/69/0/56/0%、反引号 62/56/0/19/0%，都仍在门下——拆开不新判掉任何人。**
>
> ★ 席 D 独立发现（它的机制说错了：称两者从不同时出现，**实测同时出现 6/16**；结论对）。
>
> 新增「并集可定侧」**只报不拦**：五人 **50/81/81/88/94%**，四个越 75%。
> 不设硬门是因为设了会当场判掉 Adams #131（发布门已过、正在打包）——**那要人裁，见 ㉓**。
>
> ★★ 回归对照已固化，且**它自己的第一版断言是错的**：
> 我写「并集必须 >75%」，夹具跑出来 **69%**——夹具要证的本来就不是这个，
> 是「合并只报 12%、并集报 69%，**合并把它藏起来了**」。已改成夹具真正证明的那件事。
...
> 看它会不会照印肯定句」这套扫法固定下来（那次是手工扫的，**手工扫出来的
> 东西没有主人**）。实跑 12 件、如实报出 10 件因缺必填项测不到；
> 反对照：撤掉 check_rights_basis 的修复后本件 **rc=1 并点名它**。
...
> ★★ **2026-08-11 第三次更新**：判据 86→**89**、脚本 123→**130**、checksum 460→**474**。
> 当日新增 `check_longs_corruption.py`（长 s 讹字面板）与 `emit_lane_scope.py`（研究道 Scope 表由台账现算），
> 两件各在 `scripts/` 与 `references/pipeline/checkers/` 落一份，故 checksum +4；
> 第 5 项是 `references/pipeline/checkers/00-next_person.py-是遗留的-别跑.md`——
> 它在上一个提交就入库了，而**那次没重算 checksums**，这次一并补上（逐项 diff 出来的，不是推的）。
> ★ 同样按判据写死的口径独立复算，不是抄输出：
> `ls scripts/check_*.py | wc -l` = 89、`ls scripts/*.py | wc -l` = 130、`wc -l < checksums.sha256` = 474、products = 102。
> ★★★ **2026-08-12 第四次更新**：判据 89（不变）、脚本 130（不变）、checksum 474→**475**。
> **+1 是 `tests/test_checkers_actually_run.py`**（`PACKAGE_MANIFEST.json` 那行是改不是增）——
> **这一条是 diff 出来的，不是推的**：`git diff -- checksums.sha256 | grep '^+[^+]'` 只有这两行。
> ★★ **报出这次陈旧的，是当天刚接进漂移门的 `check_verification_counts`**——
> 接线的下一次提交它就抓到了我自己。若不接，这个 474 会一路带进移交树。
> ⇒ 与 [[a-checker-nothing-calls-is-not-a-checker]] 同一件事的正面证据：**判据要有调用方才算数。**
> ★ 独立复算：`ls scripts/check_*.py | wc -l` = 89、`ls scripts/*.py | wc -l` = 130、
>   `wc -l < checksums.sha256` = 475。
> ★★★ **2026-08-12 第五次更新**：判据 89→**90**、脚本 130→**131**、checksum 475→**477**。
> 新增 `check_selftest_reach.py`（判据的自测有没有走到它自己的判定函数），
> **scripts/ 与 references/pipeline/checkers/ 各一份，故 checksum +2，判据 +1**。
> ★ 这三个数**又是判据报出来的，不是我记得改的**——同一道门在同一天抓到我两次
...
> ★ **第八次当时**现算：判据 92、脚本 133、清单 482。（历史记录，不是最新值——最新见第九次。）
...
>   `wc -l < checksums.sha256` = 481。
> ★★★ **2026-08-12 第六次更新**：判据 90→**91**、脚本 131→**132**、checksum 477→**479**。
> 新增 `check_paper_lanes.py`（纸面道：某道的全部支撑都来自多道源），scripts/ 与
> references/pipeline/checkers/ 各一份 ⇒ checksum +2；另 VERIFICATION 与漂移门自身改动。
> ★ **这三个数是现算的，不是手写的**：上一次我手写 476 而正文写着「+2」（475+2=477），
>   同一道门当场抓住。本次直接把 `ls | wc -l` 的输出喂进替换。
> ★★ **同一天里这道门抓到我七次**——镜像缺件、清单未重算、自报数字三轮。
>   [[a-checker-nothing-calls-is-not-a-checker]] 的正面证据：接线当天就开始还本。
> ★ 独立复算：`ls scripts/check_*.py | wc -l` = 91、`ls scripts/*.py | wc -l` = 132、
>   `wc -l < checksums.sha256` = 479。
> ★★ **我第一版把这个数写成 476，同一段正文里却写着「故 checksum +2」——
>   叙述与算术自相矛盾，门当场抓住。** 475+2=477。
>   ⇒ [[self-reported-numbers-must-be-computed]]：手写就必然往好里漂，**要现算**。
> ★★★ **2026-08-12 第九次更新**：判据 92（不变）、脚本 133（不变）、checksum 482→**483**。
> **+1 是 `references/ledgers/_变异装置本身要先验-2026-08-12.md`**——判据没增，只多了一份台账。
> ★★ **又是这道门抓的**：我提交台账的那条命令里 `build_manifest` 跑了、漂移门也跑了，
>   而我先看的是 `git log` ——**漂移门报 rc=1 时我已经提交完了**。
>   ⇒ 规矩仍是那条（[[a-gates-scan-set-is-smaller-than-reality]] 第 ④ 行）：
>     `checksums.sha256` 管**整个 skill 目录**，改台账、改文档一样要重算。
> ★ 三个数**现算**（`wc -l`/`ls | wc -l` 的输出直接喂进替换，没手打）：
>   判据 92、脚本 133、清单 483。
> ★★★ **2026-08-12 第十次更新**：判据 92（不变）、脚本 133（不变）、checksum 483→**484**。
> **+1 是 `references/ledgers/_拒答溢出门-整代人物空过-2026-08-12.md`**。
> ★ 同批还改了四件判据的自测（quote_integrity / anchor_coherence / material_split /
>   refusal_overflow）与 `quality_check` 的一处接线，**那些是改不是增**，不进清单行数。
> ★ **第十次当时**现算：判据 92、脚本 133、清单 484。（历史记录，最新见第十一次。）
> ★★★ **2026-08-12 第十一次更新**：判据 92（不变）、脚本 133（不变）、checksum 484→**485**。
> **+1 是 `references/ledgers/_判分产物三种形状-2026-08-12.md`**。
> ★ 那份台账记的是一次**停在报数之前**的调查：想给 ㉛ 补「历史 delta 分布」，
>   算完发现全库 `results.jsonl` 是三种形状、两条臂（`baseline` 26 份 vs `bare_model` 2 份），
>   同一个人能算出两个都对、但回答不同问题的数 ⇒ **那张分布表作废，改为记录「取不出来」**。
> ★ **第十一次当时**现算：判据 92、脚本 133、清单 485。（历史记录，最新见第十二次。）
> ★★★ **2026-08-12 第十二次更新**：判据 92（不变）、**脚本 133→134**、checksum 485→**487**。
> **新增 `collect_honest_delta.py`**（`scripts/` 与 `references/pipeline/checkers/` 各一份）
>   ——**「诚实 delta」的唯一口径**。
> ★ 它不是判据（不叫 `check_*`、不进 92 件那一列），是**取数工具**：把三种形状
>   （长表 28／宽表 9／空文件 1）、两条臂（`baseline` 26／`bare_model` 2）、
>   两种量纲（**按观测最大分判**——Barton 那份宽表本来就是 0–1）收成一张表。
> ★★ 它读的是数据里**本来就有**的 `round` 与 `rubric_fed`——我先按席位去猜，猜对了形状，
>   而字段一直在那儿。改读之后 Thomson **−0.0859**、Rosenhain **+0.0018** 与台账**逐位吻合**。
> ★ **第十二次当时**现算：判据 92、脚本 134、清单 487。（历史记录，最新见第十三次。）
> ★★★ **2026-08-12 第十三次更新**：判据 92（不变）、脚本 134（不变）、checksum 487→**488**。
> **+1 是 `references/ledgers/_门与分辨力-全库实测-2026-08-12.md`**——㉛ 剩下那半问的答案：
>   **门从 12.7 SE 放宽到 1 SE，通过人数只从 10 变到 14**（`+0.0359…+0.0803` 是 8.1 SE 空带，
>   现行 standard 0.05 与 deep 0.07 判出的是同一批人）。
> ★ **第十三次当时**现算：判据 92、脚本 134、清单 488。（历史记录，最新见第十四次。）
> ★★★ **2026-08-12 第十四次更新**：判据 92→**93**、脚本 134→**135**、checksum 488→**490**。
> **新增 `scripts/check_baseline_in_persona.py`（基线入戏门）**——
>   #106–112 那七个人的基线是**第三人称百科口吻**（「是南丁格尔最广为流传的著作」），
>   而 #101–104 是**入戏第一人称**；两批 delta 区间完全不重叠，
>   而**候选臂只差 +0.0320、对照臂差 −0.1183**。拿「扮演 X」比「介绍 X」，赢的是任务差异。
...
> 「实况 106，文中 104 —— **对不上**」。上一版这一行五项**全部朝偏小方向错**，
> 这一次是朝偏小方向**漏更新**：形态不同，方向一样。
