# Release verification — Persona Distiller v0.0.0.33
...
>
> 本次（v0.0.0.33）**是真的重跑了一遍**，下表每一行都是本次实跑输出。
...
| 检查器元普查（负对照有没有） | **18 件中 13 OK / 0 未过 / 4 无负对照 / 1 不可独立验证**（v0.0.0.22 时 11 件 6 OK） | `check_checkers.py scripts/` |
