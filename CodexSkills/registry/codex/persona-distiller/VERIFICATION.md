# Release verification — Persona Distiller v0.0.0.44
...
>
> 本次（v0.0.0.44）**是真的重跑了一遍**，下表每一行都是本次实跑输出。
...
| **★★★★★ 引号形态（v0.0.0.44）** | **passed**；**Pasteur #106 实测：扩形态前答案里 11 条外语引文只扫到 4 条，**7 条法文 «» 从未被核过（64%）**；扩后 22 条全扫、0 未命中 | `check_quote_integrity.py --self-test`（新增法文 «» 真实夹具） |
