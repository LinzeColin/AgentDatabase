# Release verification — Persona Distiller v0.0.0.34
...
>
> 本次（v0.0.0.34）**是真的重跑了一遍**，下表每一行都是本次实跑输出。
...
| 检查器元普查（负对照有没有） | **19 件中 14 OK / 0 未过 / 4 无负对照 / 1 不可独立验证**（v0.0.0.22 时 11 件 6 OK） | `check_checkers.py scripts/` |
| **★★ 真实夹具普查（v0.0.0.31 新增，只报不拦）** | **3 / 18 件的负对照里含真实样本夹具**（v0.0.0.32 时 2 / 17，v0.0.0.31 时 1 / 16） | `check_checkers.py scripts/` |
