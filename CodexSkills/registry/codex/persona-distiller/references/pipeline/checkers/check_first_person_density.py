...
def scan(root: pathlib.Path) -> dict:
    ...
    if n and tot_c:
        out["★★ 参照"] = (
            "Coffin #130 实测 **8 句 / 0.5 每万字**——三道 quick 门全过，"
            "而他自己说的实质的话只有 8 句。**门数的是来源，不是声口。**\n"
            "\n★★★★ **另一把尺（词数），2026-08-10 由三次实测校准出来的**：\n"
            "\n| 人物 | 一生第一人称文字合计 | 实际结果 |\n|---|---|---|\n"
            "| Nasmyth #153 | ≈ **8,075 词**（1836 证词 21 段答话 1,440 ＋ 1841 论文 6,635） "
            "| 两轮盲判 **+0.00265，测不出差别** |\n"
            "| Sellers #154 | ≈ 3,700 词 | **记延后**（不到 Nasmyth 一半） |\n"
            "| Nowlan #155 | **0**（最宽松折算 675，且二人共有） | **记延后** |\n"
            "\n★ **Nasmyth 那个 8,075 是已知的失败下界**——它是**做完整轮判分之后**"
            "得到的「测不出差别」。**低于 8,000 词就不要排期了，做完也是测不出。**\n"
            "\n★★ **这把尺与本件上面那些数不是同一个口径**：\n"
            "上表数的是「逐字口述答话 ＋ 署名论文正文」的**全部词数**，\n"
            "本件数的是「动词锚定且非套语的第一人称**句子**」——**两者不可互换，也不许相加**。\n"
            "要用上表那把尺，得单独去数（口述记录按发言人切、论文按正文全算）。\n"
            "★★★ 专利／标准文本**一律不计入声口**：那是「I claim」的律师体，"
            "撑得起 fact，撑不起 voice。")
    return out
