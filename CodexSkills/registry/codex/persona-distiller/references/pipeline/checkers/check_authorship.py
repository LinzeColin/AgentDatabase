...
def latin_genitive_evidence(text, first, last):
    ...
    **「写给弟弟的信」不是「弟弟写的」**；而同样的形式也出现在**弟弟自己的书**里
    ...
    ## ★ 试过并否掉的一条：**按重复次数分**（2026-08-11 实测）

    想法：版口每页都印，题名页只印一次 → 取「只出现一次的行式」。
    **在真语料上不成立**，三份拉丁本实测：

    | 源 | 短行命中 | 行式种数 | 只出现 1 次的行式 |
    |---|---:|---:|---|
    | `djbp_1646_lat` | 5 | 1 | **0** |
    | `djbp_1853_lat_vol1` | 1 | 1 | 1（**总共就一处，无从分辨**） |
    | `epistolae_1687_lat` | 37 | 5 | 4 —— 而这 4 种是 `HUGONLS`／`GROTIL` 之类**同一串的 OCR 噪声** |

    根因：**题名页那一行与版口的字面完全相同**（都是 `HUGONIS GROTII`），
    所谓「唯一」的只是同一串被 OCR 读坏的几个变体。
    **频次分不开它们，因为它们本来就是同一个字符串。**

    ## ★ 第二条候选：**按位置（题名页在最前）**——也否掉了，**负对照直接证伪**

    `EXT_delaet_notae_1643_lat`（**De Laet 写的，不是他写的**）首处属格在 **0.05%**，
    与目标人物自己那几本的题名页（0.00%–0.29%）**完全同一档**。
    结构性原因：**反驳性著作的题名页必然在最前面用属格点被驳者的名字**
    （`NOTAE AD DISSERTATIONEM HUGONIS GROTII…`）。
    → 位置分不开「他的书」与「关于他/驳他的书」。[[related-to-him-is-not-written-by-him]]

    ## ★★ 第三条候选：**属格之前有没有「反驳/注释」标记** —— 实测能分开

    在首处属格**之前 300 字符**里找 `DISSERTATIONEM|RESPONSIO|NOTAE|CONTRA|ADUERSUS|
    ANIMADUERSIONES|CENSURA|EXAMEN`。Grotius 语料 10 份实测：
    2 份他著全部命中标记、8 份自著全部无标记。

    ★★★ **两个前提，缺一个就测不出来**（我各踩了一次）：
    1. **姓名模式必须容忍字间空格**——`Responsio 1644` 的题名页印作 `G R O T I I`，
       不容忍就抓不到题名页，首处会掉到后面的序言里，结论跟着错。
    2. **标记词同样要容忍**——`NOTAE` 被 OCR 拆成 `N O ' T M`、`AD` 拆成 `A D`；
       第一版我用 `\bNOTAE\b` 扫，**七份全报「无标记」**，差点据此否掉这条候选。
    办法：`r"[^A-Z]{0,3}".join(list(w))`，两边都用。

    ## ★★ 第四条：**版口带页码** —— 五条 Bessemer 护栏实测全过

    撤回接线的那两条护栏（`328 HENRY BESSEMER` / `HENRY BESSEMER 328`）
    有一个现成特征：**版口紧挨裸页码**。加一条「行内含裸 1–4 位数字则拒绝」：

    | 夹具 | 期望 | 页码规则 |
    |---|---|---|
    | `328 HENRY BESSEMER` | 拒绝 | ✓ 拒绝 |
    | `156 HENRY BESSEMER` | 拒绝 | ✓ 拒绝 |
    | `HENRY BESSEMER 328` | 拒绝 | ✓ 拒绝 |
    | `Be it known that I, HENRY BESSEMER, of…` | 认出 | ✓ 无页码 |
    | `By HENRY BESSEMER, F.R.S.` | 认出 | ✓ 无页码 |

    而 Grotius 那几份题名页行**一份都没被误挡**。

    ## ★★★ 但我的探针自己有一个假阳，所以上面的「10/10」要打折

    `epistolae_ineditae_1806_lat` 的「首处属格」实际匹配到的是
    **`https://archive.org/details/hugonisgrotiiepiOO`——一个 URL**，不是题名页。
    它恰好前面没有反驳标记，于是被「分对了」，但**理由是错的**。
    → 真实成绩是 **9 份真题名页 + 1 次凑巧**；接线前必须加 URL/路径护栏。

    ## 结论（**仍然不接线**）

    四条候选里两条被否、两条看起来可行，但：
    - 样本只有**一个人物、一种语言、10 份**；[[samples-cannot-support-universal-claims]]
    - 只测了 5 条 Bessemer 护栏，**没跑本件完整自测**；
    - 探针自身的 URL 假阳说明**姓名模式放宽后会吃到路径与元数据**。
    接线之前要做的三件：①加 URL/路径护栏 ②在另外至少两个印刷时代人物上复测
    ③跑完整自测确认没有别的护栏变红。
    """
