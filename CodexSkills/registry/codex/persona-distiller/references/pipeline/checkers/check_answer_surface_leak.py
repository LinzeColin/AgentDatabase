...
MAX_EXPLOIT = 0.75
# ★ 与别的门直接冲突的通道：只报不拦（见 verdict() 里的理由）
REPORT_ONLY_SURFACE = ("圆括号注", "★ 标记")
...
CHANNELS = {
    ...
    "空行分段":   re.compile(r"\n\s*\n"),
    "破折号":     re.compile(r"——"),
    # ★★★ v0.0.0.140 追加两条，Adams #131 实测出来的——**门报「全过」的同一份载荷上**：
    #     圆括号注  候选 13/16 对 基线 3/16 = **62%**
    #     ★ 标记    候选  7/16 对 基线 0/16 = **44%**
    #   两条都比当时门里最高的那条（反引号 62%）不低，而**一条也没被量过**。
    #   ★ 判据自己的收尾句一直写着「只说明已知通道堵上了，不代表没有别的通道」——
    #     这回就是那句话应验了，而且是我拿别的字符去手扫才发现的。
    #   ★★ 圆括号这条**是被另一道门逼出来的**：引文坐标门要求坐标写在引文同一段里，
    #     我于是写成「（卷 XXXVIII pt.1，1919）」——**两道门互相拉扯**，
    #     与已记的「格式与长度两道泄题门叠起来五个人零通过」是同一种形状。
    #     所以本条**只报不拦**：拦了就等于要求「引文不许带坐标」。
    "圆括号注":   re.compile(r"（[^）]{2,}）"),
    "★ 标记":     re.compile(r"★"),
}
    ...
    **判据打了绿灯，盲判其实已经破了。**
    ...
    for name, s in m.get("surface", {}).items():
        if s["exploit"] > MAX_EXPLOIT:
            # ★★ 这两条**只报不拦**，理由不是宽容，是它们与别的门直接冲突：
            #   `圆括号注` —— 引文坐标门**要求**坐标写在引文同一段里，
            #     写出来就是「（卷 XXXVIII pt.1，1919）」。拦它等于要求「引文不许带坐标」。
            #   `★ 标记`  —— 出戏门与 rubric 都**鼓励**把推测/重建显式标出来，
            #     而裸模型基线不用这个记号。拦它等于惩罚「把话标清楚」。
            #   → 两条都记进 `_soft_surface`，由调用方原样印出。**免拦不等于免报。**
            (soft if name in REPORT_ONLY_SURFACE else bad).append(f"**「{name}」能指认 {max(s['cand_only'], s['base_only'])}/{m['n']} = "
