...
def main() -> int:
    ...
    if bad:
        print(f"\n✗ **{len(bad)} 个工作区里找不到语料**——"
              "账本、断言、文档都还在，而 `primary_ratio`、引文核查、覆盖率"
              "**会对着虚空算**：")
        for name in bad:
            print(f"    {name}")
        # ★ **「工作区里没有」不等于「语料没了」。**
        #   Livermore #100 实测：当初的裁定就是「语料放 D 档不进 git」
        #   （7.9 MB 一个人物、600 人就是 4.7 GB）。本判据只看工作区内，
        #   报「找不到」是如实的，**但把它读成「丢了」是过头的**。
        #   —— 反过来也不许自动放行：D 档那一份与账本只有 143/536 字节相同，
        #   **「在别处」与「在别处且是同一份」是两句话。**
        print("  **「工作区里找不到」不等于「语料没了」**——"
              "先去看这个人物当初是不是裁定放在 D 档（`~/Downloads/蒸馏/_corpora/`）。\n"
              "  **但找到了也要逐份核校验和**：Livermore #100 的 D 档压缩包"
              "与账本只有 143/536 字节相同，其余是另一次抓取。")
    return 1
