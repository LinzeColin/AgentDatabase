...
> ### ★★★ 先说清楚：**上面这三条，到 v0.0.0.154 为止一条都没有载体**（2026-08-18 实测）
>
> 这一节标着「硬门」，而**它是这份文档的第一屏** —— 读的人正是要据此决定用不用。
> 所以下面的数必须摆在同一屏：
>
> | 硬门那一条 | 实测 |
> |---|---|
> | ① rubric 每条事实断言挂 `source_id` | `cases.jsonl` **54 份**、带 rubric 的题 **1174 道** ⇒ rubric **内部**出现 `source_id`/`src-` 的 **0 / 1174** |
> | ② 至少一名评委被要求「跳出 rubric 独立核查」 | 评委指令模板 4 份 **命中 0**；各工作区实发的评委指令 30 份 **命中 0** |
> | ③ 评委指出 rubric 有误须同修并重判 | 无产物可查（②从未发生，③也就无从触发） |
>
> **那句话在全仓只出现在三处：本文件和两份 RUNBOOK —— 全是描述规则的散文。**
>
> ★ **一个极容易被当成合规的陷阱**：数据里**确实有** `src-` 号，但它们在
> `holdout_source_ids`（893 道有此字段、184 道非空）——那回答的是
> 「**哪些源被扣住不给人物看**」，**不是**「这条断言由哪个一手条目支撑」。
> **两个不同的问题共用一种号码。**
>
> ★★ **这个洞已经有一次实测代价**：Pasteur #106 的 `lp-voice-01`，rubric 与题面钉着
> 「1881 年 12 月 10 日」，而一手原文（CR t.92, `src-4d783fdbfc30`）写的是
> `Le 10 décembre **dernier**` —— **相对日期**，按刊期定年只能是 **1880-12-10**。
> **错了整一年，而它同时是产物的事实、也是尺子的刻度**，活过了三轮判分。
> 假如这条断言按①挂上 `source_id`，就必须有人打开那份源 —— **一打开就会看见 `dernier`**。
>
> ★★★ **为什么至今不补②**：要先答一个更前面的问题 —— **评委做得到吗？**
> 本项目已实测「评委没有语料，验不了引文」；「独立联网核查」在现有装置里能否执行
> **尚未验证**。做不到就只是换一种空转，并会造出一道永远红不了的绿灯。
> 而且评委指令是**按人物冻结的尺子**，改了此前分数就不再可比 —— **这是 Owner 的决定，不是清理。**
>
> **判据**：`_ledgers/_pipeline/check_rubric_independent_verification_gate.py`
> （**不随包分发**，在开发台账树；它是**报告不是门**，永远 rc=0，理由同上）。
...
### 蒸馏实测铁律（v0.0.0.7+，三线程 200+ 人验证，违反即返工）

- **style-decoy（无数字题）的“中文数字一”盲区**：核实答案里"一言以蔽之/一面镜子/一份/每一/一句/一生/身份"等含"一/份/句/生/年"的词——判据把 `[中文数字]{1,3}字` 当「自报字数」、把所有中文量词当数字。**答案全文禁 `一/二…十/每/几/两/份/项/句/束/袋/年`**；引文坐标用《书名》不用〔年份〕。
- **trajectory（生平密集题）锚点必须逐条列全**：把 rubric 的完整锚点清单（含 OCR 引文）逐条写进生成提示，生成后**逐锚点 grep 校验**；含糊的"按 notice 转述"会被第一人称自述顶掉，提示里必须给"据编者所记…"口吻示例。
- **重建盲判载荷后旧 judge 分数全部作废**：build_blind_payload 每次重新随机化 q 编号，旧 judge_D/E_g1/g2 按 q 存的分数全错位。**必须清空 4 个 judge 文件整份重判**（不是只重判改动题）；判分脚本逐题落盘支持断点续判。
- **conversations 道的手写信 OCR 可能全灭（Edison #180 实测）**：19 世纪手写信 OCR 常为乱码（0-19 词）。**探源时实测抓 1-2 封核 OCR 可用性**；不可用则借 expression 道演讲转述替代或延后，别硬凑 3 道。
- **quick 档第 3 道用 decisions 道补齐（Babson #234 实测）**：JSTOR Early Journal Content 的政论/政策建议类文章可归 decisions 道（免费开放、PD 稳），补足 3 道门槛。
- **答题/判分生成完必须合并分片后检查完整**（Wanamaker #193 实测）：`--only` 重跑会覆盖分片文件导致已生成题丢失；合并后逐 case 校验 32/32 齐再 build payload。
