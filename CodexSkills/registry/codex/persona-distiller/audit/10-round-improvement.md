...
| 8 | Runtime identity/version prompts burdened callers, while repeated distillations lacked the correct release counter | Removed the runtime identity gate and invocation versions; added automatic internal routing, optional unnumbered audit, and per-person `0.0.0.1..999` product registration with locking and contiguous validation | direct-call/router tests, unnumbered audit concurrency, first/next/999/exhaustion/failed-registration tests |
