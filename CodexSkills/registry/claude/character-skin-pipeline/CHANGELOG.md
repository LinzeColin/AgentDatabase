...
- 新增 `references/19-local-sdxl.md`：本地 SDXL 零成本路线的**已验证边界**。
  画质/成本/零审核三项本地赢；身份保真（27 个 LoRA 只覆盖 8%）与吞吐（555 秒/张、
  414 变体约 64 小时）两项没过关。
- 记录 **IP-Adapter 是错的工具**：weight 0.7 两种失败模式（画面长出两个人 / 渲染扁平劣质），
  用户两张全否；不加锚图只用 LoRA 的反而更好。别再试第二遍。
- 记录环境坑：`--bf16-unet` 在 MPS 上让 SDXL 出 NaN 但仍报 success；
  ComfyUI 的 queue/history 是内存态重启即清（判产物一律扫盘）；
  同一配方连败 2 次就停手做最小复现，别换 seed 重试。
- 记录一条合规判断：**OpenAI 的审核以前是没算进去的兜底**（67/110 次拒绝挡下了 67 张过线素材），
  切本地后合规 100% 落在自己的 G 轴闸门上，该由 T00 拍板。
