...
| 已入库人物档案 | **102** |
| 在制工作区 | **53** |
| 语料（★ 两处布局，口径见下） | **821 份** ＝ `raw/` 245 ＋ `references/sources/` 245 ＋ 其它布局 331 |
| 断言 | **1,012 条** |
| 盲判用例 | **1,046 题**（50 个工作区） |
| 记延后/拒发 | **186 条**（都写明了理由与解锁条件） |
...
**2026-08-17 按 `git ls-files` 口径逐个数过 53 个工作区**（＝你 clone 之后真正拿到的）：

| 情形 | 个数 | 语料在哪 |
|---|---:|---|
| 正文就在仓里 | **0** | ★ 见下面那条说明 —— 这一档已经归零，不是漏数 |
| 有重建指针 | **19** | `…/raw/_ids-rebuild.txt`（每行一个 Internet Archive identifier） |
| 正文与指针**都不在仓里** | **34** | 要重新抓；台账里有 `source_id`，但没有 `_ids-rebuild.txt` |
| **取不回来** | **0** | 0 + 19 + 34 = 53，没有第五种 |

★★★ **2026-08-17 订正：「正文就在仓里」从 32 变成 0，而且这不是回归。**
  新工作区的 `raw/*.txt` 已被 `.gitignore` 挡在仓外（「语料不进 git」那条裁定的落地），
  于是这一档随时间归零。**旧表把 34 个这样的工作区记成「空工作区（没开工，台账也空）」——
  数字错，标签也错**：它们大多台账满满，只是正文不在仓里。

★ 首屏那张六格表同日一起订正：语料 **6,494 → 821 份**（差 8 倍）、
  断言 1,011 → 1,012、记延后/拒发 185 → 186。
  **收件人第一眼看到的数字里，最大的那个错了 8 倍。**

★ 这张三列表**现在由判据管着**（`check_start_here_numbers.py` 的第 7 项），
数字漂了它会红；但它**不由 `--apply` 改**（三列结构不同且带散文），要人改。
8-13 白天写的是 24／14／10／0 共 48 个——**那时的结构对、数字旧了**。
