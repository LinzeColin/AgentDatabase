...
// 最后才是漏斗和价值路径。
// glass 没有 slab —— 它的等价物是一张整宽卡。
const card = html => `<div class="card w6" style="margin:14px 0">${html}</div>`;

export async function render(host) {
  const C = D.A().compounding;
  if (!C) {
    host.innerHTML = sec('转化', '这一块还没生成')
...
${card(`
  ...
  <div style="font-family:var(--disp);font-weight:640;font-size:19px;letter-spacing:-.02em;margin:6px 0 8px">${esc(C.champion.problem || '—')}</div>
...
${(C.champion.evidence || []).length ? `<p class="hint"><b>依据：</b>${
  ...
  // ── B. 做了很多，但还没变成结果 ──
  const debt = (C.debt || []).length ? (C.debt || []).map(d => card(`
  <div class="ck">${state('看这里')}　${esc(d.kind_label)}　·　${esc(d.size_label)}</div>
  <div style="font-family:var(--disp);font-weight:640;font-size:19px;letter-spacing:-.02em;margin:6px 0 8px">${esc(d.title)}</div>
