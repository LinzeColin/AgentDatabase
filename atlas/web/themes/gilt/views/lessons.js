...
${L.repeats.slice(0, 14).map((r, i) => slab(`
  <div class="ck">${String(i + 1).padStart(2, '0')}　问了 ${r.n} 遍　横跨 ${r.days} 天　
    <span class="lnk" data-day="${r.first}">${r.first}</span> →
    <span class="lnk" data-day="${r.last}">${r.last}</span></div>
  <div style="font-size:14.5px;color:var(--fg);margin:8px 0 11px;line-height:1.65;max-width:74ch">${esc(r.text)}</div>
...
${orbit(L.pain.map(p => ({ k: p.name, v: p.per, label: `${p.per.toFixed(1)} 次提及/场（共 ${p.sessions} 场）` })))}
