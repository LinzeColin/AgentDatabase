...
export async function render(host) {
  ...
  host.innerHTML = `${sec('矩阵')}
${lede(`把这段时间摊成一张表：每一行是一类活，每一列是一周，格子越深表示那一周在这类活上花得越多。
  这样看得出「什么时候换了方向」，而不是只看到一堆点。`)}
<div class="ctl">
...
${figure('<canvas class="viz" id="mx"></canvas>', '悬停任一格看那一周的数；右侧是该行合计。')}`;
  ...
  const draw = () => {
    ...
    rows.forEach((r, ri) => {
      ...
      weeks.forEach((wk, i) => {
        ...
        ctx.fillRect(labelW + i * cw + .5, y + 3, Math.max(1, cw - 1), rowH - 6);
      });
      ctx.globalAlpha = 1;
