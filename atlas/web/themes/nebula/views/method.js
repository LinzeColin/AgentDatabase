...
${warn(`状态只有四个词：<b>通 / 断了 / 没做 / 不确定</b>。算不出来的写「不确定」，不写「没问题」。
...
${drawer('agent 密集扇出', table([{ t: '时段' }], (m.fanout_hours || []).map(f => [esc(f.when)])))}
${sec('算法')}
...
${warn('<b>费用不估算。</b>不同模型、不同缓存命中、不同套餐的单价都不一样，拿不到真实账单就标「不确定」。')}
${sec('主题归并')}
