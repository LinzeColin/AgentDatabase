...
export function RealityCalibrationPanel() {
  ...
  return <section className="ma-reality-panel" aria-labelledby="ma-reality-title" data-run-id={snapshot.run.run_id} data-trace-id={snapshot.run.trace_id} data-release-id={snapshot.release.release_id ?? "UNVERIFIED"} data-deployment-revision={snapshot.release.deployment_revision ?? "UNVERIFIED"}>
    ...
    <article className="ma-benchmark" data-oracle="benchmark_state" data-oracle-value={snapshot.benchmarks.state}><div><p>全球公开参照</p><h3>{humanizeMachineText(snapshot.benchmarks.state)}</h3></div><p>{snapshot.benchmarks.limitations.join(" ")}</p><ul>{snapshot.benchmarks.comparisons.slice(0,4).map((row,index) => <li key={String(row.benchmark_id ?? index)}><b>{String(row.label_zh ?? row.benchmark_id ?? "参照")}</b><span>{humanizeMachineText(String(row.comparability_state ?? "NOT_COMPARABLE"))}</span><small>{String(row.reason_zh ?? "口径不足")}</small></li>)}</ul></article>
