...
function isSnapshot(value: unknown): value is PrivateAnalyticsSnapshotV31 {
  ...
  return row.schema_version === "memory_atlas.private_analytics.v1"
    && contract?.mode === "private_full_fidelity_read_only_analytics"
    && contract.writeback === "proposal_only"
...
async function fetchSnapshot(signal?: AbortSignal): Promise<PrivateAnalyticsSnapshotV31 | null> {
  ...
  if (response.status === 403) throw new Error("私有分析读取未通过 Owner 身份验证");
...
export function PrivateAnalyticsProvider({ children }: PropsWithChildren) {
  ...
  const requestAction = useCallback(async (action: "capture-request" | "diagnose" | "restore-drill") => {
    ...
    const result = payload as ActionResponseV31;
    if (["WAITING_SOURCE", "QUEUED", "RUNNING"].includes(result.state)) {
      window.setTimeout(() => void refresh(), 2500);
      window.setTimeout(() => void refresh(), 7500);
    } else {
      void refresh();
    }
    return result;
  ...
  const value = useMemo(
    () => ({ snapshot, state, error, refresh, requestAction }),
    [error, refresh, requestAction, snapshot, state],
  );
