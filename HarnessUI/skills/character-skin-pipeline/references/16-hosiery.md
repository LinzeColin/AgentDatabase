> ⚠ **2026-08-24 迁移：丝袜配置已升为 I 轴，唯一真源搬到**
> `04_DouyinOps/00_治理与登记/05_素材维度/I-腿部服装.csv`（类型 7 × 颜色 8 × 质地 6 = 336 组合，
> 原 `research/hosiery.json` 只有 9 个扁平枚举）。**skill 只读不写。**
> 选择规则见同目录 `I-腿部服装-选择规则.csv`；用法见 `17-axes.md`。
>
> 同时修正一条方向错误：旧 auto_rules 的「浅色/和风系→白丝」把一批角色推向语料稀薄的组合
> （`white_thighhighs` 34.6万 vs `black_thighhighs` 50.3万），出图质量会系统性偏低。
> 新默认是 `thighhighs/black/sheer`。
